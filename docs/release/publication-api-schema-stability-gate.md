...
technical-preview repository has v0.2.4 selected-channel publication proof for GitHub pre-release,
...
That contract validates stable v1 machine-readable fields for local and package-installed v0.2.4
...
| --- | --- | --- | --- |
| `api_compatibility_window` | local_v1_contract_ready | Published API stability tiers, compatibility window, deprecation policy, breaking-change approval. | Local v1 stable-field aliases and deprecation policy are declared for the v0.2.4 technical preview. |
...
| `package_identity_approval` | selected_channels_ready | Approved package identities, channel ownership, naming, install/uninstall/rollback proof. | GitHub/TestPyPI/PyPI/Homebrew are published and proof-backed for v0.2.4. |
...
| `checksum_manifest` | selected_channels_ready | Checksum manifest tied to the approved release artifacts and source revision. | The v0.2.4 GitHub download proof verifies the attached checksum manifest and each selected release asset; broader signing/attestation remains separate. |
| `sbom_bundle` | selected_channels_ready | Approved Rust, Python and CLI SBOM bundle. | The v0.2.4 GitHub download proof verifies the attached SBOMs and provenance; image artifacts and future channels remain outside this release. |
| `publication_approval` | selected_channels_ready | Explicit maintainer approval, release notes, tag approval, package-channel gate pass. | Maintainer approval and channel proof exist for the v0.2.4 GitHub, TestPyPI, PyPI, and Homebrew sequence. |
...
package-channel, or broad runtime claims. Selected v0.2.4 package publication is proof-backed; the
