...
technical-preview repository has v0.2.2 selected-channel publication proof for GitHub pre-release,
...
That contract validates stable v1 machine-readable fields for local and package-installed v0.2.2
...
| --- | --- | --- | --- |
| `api_compatibility_window` | local_v1_contract_ready | Published API stability tiers, compatibility window, deprecation policy, breaking-change approval. | Local v1 stable-field aliases and deprecation policy are declared for the v0.2.2 technical preview. |
...
| `package_identity_approval` | selected_channels_ready | Approved package identities, channel ownership, naming, install/uninstall/rollback proof. | GitHub/TestPyPI/PyPI/Homebrew are published and proof-backed for v0.2.2. |
...
| `publication_approval` | selected_channels_ready | Explicit maintainer approval, release notes, tag approval, package-channel gate pass. | Maintainer approval and channel proof exist for the v0.2.2 GitHub, TestPyPI, PyPI, and Homebrew sequence. |
...
package-channel, or broad runtime claims. Selected v0.2.2 package publication is proof-backed; the
