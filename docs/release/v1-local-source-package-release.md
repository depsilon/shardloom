...
1. Merge the v0.2.3 release-prep source revision.
2. Create the GitHub v0.2.3 release and attach source, wheel, sdist, CLI, checksums, SBOM, and
   provenance assets.
...
5. Publish PyPI through Trusted Publisher/OIDC and run the clean registry install/uninstall smoke
   transcript.
6. Publish the Homebrew tap formula against the immutable GitHub v0.2.3 source archive and run
   `brew install`, `shardloom status --format json`, and `brew uninstall` proof.

The completed publication proof records:

- release version and tag: `v0.2.3`
- selected channels: GitHub pre-release, TestPyPI, PyPI, Homebrew
- exact source revision: `79f1ad6d96345d9ef9b36b89159e4d9ee673fb9c`
- release notes
