...
Status: selected v0.2.3 release channels are published and proof-backed. This document does not
...
The selected v0.2.3 release-channel IDs and shared status vocabulary are centralized in
...
Selected v0.2.3 rows are ready only when channel-specific install, uninstall, clean-install, smoke,
...
- PyPI and TestPyPI require Trusted Publisher/OIDC posture. Long-lived upload tokens are not
  release-grade for the public package path. The v0.2.3 Trusted Publisher uploads and
...
- Every package channel now carries a v1 feasibility decision in the JSON matrix. GitHub
  pre-release, TestPyPI, PyPI, and Homebrew are included and proof-backed in the v0.2.3 package
...
| --- | --- | --- | --- | --- | --- | --- |
| GitHub pre-release | GitHub v0.2.3 pre-release assets | included channel proof passed | `gh release download v0.2.3 --repo depsilon/shardloom --pattern '*' --dir <install-dir>` | `rm -rf <install-dir>` | Approved tag/release, attached checksums, SBOM, provenance, clean download/smoke transcript, rollback/delete policy. | `ready`: `docs/release/channel-proofs/github-prerelease-v0.2.3-transcript.json` verifies attached assets, checksums, and CLI smoke. |
| TestPyPI | Python package `shardloom` | included channel proof passed | `python -m pip install --index-url https://test.pypi.org/simple/ --no-deps shardloom==0.2.3` | `python -m pip uninstall -y shardloom` | TestPyPI Trusted Publisher/OIDC, isolated no-cache registry download, downloaded-artifact SHA256 binding, local wheel install, smoke, no committed token. | `ready`: `docs/release/channel-proofs/testpypi-v0.2.3-transcript.json` verifies isolated no-cache registry download, local install from the downloaded wheel `shardloom-0.2.3-cp313-cp313-macosx_26_0_arm64.whl`, Python client smoke, uninstall, and downloaded SHA256 `fd52d0f2e2a24d585fdef8bfb571d7b6988f9366e8fdcabfbb4a4c916dc1a9d3`. |
| PyPI | Python package `shardloom` | included channel proof passed | `python -m pip install shardloom==0.2.3` | `python -m pip uninstall -y shardloom` | PyPI Trusted Publisher/OIDC, prior TestPyPI proof, isolated no-cache registry download, downloaded-artifact SHA256 binding, local wheel install, smoke, SBOM/checksum/provenance, yank policy. | `ready`: `docs/release/channel-proofs/pypi-v0.2.3-transcript.json` verifies isolated no-cache public registry download, local install from the downloaded wheel `shardloom-0.2.3-cp313-cp313-macosx_26_0_arm64.whl`, Python client smoke, prior TestPyPI proof ref, uninstall, and downloaded SHA256 `b1d563359643955165064a2f10fbf3dd8c70ea9c6924ae7c640437034bdb6588`. |
| Homebrew tap | CLI formula | included channel proof passed | `brew install depsilon/tap/shardloom` | `brew uninstall shardloom` | Tap/formula proof, versioned artifact checksum, install/uninstall, smoke, rollback/deprecate policy. | `ready`: `docs/release/channel-proofs/homebrew-v0.2.3-transcript.json` verifies tap audit/style/test, source build install, CLI smoke, and uninstall. |
...
The current matrix is ready for the selected v0.2.3 package channels only. It is not evidence for
