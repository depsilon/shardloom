...
Status: selected v0.1.4 release channels are published and proof-backed. This document does not
...
The selected v0.1.4 release-channel IDs and shared status vocabulary are centralized in
...
Selected v0.1.4 rows are ready only when channel-specific install, uninstall, clean-install, smoke,
...
- PyPI and TestPyPI require Trusted Publisher/OIDC posture. Long-lived upload tokens are not
  release-grade for the public package path. The v0.1.4 Trusted Publisher uploads and
...
- Every package channel now carries a v1 feasibility decision in the JSON matrix. GitHub
  pre-release, TestPyPI, PyPI, and Homebrew are included and proof-backed in the v0.1.4 package
...
| --- | --- | --- | --- | --- | --- | --- |
| GitHub pre-release | GitHub v0.1.4 pre-release assets | included channel proof passed | `gh release download v0.1.4 --repo depsilon/shardloom --pattern '*' --dir <install-dir>` | `rm -rf <install-dir>` | Approved tag/release, attached checksums, SBOM, provenance, clean download/smoke transcript, rollback/delete policy. | `ready`: `docs/release/channel-proofs/github-prerelease-v0.1.4-transcript.json` verifies attached assets, checksums, and CLI smoke. |
| TestPyPI | Python package `shardloom` | included channel proof passed | `python -m pip install --index-url https://test.pypi.org/simple/ --no-deps shardloom==0.1.4` | `python -m pip uninstall -y shardloom` | TestPyPI Trusted Publisher/OIDC, clean install, uninstall, smoke, no committed token. | `ready`: `docs/release/channel-proofs/testpypi-v0.1.4-transcript.json` verifies registry install, Python client smoke, and uninstall. |
| PyPI | Python package `shardloom` | included channel proof passed | `python -m pip install shardloom==0.1.4` | `python -m pip uninstall -y shardloom` | PyPI Trusted Publisher/OIDC, prior TestPyPI proof, clean install, uninstall, smoke, SBOM/checksum/provenance, yank policy. | `ready`: `docs/release/channel-proofs/pypi-v0.1.4-transcript.json` verifies public registry install, Python client smoke, prior TestPyPI proof ref, and uninstall. |
| Homebrew tap | CLI formula | included channel proof passed | `brew install depsilon/tap/shardloom` | `brew uninstall shardloom` | Tap/formula proof, versioned artifact checksum, install/uninstall, smoke, rollback/deprecate policy. | `ready`: `docs/release/channel-proofs/homebrew-v0.1.4-transcript.json` verifies tap audit/style/test, source build install, CLI smoke, and uninstall. |
...
The current matrix is ready for the selected v0.1.4 package channels only. It is not evidence for
