...
Status: selected v0.2.2 release channels are published and proof-backed. This document does not
...
The selected v0.2.2 release-channel IDs and shared status vocabulary are centralized in
...
Selected v0.2.2 rows are ready only when channel-specific install, uninstall, clean-install, smoke,
...
- PyPI and TestPyPI require Trusted Publisher/OIDC posture. Long-lived upload tokens are not
  release-grade for the public package path. The v0.2.2 Trusted Publisher uploads and
...
- Every package channel now carries a v1 feasibility decision in the JSON matrix. GitHub
  pre-release, TestPyPI, PyPI, and Homebrew are included and proof-backed in the v0.2.2 package
...
| --- | --- | --- | --- | --- | --- | --- |
| GitHub pre-release | GitHub v0.2.2 pre-release assets | included channel proof passed | `gh release download v0.2.2 --repo depsilon/shardloom --pattern '*' --dir <install-dir>` | `rm -rf <install-dir>` | Approved tag/release, attached checksums, SBOM, provenance, clean download/smoke transcript, rollback/delete policy. | `ready`: `docs/release/channel-proofs/github-prerelease-v0.2.2-transcript.json` verifies attached assets, checksums, and CLI smoke. |
| TestPyPI | Python package `shardloom` | included channel proof passed | `python -m pip install --index-url https://test.pypi.org/simple/ --no-deps shardloom==0.2.2` | `python -m pip uninstall -y shardloom` | TestPyPI Trusted Publisher/OIDC, isolated no-cache registry download, downloaded-artifact SHA256 binding, local wheel install, smoke, no committed token. | `ready`: `docs/release/channel-proofs/testpypi-v0.2.2-transcript.json` verifies isolated no-cache registry download, local install from the downloaded wheel `shardloom-0.2.2-cp313-cp313-macosx_10_13_universal2.whl`, Python client smoke, uninstall, and downloaded SHA256 `e81f18946ef093ca4695ac9a0512a57534cb4ce7bea2ee1c3a76f3d625634861`. |
| PyPI | Python package `shardloom` | included channel proof passed | `python -m pip install shardloom==0.2.2` | `python -m pip uninstall -y shardloom` | PyPI Trusted Publisher/OIDC, prior TestPyPI proof, isolated no-cache registry download, downloaded-artifact SHA256 binding, local wheel install, smoke, SBOM/checksum/provenance, yank policy. | `ready`: `docs/release/channel-proofs/pypi-v0.2.2-transcript.json` verifies isolated no-cache public registry download, local install from the downloaded wheel `shardloom-0.2.2-cp313-cp313-macosx_10_13_universal2.whl`, Python client smoke, prior TestPyPI proof ref, uninstall, and downloaded SHA256 `1f6a0785c90f1b4e99ed305bc040348d26ea2c52fde58a6a891e6d4387ea3b26`. |
| Homebrew tap | CLI formula | included channel proof passed | `brew install depsilon/tap/shardloom` | `brew uninstall shardloom` | Tap/formula proof, versioned artifact checksum, install/uninstall, smoke, rollback/deprecate policy. | `ready`: `docs/release/channel-proofs/homebrew-v0.2.2-transcript.json` verifies tap audit/style/test, source build install, CLI smoke, and uninstall. |
...
The current matrix is ready for the selected v0.2.2 package channels only. It is not evidence for
