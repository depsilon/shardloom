...
infer from the current repository state. v0.1.10 package channels are published where explicitly
...
Selected package channels are published for v0.1.10; real production environment gates remain
blocked.
...
```text
package_publication_status=published_v0.1.10_selected_channels
...
| SourceState and prepared-state reuse | Scoped local prepared-state reuse is closed by the SourceState/prepared-state scope, golden fixtures, invalidation matrix, and current benchmark evidence fields. The canonical claim remains scoped local prepared-state reuse. | Local SourceState normalization, workspace or explicit VortexPreparedState reuse, internal local-source smoke labeling, and deterministic invalidation proof. | Global hidden cache, external cache service, object-store/table prepared-state reuse, broad non-local preparation, production cache claims. |
...
| Package/release channels | v0.1.10 selected channels are published and proof-backed: GitHub pre-release assets, TestPyPI, PyPI, and Homebrew. | Install access through `gh release download v0.1.10`, `python -m pip install shardloom==0.1.10`, and `brew install depsilon/tap/shardloom`; channel proof refs remain checked in. | Scoop, winget, conda-forge, GHCR, crates.io, real production environment gates, signing/attestation expansion, and production/package-maturity claims remain blocked. |
...
- Package wording must distinguish source checkout, local wheel dry run, selected published
  v0.1.10 channels, blocked future channels, and production/package-maturity claims.
