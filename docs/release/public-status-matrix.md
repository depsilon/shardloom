...
infer from the current repository state. v0.2.4 package channels are published where explicitly
...
Selected package channels are published for v0.2.4; real production environment gates remain
blocked.
...
```text
package_publication_status=published_v0.2.4_selected_channels
...
- Package wording must distinguish source checkout, local wheel dry run, selected published
  v0.2.4 channels, blocked future channels, and production/package-maturity claims.
