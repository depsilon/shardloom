...
infer from the current repository state. v0.1.0 package channels are published where explicitly
...
Selected package channels are published for v0.1.0; real production environment gates remain
blocked.
...
```text
package_publication_status=published_v0.1.0_selected_channels
...
| Package/release channels | v0.1.0 selected channels are published and proof-backed: GitHub pre-release assets, TestPyPI, PyPI, and Homebrew. | Install access through `gh release download v0.1.0`, `python -m pip install shardloom==0.1.0`, and `brew install depsilon/tap/shardloom`; channel proof refs remain checked in. | Scoop, winget, conda-forge, GHCR, crates.io, real production environment gates, signing/attestation expansion, and production/package-maturity claims remain blocked. |
...
- Package wording must distinguish source checkout, local wheel dry run, selected published
  v0.1.0 channels, blocked future channels, and production/package-maturity claims.
