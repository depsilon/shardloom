...
infer from the current repository state. v0.2.3 package channels are published where explicitly
...
Selected package channels are published for v0.2.3; real production environment gates remain
blocked.
...
```text
package_publication_status=published_v0.2.3_selected_channels
...
- Package wording must distinguish source checkout, local wheel dry run, selected published
  v0.2.3 channels, blocked future channels, and production/package-maturity claims.
