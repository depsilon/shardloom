...
| `diagnostic_row_id=object_store_runtime` | Object store | `object_store_read`, `object_store_write`, cloud URI inputs, object-store fixture smokes | `SL_UNSUPPORTED_PRODUCTION_OBJECT_STORE` | `review-p0-3.object_store_runtime_and_path_safety_required` | Production cloud reads/writes, credentials, retries, range reads, and commit safety are blocked outside admitted fixture scope. |
...
| `diagnostic_row_id=public_package_publication` | Package publication | TestPyPI, PyPI, Homebrew, Scoop, winget, conda-forge, GHCR, crates.io | `SL_UNSUPPORTED_PUBLIC_PACKAGE_PUBLICATION` | `release.package_publication_gate_required` | Public package channels, tags, signing, uploads, and install claims require maintainer approval and channel proof. |
