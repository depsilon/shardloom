...
| `python_package_smoke` | `python-package` | `python -m build python`<br>`python scripts/release_dry_run_proof.py --rows 8 --iterations 1 --skip-clean-conda` | `target/release-dry-run-proof/transcript.json`<br>`release-local-smoke-evidence` | package/install smoke; local provenance dry run |
...
- `python-package` keeps the GitHub check name `Python and package smoke` for branch-protection
  continuity and emits `release-local-smoke-evidence` with the compact dry-run transcript. Package
  and binary artifacts are verified inside the producing job instead of being shipped between CI
  jobs.
