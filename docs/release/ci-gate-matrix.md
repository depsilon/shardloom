...
- `release-readiness` has `needs:` entries for `ci-gate-matrix`, `dependency-security`,
  ...
  v1 API/schema stability, and hard-readiness aggregate scripts consume precomputed evidence and
...
production usability, v1 API/schema stability, hard release readiness, and release readiness
artifact aggregation instead of serially regenerating every upstream report. The strict existence
check makes artifact wiring a fast failure without adding another long producer command to the
final job.
