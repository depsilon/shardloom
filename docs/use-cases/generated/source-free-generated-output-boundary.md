...
- **Engine mode:** `batch`
- **Claim boundary:** Scoped local user-row, literal-table, calendar/date-dimension, range, sequence, SQL VALUES, and SQL literal SELECT JSONL fixture smokes only; no broad SQL/DataFrame runtime, S3/object-store write, Foundry production, performance, production, or package-publication claim.
...
`none, generated_rows, literal_table_rows, calendar_dimension, range, sequence, sql_values, sql_literal_select -> source_free_generated_output -> batch -> local_jsonl_output_artifact, generated_source_certificate, output_native_io_certificate -> evidence -> claim gate`
