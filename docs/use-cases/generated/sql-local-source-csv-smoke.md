...
- **Engine mode:** `batch`
- **Claim boundary:** Scoped CSV, flat JSON/JSONL/NDJSON, and feature-gated flat scalar Parquet/Arrow IPC/Avro/ORC/Vortex SELECT/filter/limit, aggregate/group/top-N, ctx.sql collect/write, local sinks/fanout, joins, and join aggregates. Arrow IPC binary sources admit projection, binary literal predicates, and bytewise ORDER BY; Parquet/Arrow IPC/Avro/ORC preserve binary sinks; local Vortex preserves non-null binary sinks; Parquet/Arrow IPC/Avro preserve typed decimal sinks. Computed filters/projections admit listed scalar helpers, exact decimal exponent notation normalized to declared target scale, and bounded IN/EXISTS/ANY/ALL/correlated/projected subqueries. Blocks include lossy coercion/type/date/time errors, NULL mismatches, scalar-left multi-column, broad correlated subqueries, non-binary source columns compared to binary literals, NULL-bearing Vortex output before writer conversion, non-scoped binary sinks, ORC/Vortex typed decimal sinks, and broad binary execution/subqueries/projection trees. No broad SQL/DataFrame, production, broad format fidelity, claim-grade fanout/replay, object-store/table source, generalized joins/orderings, named-timezone/collation, fallback, or performance claim.
...
Scoped CSV, flat JSON/JSONL/NDJSON, and feature-gated flat scalar Parquet/Arrow IPC/Avro/ORC/Vortex SELECT/filter/limit, aggregate/group/top-N, ctx.sql collect/write, local sinks/fanout, joins, and join aggregates. Arrow IPC binary sources admit projection, binary literal predicates, and bytewise ORDER BY; Parquet/Arrow IPC/Avro/ORC preserve binary sinks; local Vortex preserves non-null binary sinks; Parquet/Arrow IPC/Avro preserve typed decimal sinks. Computed filters/projections admit listed scalar helpers, exact decimal exponent notation normalized to declared target scale, and bounded IN/EXISTS/ANY/ALL/correlated/projected subqueries. Blocks include lossy coercion/type/date/time errors, NULL mismatches, scalar-left multi-column, broad correlated subqueries, non-binary source columns compared to binary literals, NULL-bearing Vortex output before writer conversion, non-scoped binary sinks, ORC/Vortex typed decimal sinks, and broad binary execution/subqueries/projection trees. No broad SQL/DataFrame, production, broad format fidelity, claim-grade fanout/replay, object-store/table source, generalized joins/orderings, named-timezone/collation, fallback, or performance claim.
...
Vortex SQL sources, broad structured type/nesting coverage beyond flat scalar and scoped binary source projection/predicate/order plus Parquet/Arrow IPC/Avro/ORC and non-null Vortex binary sinks, default-build structured formats/Vortex writes, arbitrary join predicates, named-timezone/collation/null ordering, nullable/all-null Vortex binary rows and other NULL-bearing Vortex output batches before writer conversion, binary sinks outside scoped Parquet/Arrow IPC/Avro/ORC and non-null Vortex flat outputs, ORC/Vortex typed decimal sinks, broad decimal coercion beyond exact exponent normalization, broad binary execution, non-binary source columns compared to binary literals, scalar-left multi-column, unbound or broad correlated subqueries, broad grouped aggregates, broad expression projections/functions, broad predicate trees, catalogs, object stores, tables/lakehouse, claim-grade output/fanout replay, and production SQL/DataFrame support require later slices.
