...
- **Engine mode:** `batch`
- **Claim boundary:** Scoped local CSV, flat JSON/JSONL/NDJSON, and feature-gated flat scalar Parquet/Arrow IPC/Avro/ORC SELECT projection/filter/limit, aggregates, group-by, top-N, ctx.sql collect/write, local sinks/fanout, joins, and join aggregates. The feature-gated local columnar materialization boundary admits Arrow binary byte-array source columns for direct projection as `binary[hex=...]` scalar result evidence, direct binary literal predicates, and bytewise source-column ordering, with executable SQL proof through Arrow IPC. Computed filters/projections admit listed cast, null, conditional, numeric, Date32, UTC-or-fixed-offset timestamp, temporal-difference, scalar/row-value IN-subqueries including projected variants, source-qualified local subqueries, scoped correlated `outer.<column>` scalar/row-value/EXISTS/quantified filters, scoped EXISTS/NOT EXISTS, scoped quantified ANY/ALL including projected variants, and UTF-8 helpers including LIKE/NOT LIKE ESCAPE. Lossy coercion, arithmetic/type/date/time errors, NULL fallback/sentinel or CASE mismatches, scalar-left multi-column, unbound qualified or broad correlated subquery shapes, non-binary source columns compared to binary literals, binary sinks, broader binary execution beyond scoped feature-gated source projection/predicate/order, other broad non-admitted subqueries, and broad projection trees block. No broad SQL/DataFrame or production runtime, broad format fidelity, claim-grade fanout/replay, object-store/table source, generalized joins/groups/orderings, named-timezone/collation completeness, fallback, or performance claim.
...
- `expecting_broad_binary_source_sinks_or_nested_binary_execution`
