...
Feature-gated local prepare-once fixture smoke over flat non-null scalar rows, including non-null binary byte payloads. ingest_minimal records artifact/digest/writer evidence, ingest_certified reopens/scans row-count proof, and ingest_full_replay blocks until downstream replay exists. Scout ingress, layout/write advisor, copy-budget, append-only differential refinement, and capillary evidence stay inside vortex_ingest. Automatic append-only refinement is limited to local CSV/JSONL prefix-verified appends, writes only a delta Vortex artifact, leaves the base unchanged, and admits count-family consumer evidence only. Nullable/all-null binary rows and other NULL-bearing Vortex output batches block before writer conversion; malformed sources, unsupported strategies, unsafe lifetime shortcuts, non-append drift, missing manifests, changed compression/format posture, and update/delete/upsert/schema mismatch block before claims. Python session reuse is local and fingerprint-gated. No broad Vortex writer, object-store/table sink, CDC/table transaction, production SQL/DataFrame, persistent/distributed cache, performance, superiority, Foundry, package-publication, or Spark-replacement claim.
