...
- **Engine mode:** `batch`
- **Claim boundary:** Feature-gated local prepare-once fixture smoke over flat non-null scalar rows only. Certification levels are explicit: ingest_minimal records artifact/digest/writer evidence, ingest_certified reopens/scans for row-count proof, and ingest_full_replay blocks until downstream replay exists. Scout ingress, layout/write advisor, copy-budget, append-only differential preparation/refinement, and capillary evidence stay inside vortex_ingest. Automatic append-only refinement is limited to local CSV/JSONL prefix-verified appends, writes only a delta Vortex artifact, leaves the base unchanged, and admits count-family consumer evidence only. Malformed sources, unsupported strategies, unsafe lifetime shortcuts, non-append drift, missing base manifests, changed compression/format posture, and update/delete/upsert/schema mismatch block before claims. Python ShardLoomSession reuse is local and fingerprint-gated. No broad Vortex writer, object-store/table sink, standalone cold lane, CDC/table transaction, production SQL/DataFrame, persistent/distributed cache, performance, superiority, Foundry, package-publication, or Spark-replacement claim.
...
Feature-gated local prepare-once fixture smoke over flat non-null scalar rows only. Certification levels are explicit: ingest_minimal records artifact/digest/writer evidence, ingest_certified reopens/scans for row-count proof, and ingest_full_replay blocks until downstream replay exists. Scout ingress, layout/write advisor, copy-budget, append-only differential preparation/refinement, and capillary evidence stay inside vortex_ingest. Automatic append-only refinement is limited to local CSV/JSONL prefix-verified appends, writes only a delta Vortex artifact, leaves the base unchanged, and admits count-family consumer evidence only. Malformed sources, unsupported strategies, unsafe lifetime shortcuts, non-append drift, missing base manifests, changed compression/format posture, and update/delete/upsert/schema mismatch block before claims. Python ShardLoomSession reuse is local and fingerprint-gated. No broad Vortex writer, object-store/table sink, standalone cold lane, CDC/table transaction, production SQL/DataFrame, persistent/distributed cache, performance, superiority, Foundry, package-publication, or Spark-replacement claim.
