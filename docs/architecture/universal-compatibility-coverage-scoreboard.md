...
| Generated / source-free outputs | Generated source | `smoke-supported` | `shardloom.generated_source_certificate_contract.v1` exposes no-dataset, user-generated, and engine-native generated-source contract rows, and GAR-GEN-1C adds one scoped user-row local JSONL output smoke. Engine-native generator nodes remain report-only. | GAR-GEN-1A/1B own the report-only GeneratedSourceCertificate contract; GAR-GEN-1C owns the local user-row output smoke; GAR-COMPAT-1B owns compatibility/status projection. | No source Native I/O certificate is claimed when no source dataset is read; output evidence remains required. | Engine-native generator nodes, SQL/DataFrame admission rows, object-store/Foundry output proof, broader output formats. | Only one local user-row JSONL fixture-smoke generated-output runtime is supported. |
| Python rows / DataFrame | User API | `smoke-supported` | Python `ctx.from_rows([...]).write(...)` can run the scoped local JSONL generated-output smoke, but broad DataFrame runtime is not supported. | Future rows must classify `ctx.range`, `ctx.literal_table`, `ctx.calendar`, SQL/VALUES, builder methods, writes, blockers, and evidence. | No hidden pandas/Polars/DuckDB/DataFusion execution may occur. | Typed API contract, generated-source evidence, local sink evidence, no-fallback tests. | User-layer posture only; no broad DataFrame runtime claim. |
| SQL `VALUES` / literals | SQL frontend | `report-only` | SQL source-free execution is not supported. | Future rows must classify parse, bind, plan, source-free projection, and deterministic unsupported diagnostics. | No SQL execution certificate may be claimed. | Parser/binder policy, literal expression semantics, generated-source certificate, output evidence. | No broad SQL runtime claim. |
