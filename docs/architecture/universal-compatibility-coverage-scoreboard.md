...
| Generated / source-free outputs | Generated source | `report-only` | `shardloom.generated_source_certificate_contract.v1` now exposes no-dataset, user-generated, and engine-native generated-source contract rows, but no first-class user-generated or engine-native generated-output runtime is supported yet. | GAR-GEN-1A/1B own the report-only GeneratedSourceCertificate contract; GAR-COMPAT-1B owns compatibility/status projection. | No source Native I/O certificate is claimed when no source dataset is read; output evidence remains required. | Local output sink evidence, generated-row correctness evidence, Python/SQL/DataFrame admission rows, output Native I/O certificate. | No generated-output runtime claim yet. |
| Python rows / DataFrame | User API | `report-only` | Python capability and query-builder surfaces exist, but broad DataFrame runtime is not supported. | Future rows must classify `ctx.from_rows`, builder methods, writes, blockers, and evidence. | No hidden pandas/Polars/DuckDB/DataFusion execution may occur. | Typed API contract, generated-source evidence, local sink evidence, no-fallback tests. | User-layer posture only unless a scoped runtime slice lands. |
