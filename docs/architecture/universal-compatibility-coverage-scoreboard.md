...
| --- | --- | --- | --- | --- | --- | --- | --- |
| CSV | Local compatibility file | `runtime-supported` | Local CSV now routes through the shared SourceState adapter and `vortex_ingest` normalization for admitted flat schemas. Internal source smoke execution and decoded CSV output remain internal smoke-only safeguards. | Coverage rows distinguish compatibility import, prepared/native Vortex execution, and claim boundaries. | SourceState and Vortex ingest evidence are scoped to admitted local workflows; public decoded CSV export requires a native Vortex-derived export contract. | Native Vortex-derived CSV export contract, object-store/table, and production claim evidence. | Local CSV SourceState and Vortex normalization only; internal source smoke and decoded CSV output are not public workflow routes. |
| JSONL / NDJSON | Local compatibility file | `runtime-supported` | Local flat JSONL/NDJSON now routes through the shared SourceState adapter and `vortex_ingest` normalization. Internal source smoke execution and decoded JSONL output remain internal smoke-only safeguards. | Capability rows distinguish line-record JSONL/NDJSON from nested/general JSON semantics. | SourceState and Vortex ingest evidence are scoped to admitted local workflows; public decoded JSONL export requires a native Vortex-derived export contract. | Native Vortex-derived JSONL export contract, nested JSON, JSONPath, object-store/table, and production evidence. | Local flat JSONL/NDJSON SourceState and Vortex normalization only. |
...
| Parquet | Local compatibility file | `runtime-supported` | Feature-gated flat scalar local Parquet now routes through columnar SourceState and `vortex_ingest` normalization. Decoded Parquet output remains a separate compatibility export contract. | Rows separate Parquet preparation/runtime from Vortex-native execution and table/lakehouse claims. | Columnar SourceState and Vortex ingest evidence are scoped to admitted flat schemas; public decoded Parquet export requires a native Vortex-derived export contract. | Native Vortex-derived Parquet export contract, nested types, metadata fidelity, table/object-store, and production evidence. | Feature-gated flat scalar local Parquet SourceState and Vortex normalization only. |
| Arrow IPC | Local compatibility file | `runtime-supported` | Feature-gated flat scalar local Arrow IPC now routes through columnar SourceState and `vortex_ingest` normalization. Decoded Arrow IPC output remains a separate compatibility export contract. | Arrow boundaries remain compatibility/interop surfaces, not the internal execution substrate claim. | Native I/O evidence records the decode/materialization boundary; public decoded Arrow IPC export requires a native Vortex-derived export contract. | Native Vortex-derived Arrow IPC export contract, zero-copy, streaming, nested type, default-build, object-store, and production evidence. | Feature-gated flat scalar local Arrow IPC SourceState and Vortex normalization only. |
| Avro | Local compatibility file | `runtime-supported` | Feature-gated flat scalar local Avro now routes through columnar SourceState and `vortex_ingest` normalization. Decoded Avro output remains a separate compatibility export contract. | Avro remains a compatibility preparation/runtime surface. | Native I/O evidence is scoped to the admitted input path and cannot imply native Avro execution. | Native Vortex-derived Avro export contract, schema evolution, logical type completeness, object-store/table, and production evidence. | Feature-gated flat scalar local Avro SourceState and Vortex normalization only. |
| ORC | Local compatibility file | `runtime-supported` | Feature-gated flat scalar local ORC now routes through columnar SourceState and `vortex_ingest` normalization. Decoded ORC output remains a separate compatibility export contract. | ORC remains a compatibility preparation/runtime surface. | Native I/O evidence is scoped to the admitted input path and cannot imply native ORC execution. | Native Vortex-derived ORC export contract, stripe/statistics, nested type, object-store/table, and production evidence. | Feature-gated flat scalar local ORC SourceState and Vortex normalization only. |
