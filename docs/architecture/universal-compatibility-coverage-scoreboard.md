...
| Python rows / DataFrame | User API | `smoke-supported` | Python `ctx.from_rows([...]).write(...)`, `ctx.from_rows([...]).with_column(literal).write(...)`, `ctx.literal_table([...]).write(...)`, `ctx.calendar(...).write(...)`, `ctx.range(...).write(...)`, `ctx.range(...).with_column(int64_expression).write(...)`, `ctx.sequence(...).write(...)`, `ctx.sql_values(...).write(...)`, `ctx.sql_literal_select(...).write(...)`, `ctx.sql("SELECT * FROM generate_series/range(...)").write(...)`, and scoped `ctx.sql("SELECT value AS id, value + 1 AS next FROM range(...)").write(...)` can run local JSONL/CSV generated-output smokes; broad DataFrame runtime is not supported. | `shardloom.generated_source_api_admission.v1` classifies `python_ctx_from_rows`, `python_ctx_literal_table`, `python_ctx_calendar`, `python_ctx_range`, `python_ctx_sequence`, `python_generated_source_write`, SQL rows, scoped generated `with_column`, DataFrame source-free projection blockers, and evidence. | No hidden pandas/Polars/DuckDB/DataFusion execution may occur. | Typed API admission contract, generated-source evidence, local sink evidence, no-fallback tests. | User-layer generated-output posture only; no broad DataFrame runtime claim. |
| SQL `VALUES` / literals / range generators | SQL frontend | `smoke-supported` | Source-free SQL literal `SELECT`, SQL `VALUES`, `SELECT * FROM generate_series/range(...)`, and scoped `value` column/int64 arithmetic projections from `generate_series/range(...)` can run scoped local JSONL/CSV generated-output smokes with admitted literal or int64 generator arguments. SQL over input datasets, source-free projection beyond the admitted range-generator subset, arbitrary functions, joins, subqueries, and UDFs remain report-only or blocked. | GAR-GEN-1E owns the SQL admission rows; GAR-RUNTIME-IMPL-1A owns the scoped local source-free SQL smoke. Future slices must classify broader parse, bind, plan, source-free projection, unsupported diagnostics, and local-source SQL before runtime. | SQL parser, binder, planner, generated-source certificate, output Native I/O certificate, and execution certificate are claimed only for the scoped local literal/VALUES/range-generator smoke paths. | Broader parser/binder policy, expression semantics, local-source SQL planning, generated-source certificate expansion, output evidence. | Only source-free SQL `VALUES`/literal/range-generator and scoped range-projection local JSONL/CSV fixture smokes are admitted; no broad SQL runtime claim. |
