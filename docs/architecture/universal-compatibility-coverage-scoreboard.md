...
| --- | --- | --- | --- | --- | --- | --- | --- |
| CSV | Local compatibility file | `runtime-supported` | Local CSV now routes through the shared SourceState adapter and `vortex_ingest` normalization for admitted flat schemas. Direct transient execution and decoded CSV output remain internal smoke-only safeguards. | Coverage rows distinguish compatibility import, prepared/native Vortex execution, and claim boundaries. | SourceState and Vortex ingest evidence are scoped to admitted local workflows; public decoded CSV export requires a native Vortex-derived export contract. | Native Vortex-derived CSV export contract, object-store/table, and production claim evidence. | Local CSV SourceState and Vortex normalization only; direct transient and decoded CSV output are not public workflow routes. |
| JSONL / NDJSON | Local compatibility file | `runtime-supported` | Local flat JSONL/NDJSON now routes through the shared SourceState adapter and `vortex_ingest` normalization. Direct transient execution and decoded JSONL output remain internal smoke-only safeguards. | Capability rows distinguish line-record JSONL/NDJSON from nested/general JSON semantics. | SourceState and Vortex ingest evidence are scoped to admitted local workflows; public decoded JSONL export requires a native Vortex-derived export contract. | Native Vortex-derived JSONL export contract, nested JSON, JSONPath, object-store/table, and production evidence. | Local flat JSONL/NDJSON SourceState and Vortex normalization only. |
