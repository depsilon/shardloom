...
- **Planning, diagnostics, and provenance**: `pushdown proof`, `residual expression`, `lowering provenance`, `portability loss`, `intermediate artifact`, `layout health report`, `compaction planning report`, `object-store range planning report`, `object-store request coalescing report`, `object-store commit protocol report`, `fallback_attempted`, `unsupported`.
