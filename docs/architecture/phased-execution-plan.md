...
1. Real encoded read path
   ...
   - CG-1.2b.1 complete
   - CG-1.2c complete
   - CG-1.2d.2 current (deterministic async/session boundary contract; metadata/footer invocation deferred to CG-1.2d.3)
...
- CG-1.2d.6 compile probes now reference `VortexOpenOptions::open_path` method item shape without calling it; production metadata/footer invocation remains deferred because invoking open/footer would perform IO and require an approved async runtime/execution harness. CG-1.2d.8 (complete/blocked): harness ingredient decision completed. Test-only executor ingredient is available (`futures-executor` exists transitively in the `vortex-file-io` graph), but no repository-local `.vortex` fixture exists and no confirmed public no-IO `Footer` construction route is available. CG-1.2 metadata/footer execution remains open/paused pending one of those inputs.
