...
`generated_source_output`. SourceState and VortexPreparedState row contracts are now established by
GAR-IOREUSE-1A and GAR-IOREUSE-1B. Required future timing/reuse fields for the remaining
OutputPlan/fanout bundle include `operator_compute_millis`, `output_plan_millis`,
`output_write_millis`, `output_replay_millis`, `total_runtime_millis`, `output_plan_reuse_hit`,
