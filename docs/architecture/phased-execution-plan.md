...
- [x] Priority 7.4 - claim-grade compute-engine completion
  ...
  - [x] P7.4.4 benchmark taxonomy execution and measured source-backed row population bundle.
    ...
    - Completed sub-slice in ledger: `partition pruning` now executes through the ShardLoom local
      ...
      remain blocked.
    - Completed sub-slice in ledger: `many-small-files scan` and `null-heavy aggregate` now execute
      through the ShardLoom local Vortex import/replay/result-sink path for generated split
    ...
    - Completed sub-slice in ledger: CDC-overlay `small change over large base` now executes
      through the ShardLoom local Vortex import/replay/result-sink path using an explicit generated
      delta sidecar imported into a separate Vortex artifact. This does not certify general
      incremental-state execution, table-format CDC, or object-store/catalog change planning.
