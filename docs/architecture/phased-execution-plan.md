...
- [ ] GAR-0020-C local manifest-backed table metadata read smoke
  - Source: RFC 0020; `GAR-0020-A` table/catalog metadata admission gate; table-intelligence
    layer; RFC 0041.
  - Current state: `GAR-0020-A` exposes deterministic admission diagnostics for catalog
    resolution, snapshot/manifest reads, table metadata reads, data reads, credential use, and
    external table-format dependencies; no table/catalog metadata read runtime exists.
  - Next slice outcome: one explicitly local, no-network, manifest-backed table metadata read smoke
    path that returns a typed metadata summary and evidence refs without reading data files.
  - User-visible surface: CLI table-intelligence/catalog metadata output, capability fields,
    fixture evidence, release-readiness metadata.
  - Implementation scope: table metadata fixture/model, local manifest/catalog-ref admission path,
    CLI output fields, tests, docs.
  - Evidence required: correctness ref for the fixture metadata summary, execution certificate ref,
    Native I/O certificate ref if a source/sink boundary is exercised, materialization/decode refs,
    diagnostic/no-fallback refs, dependency and credential boundary refs.
  - Acceptance: one local manifest-backed table metadata smoke path is runtime-supported; external
    catalog formats, object-store paths, credentials, data reads, writes, CDC/delete/maintenance,
    and broad table-format runtime remain deterministic unsupported/report-only paths.
  - Verification: focused table-intelligence/catalog metadata tests, no-fallback/plan-only contract
    tests, release readiness metadata tests, default GAR verification.
  - Non-goals: no Iceberg/Delta/Hudi parser dependency, object-store I/O, credential loading, data
    file scan, table write, commit, CDC/delete execution, lakehouse claim, or performance claim.
  - Fallback/claim boundary: may claim one scoped local metadata smoke path only; no production
    table/catalog, lakehouse, object-store, SQL/DataFrame, or external-catalog runtime claim.
  - Dependencies/blockers: `GAR-0020-A` complete; broader object-store and credential runtime gates
    still required for remote or external catalog support.
