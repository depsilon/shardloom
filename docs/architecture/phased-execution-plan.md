...
- [ ] GAR-RUNTIME-IMPL-6D:last_order.broad_sql_grammar: Broad SQL grammar over
  ...
  Scoped UTF-8 `RLIKE` / `REGEXP` / `REGEXP_LIKE` predicates are admitted separately through
  ShardLoom-owned regex evaluation while locale-aware collation/regex semantics remain blocked.
