...
These slices separate benchmark execution from website rendering so competitor lanes cannot silently
disappear when optional Python, JVM, or external baseline libraries are missing from the static-site
generation environment. The website must render an explicit benchmark artifact produced by a
declared benchmark profile, not rediscover competitor availability from the Cloudflare/static
publishing environment.

The benchmark publishing path must preserve the existing claim posture: external engines are
baseline context only, never fallback execution; CPU, Spark, extended, GPU, object-store, and
I/O-reuse profiles remain separate; and benchmark pages remain evidence dashboards, not public
rankings or performance claims.

- [ ] GAR-BENCH-PUB-1A benchmark competitor registry and required-lane policy
  - Source:
    - `benchmarks/traditional_analytics`.
    - `benchmarks/traditional_analytics/README.md`.
    - `docs/benchmarks/local-taxonomy-benchmark.md`.
    - `website/benchmarks.html`.
    - latest local full benchmark artifacts with competitor lanes.
    - official ecosystem docs for external baseline lane identity and profile membership.
  - Current state:
    - Traditional analytics benchmarks can include external baseline engines when their optional
      dependencies are installed.
    - Website benchmark generation can reflect the locally available artifact set, which means
      whole competitor lanes can disappear from the published page when the generating environment
      is not the full benchmark environment.
    - External engines are already documented as baseline-only and must not satisfy ShardLoom
      evidence gates.
  - Next slice outcome:
    - Add a machine-readable competitor registry that defines expected, required, optional, and
      unavailable lanes per benchmark profile.
    - Make lane availability explicit in benchmark artifacts and website rendering.
  - User-visible surface:
    - benchmark JSON artifacts.
    - benchmark Markdown/HTML reports.
    - `website/benchmarks.html`.
    - benchmark docs and runbooks.
  - Implementation scope:
    - benchmark competitor registry module or data file.
    - traditional analytics harness availability collection.
    - benchmark row/report schema.
    - website benchmark renderer and readiness checks where lane status is displayed.
  - Required lane groups:
    - ShardLoom lanes:
      - `shardloom`.
      - `shardloom-prepared-vortex`.
      - `shardloom-vortex` / `native-vortex`.
    - core local baselines:
      - `pandas`.
      - `polars-eager`.
      - `polars-lazy`.
      - `duckdb`.
  ...
  - Evidence required:
    ...
    - `lane_version`.
    - `lane_availability_reason`.
    - `lane_profile_membership`.
    - `external_baseline_only=true` for non-ShardLoom lanes.
    - `claim_gate_status`.
  - Acceptance:
    - Missing required lanes fail the selected full benchmark profile before publishable artifact
      creation.
    - Missing optional lanes remain visible with a deterministic availability reason.
    - External lanes are always marked `external_baseline_only` and cannot satisfy ShardLoom
      correctness, Native I/O, no-fallback, materialization/decode, or claim-grade gates.
    - Ibis lanes are explicitly adapter/front-end lanes whose backend engine is declared.
    - GPU lanes are never mixed into the default CPU-local profile.
  - Verification:
    - competitor registry unit test.
    - benchmark harness contract test.
    - website readiness check that rejects silently missing expected lanes.
    - `cargo test -p shardloom-contract-tests --test traditional_benchmark_harness`.
    - `git diff --check`.
  - Non-goals:
    - no performance/superiority claim.
    ...
    - no external engine fallback.
    - no object-store runtime admission.
    - no GPU requirement for CPU-local benchmark profiles.
  - Claim boundary:
    - Competitors are benchmark context/baseline lanes only.
    - Lane presence, absence, or timing cannot imply ShardLoom advantage, best-default posture,
      Spark replacement, production readiness, or package-release readiness.
  - Fallback boundary:
    - ShardLoom rows must preserve `fallback_attempted=false` and
      `external_engine_invoked=false`.
    - External baseline rows must be marked as external baselines, not ShardLoom fallback
      execution.
  - Ledger rule:
    - When complete, move the detailed completed session to
      `docs/architecture/phased-execution-completed-ledger.md` with registry schema refs,
      profile-lane examples, and failing/missing-lane diagnostics.
  - Dependencies/blockers:
    - stable benchmark row schema, website benchmark renderer, and profile definitions from
      GAR-BENCH-PUB-1E.

- [ ] GAR-BENCH-PUB-1B full benchmark environment lock and preflight
  - Source:
    - GAR-BENCH-PUB-1A competitor registry.
    - `benchmarks/traditional_analytics/README.md`.
    - local full benchmark artifact generation workflow.
    - Python and JVM dependency requirements for external baseline engines.
  - Current state:
    - Benchmark availability depends on locally installed libraries.
    - Missing dependencies can remove competitor lanes from generated artifacts unless the harness
      is run with strict options and the website artifact records lane status.
  - Next slice outcome:
    - Add benchmark environment specifications and a preflight validator for benchmark profiles.
  - User-visible surface:
    - benchmark runbook.
    - contributor/dev workflow.
    - benchmark artifact lane availability report.
  - Implementation scope:
    ...
    - optional `benchmarks/traditional_analytics/environment.full-local.yml`.
    - `scripts/check_benchmark_environment.py`.
    - benchmark docs and CI/release notes for optional profiles.
  - Evidence required:
    - profile name.
    - required package/module imports.
    - optional package/module imports.
    - package versions.
    - Python version.
    - platform and CPU metadata.
    - Java/JDK availability for Spark profiles.
    - GPU/CUDA availability for GPU profiles when requested.
    - lane availability reason.
  - Acceptance:
    - `full_local` fails preflight if any required core local lane is missing.
    - `full_local_plus_spark` fails preflight if Spark or Java/JDK requirements are missing.
    - `extended_local` and `gpu_optional` report optional missing lanes without hiding them.
    - Preflight output is deterministic enough to embed or reference from the benchmark manifest.
    - Preflight docs explain that Cloudflare/static website deployment does not need competitor
      libraries installed.
  - Verification:
    ...
    - `python -m compileall -q scripts benchmarks/traditional_analytics`.
    - `git diff --check`.
  - Non-goals:
    - no package publication.
    - no GPU requirement for CPU benchmark profiles.
    - no live dependency installation from website generation.
    - no performance claim.
  - Claim boundary:
    - Preflight proves benchmark environment completeness only.
    - It does not prove workload performance, production readiness, or benchmark claim-grade status.
  - Fallback boundary:
    - Preflight must never configure external engines as ShardLoom fallback execution.
    - ShardLoom rows remain subject to `fallback_attempted=false` and
      `external_engine_invoked=false`.
  - Ledger rule:
    - When complete, move the detailed completed session to the completed ledger with preflight
      output examples for complete and incomplete profiles.
  - Dependencies/blockers:
    - competitor registry and profile matrix from GAR-BENCH-PUB-1A and GAR-BENCH-PUB-1E.

- [ ] GAR-BENCH-PUB-1C static benchmark artifact ingestion for website publishing
  - Source:
    - GAR-BENCH-PUB-1A lane registry.
    - GAR-BENCH-PUB-1E profile matrix.
    - `website/build_static_pages.py`.
    - `website/benchmarks.html`.
    - `website/assets/data/benchmark-evidence.json`.
    - full local benchmark artifacts under `target/`.
  - Current state:
    - Website benchmark content is generated from locally available benchmark evidence artifacts.
    - The published page can therefore reflect an incomplete generation environment unless the
      artifact itself records lane completeness.
  - Next slice outcome:
    - Add a formal static artifact ingestion path so the website consumes a committed benchmark
      manifest and result bundle.
  - User-visible surface:
    - `website/benchmarks.html`.
    - `website/assets/benchmarks/latest/manifest.json`.
    - `website/assets/benchmarks/latest/benchmark-results.json`.
    - `website/assets/benchmarks/latest/benchmark-results.md`.
    - optional `website/assets/benchmarks/latest/benchmark-results.html`.
    - optional archive directory under `website/assets/benchmarks/archive/`.
  - Implementation scope:
    - benchmark artifact preparation script.
    - website static-page generator manifest argument.
    - website readiness and static asset validators.
    - benchmark docs and publishing runbook.
  ...
  - Evidence required:
    ...
    - lane availability rows.
    - scenario coverage summary.
    - environment fingerprint.
    - claim boundary.
    - `fallback_attempted=false` and `external_engine_invoked=false` for ShardLoom rows.
    - `external_baseline_only=true` for external rows.
  - Acceptance:
    - Website renders from the ingested artifact, not from installed Python libraries.
    - Website displays artifact status as complete, incomplete, or partial-with-reason.
    - Missing required lanes block publishable `full_local`/`full_local_plus_spark` artifacts unless
      a documented non-default override records the artifact as incomplete.
    - Missing optional lanes are shown visibly with reason.
    - No runtime GitHub/raw artifact fetch is introduced.
  - Verification:
    ...
    - `git diff --check`.
  - Non-goals:
    - no live benchmark execution on Cloudflare.
    - no remote dependency fetch.
    - no benchmark recomputation in website deploy.
    - no runtime code changes unless row schema work is explicitly required.
  - Claim boundary:
    - Static artifact ingestion makes benchmark provenance visible only.
    - It does not create performance, superiority, production, SQL/DataFrame, object-store,
      lakehouse, Foundry, or package-publication claims.
  - Fallback boundary:
    - Ingested artifacts must preserve ShardLoom no-fallback fields and external-baseline-only
      markers exactly.
  - Ledger rule:
    - When complete, move the detailed completed session to the completed ledger with manifest
      schema refs, sample latest artifact refs, and website readiness output.
  - Dependencies/blockers:
    - manifest schema, completeness gate, website renderer update, and artifact runbook.

- [ ] GAR-BENCH-PUB-1D competitor-lane expansion
  - Source:
    - GAR-BENCH-PUB-1A competitor registry.
    - current lanes in `benchmarks/traditional_analytics`.
    - external ecosystem docs for lane identity and profile fit.
  - Current state:
    - Core lanes exist or are partially represented for ShardLoom, pandas, Polars, DuckDB, Spark,
      DataFusion, and Dask.
    - Some lanes are collapsed or optional in practice, and additional ecosystem baselines are not
      consistently represented.
  - Next slice outcome:
    - Expand baseline lane vocabulary with explicit profile membership and deterministic
      unavailable-row handling.
  - User-visible surface:
    - benchmark reports.
    - website benchmark lane availability panel.
    - benchmark docs.
  - Implementation scope:
    - competitor registry.
    - benchmark preflight.
    - benchmark harness lane adapters where feasible.
    - deterministic unavailable rows for lanes that are not installed or not implemented.
  - Add or split lanes:
    - `polars-eager`.
    - `polars-lazy`.
    - `pyarrow-dataset`.
  ...
  - Acceptance:
    - Core CPU lanes appear in `full_local`.
    - Spark lanes appear in `full_local_plus_spark`.
  ...
  - Verification:
    ...
    - benchmark smoke per new lane where feasible.
    - missing optional lane report.
    - benchmark artifact completeness checker.
  - Non-goals:
    - no performance/superiority claim.
  ...
  - Dependencies/blockers:
    - competitor registry, profile matrix, and environment preflight.

- [ ] GAR-BENCH-PUB-1E benchmark profile matrix
  - Source:
    - benchmark harness.
    - website telemetry page.
    - GAR-BENCH-PUB-1A competitor registry.
    - GAR-IOREUSE-1 benchmark vocabulary.
  - Current state:
    - Benchmark profiles are partly implicit and environment-dependent.
    - Smoke, full local, Spark, extended, GPU, object-store, and I/O-reuse contexts need explicit
      separation before website publishing can be trusted.
  - Next slice outcome:
    - Define a machine-readable benchmark profile matrix used by preflight, harness execution,
      artifact validation, and website rendering.
  - User-visible surface:
    - benchmark docs.
    - benchmark CLI/preflight output.
    - website benchmark profile banner.
    - published artifact manifest.
  - Implementation scope:
    - profile registry/schema.
    - benchmark CLI flags and validation.
    - manifest generation.
    - docs and website rendering.
  - Profiles:
    - `smoke`: ShardLoom-only quick CI/local checks.
    - `full_local`: ShardLoom plus core CPU local baselines.
    - `full_local_plus_spark`: `full_local` plus Spark default/tuned JVM baselines.
    - `extended_local`: optional local ecosystem lanes.
    - `gpu_optional`: GPU/CUDA-specific lanes such as cuDF/RAPIDS.
    - `object_store_optional`: object-store scenarios only after runtime support is separately
      admitted.
  ...
  - Required fields:
    ...
    - `optional_scenarios`.
    - `hardware_class`.
    - `object_store_runtime_required`.
    - `claim_boundary`.
  - Evidence required:
    - profile name in every artifact.
    - required/optional lane lists.
    - required/optional scenario lists.
    - completeness status.
    - claim boundary.
    - environment fingerprint.
  - Acceptance:
    - Website shows which profile produced the artifact.
    - Missing lane policy is profile-specific.
    - CPU and GPU results are not mixed.
    - Object-store results are not mixed with local-file results.
    - `object_store_optional` remains blocked/report-only unless separate object-store runtime proof
      admits it.
    - `io_reuse_and_fanout` remains separate from one-shot local-file speed rows.
  - Verification:
    - benchmark profile validation test.
    - website benchmark manifest validation.
    - `cargo test -p shardloom-contract-tests --test traditional_benchmark_harness`.
    - `git diff --check`.
  - Non-goals:
    - no automatic benchmark profile promotion.
    - no performance or superiority claims.
    - no hidden fast mode.
  - Claim boundary:
    - Profile identity explains benchmark scope only.
    - It cannot upgrade a benchmark artifact to claim-grade without separate workload-scoped
      evidence gates.
  - Fallback boundary:
    - Profile definitions must preserve no-fallback semantics for ShardLoom rows and
      external-baseline-only semantics for competitor rows.
  - Ledger rule:
    - When complete, move the detailed completed session to the completed ledger with profile
      registry refs and sample manifests for at least `smoke` and one incomplete full profile.
  - Dependencies/blockers:
    - competitor registry and artifact schema.

- [ ] GAR-BENCH-PUB-1F competitor and scenario coverage completeness gate
  - Source:
    - competitor registry.
    - benchmark profile matrix.
    - benchmark scenario catalog.
    - static website benchmark artifact manifest.
  - Current state:
    - Missing lanes or scenarios can be absent from rendered pages unless the benchmark artifact and
      website validator explicitly track completeness.
  - Next slice outcome:
    - Add a completeness gate that validates expected lanes and scenarios before a benchmark
      artifact can be published as current.
  - User-visible surface:
    - benchmark artifact validation report.
    - website benchmark completeness panel.
    - benchmark docs/runbook.
  - Implementation scope:
    - `scripts/check_benchmark_artifact_completeness.py`.
    - benchmark harness contract tests.
    - website readiness integration.
    - manifest/schema validation.
  - Gate checks:
    - every expected lane has status.
    - every expected scenario has a row per expected lane or explicit blocked/unsupported/
      unavailable reason.
    - every missing lane has reason.
    - every unavailable lane has install/preflight guidance where applicable.
    - every ShardLoom row has `fallback_attempted` and `external_engine_invoked` fields.
    - every external lane has `external_baseline_only=true`.
    - every artifact has profile, environment, freshness timestamp, and claim boundary.
  - Evidence required:
    - completeness status.
    - missing lane list.
  ...
  - Acceptance:
    - A benchmark artifact cannot publish with silently missing competitors.
    - Website page displays completeness status and lane availability.
    - Incomplete artifacts are allowed only when explicitly labeled incomplete and are not treated as
      latest full-local evidence.
    - Completeness failures are actionable and name the missing lane/scenario/profile requirement.
  - Verification:
    ...
    - `git diff --check`.
  - Non-goals:
    - no benchmark result modification.
    - no external package installation.
    - no performance claim.
  - Claim boundary:
    - Completeness validates reporting integrity, not benchmark superiority.
    - A complete artifact can still be `claim_gate_status=not_claim_grade`.
  - Fallback boundary:
    - Completeness gate must reject missing ShardLoom no-fallback fields and missing
      external-baseline-only markers.
  - Ledger rule:
    - When complete, move the detailed completed session to the completed ledger with passing and
      failing completeness examples.
  - Dependencies/blockers:
    - manifest schema, profile matrix, scenario catalog, and website readiness hooks.

- [ ] GAR-BENCH-PUB-1G website benchmark rendering for complete competitor lanes
  - Source:
    - GAR-BENCH-PUB-1C static artifact ingestion.
    - GAR-BENCH-PUB-1F completeness gate.
    - `website/benchmarks.html`.
    - `website/assets/site.css`.
    - `website/build_static_pages.py`.
  - Current state:
    - Benchmark page is claim-safe and frames results as evidence rather than a leaderboard.
    - The page still needs first-class artifact completeness, lane availability, versions, and
      environment display once manifest ingestion exists.
  - Next slice outcome:
    - Render benchmark page from the manifest and make complete/incomplete lane status visible near
      the top of the page.
  - User-visible surface:
    - `/benchmarks`.
    - static benchmark data assets.
    - benchmark freshness and completeness panels.
  - Implementation scope:
    - website benchmark generator.
    - benchmark page template/static HTML.
    - CSS components for lane availability and artifact status.
  ...
  - Required rendered sections:
    ...
    - expected lanes.
    - available lanes.
    - unavailable/missing lanes.
    - lane versions.
    - lane availability reasons.
    - environment fingerprint.
    - freshness timestamp.
    - claim boundary.
    - external baseline-only reminder.
  - Evidence required:
    - manifest ingestion path.
    - lane availability table.
    - completeness status.
    - raw timing/comparison tables retained behind explanation or collapsible sections.
    - claim-safe copy review.
  - Acceptance:
    - User can immediately tell whether the benchmark includes full competitor lanes.
    - Missing lanes are shown in a visible warning/panel, not omitted.
    - Raw tables remain visible but not framed as a public ranking.
    - “Benchmark Evidence, Not A Leaderboard” and workflow/evidence framing remain intact.
    - Website deploy does not require pandas, Polars, DuckDB, Spark, DataFusion, Dask, or Java.
  - Verification:
    - `python website/build_static_pages.py --benchmark-manifest website/assets/benchmarks/latest/manifest.json`.
    - `python scripts/check_website_readiness.py`.
    - `node website/validate_static_assets.js`.
  ...
  - Non-goals:
    ...
    - no live dependency probing on the website.
    - no runtime GitHub/raw artifact fetch.
    - no performance or superiority claim.
  - Claim boundary:
    - Website rendering remains an evidence publication surface only.
    - It cannot imply ShardLoom is faster, better, production-ready, Spark-displacing, or generally
      SQL/DataFrame/object-store/lakehouse/Foundry ready.
  - Fallback boundary:
    - Rendered ShardLoom rows must preserve no-fallback fields.
    - Rendered competitor rows must remain external baselines only.
  - Ledger rule:
    - When complete, move the detailed completed session to the completed ledger with generated page
      refs, readiness output, and an example missing-lane panel.
  - Dependencies/blockers:
    - manifest ingestion, completeness gate, website validation, and claim-safe benchmark copy.

- [ ] GAR-BENCH-PUB-1H local benchmark artifact upload/runbook
  - Source:
    - GAR-BENCH-PUB-1B preflight.
    - GAR-BENCH-PUB-1C artifact ingestion.
    - GAR-BENCH-PUB-1F completeness gate.
    - Cloudflare Workers Static Assets website deployment model.
  - Current state:
    - Local benchmark execution and website update steps are documented in pieces, but there is no
      single safe publishing runbook for full competitor artifacts.
  - Next slice outcome:
    - Add a runbook for producing, validating, committing, and publishing a full benchmark artifact
      without requiring competitor libraries in the website deployment environment.
  - User-visible surface:
    - `docs/benchmarks/static-benchmark-publishing-runbook.md`.
    - benchmark docs index.
    - optional README/website link once the workflow is implemented.
  - Implementation scope:
    - benchmark publishing runbook.
    - exact commands for preflight, benchmark execution, artifact preparation, website rebuild,
      completeness validation, readiness validation, and commit review.
    - expected file list for `website/assets/benchmarks/latest/`.
  - Runbook steps:
    - create or activate a clean benchmark environment.
    - install full-local benchmark dependencies.
    - run benchmark environment preflight.
    - run the selected benchmark profile.
    - generate manifest and static artifacts.
    - copy artifacts to `website/assets/benchmarks/latest/`.
    - rebuild static website pages from the manifest.
    - run completeness and website readiness checks.
    - commit artifacts and generated pages.
  - Evidence required:
    - command examples.
    - expected output files.
    - claim-safety checklist.
    - missing-lane handling.
    - artifact freshness guidance.
  - Acceptance:
    - A contributor can publish a full benchmark artifact without installing competitors in
      Cloudflare or static website generation environments.
    - Runbook includes exact commands and expected files.
    - Runbook explains how incomplete artifacts are labeled and why they must not replace latest
      full-local evidence by accident.
    - Runbook preserves no performance, superiority, Spark replacement, production, package,
      SQL/DataFrame, object-store/lakehouse, or Foundry claims.
  - Verification:
    - `docs/benchmarks/static-benchmark-publishing-runbook.md` exists.
    - runbook commands match current script names or are clearly marked as future commands tied to
      the preceding GAR-BENCH-PUB slices.
    - `git diff --check`.
  - Non-goals:
    - no live benchmark execution in this runbook-only slice.
    - no package publication.
    - no Cloudflare deployment changes.
  - Claim boundary:
    - The runbook publishes local benchmark evidence only.
    - It must state that benchmark artifacts are not performance/superiority claims unless a future
      release gate explicitly admits a workload-scoped claim.
  - Fallback boundary:
    - The runbook must preserve external-baseline-only rules and ShardLoom no-fallback fields in
      validation steps.
  - Ledger rule:
    - When complete, move the detailed completed session to the completed ledger with the runbook
      path and validation output.
  - Dependencies/blockers:
    - artifact manifest, completeness checker, website manifest ingestion, and benchmark preflight.
