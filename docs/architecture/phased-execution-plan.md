...
#### GAR-RUNTIME-IMPL-3 - Explicit Full-Runtime Leaf Implementation Cut List

This queue is the implementation leaf list for the remaining full runtime support gaps. It is
intentionally concrete and runtime-oriented: each item must produce usable behavior, a deterministic
unsupported/blocker surface where behavior is not yet admitted, or a release gate that prevents an
unsupported capability from being described as implemented. These items sit at the end of Planned so
future autonomous execution has an explicit checklist after the current PR-sized runtime queue.

Do not close a broad `GAR-RUNTIME-COMPLETE-*`, `GAR-RUNTIME-IMPL-1*`, or `GAR-RUNTIME-IMPL-2*`
track unless the matching leaf slices below are complete or a ledger entry explains why a leaf was
superseded by stronger runtime evidence.

- [ ] GAR-RUNTIME-IMPL-3A generated-source runtime builders and source-free SQL/DataFrame completion
  - Source: `GAR-GEN-1`, `GAR-COMPAT-1B`, `GAR-NOVEL-1A`, `GAR-RUNTIME-IMPL-2C`,
    `GAR-RUNTIME-IMPL-2D`.
  - Current state: source-free user-row and scoped SQL generated-output smokes exist, while broader
    `range`, `sequence`, `values`, `literal_table`, `calendar`, source-free projection, DataFrame
    generated expressions, and non-local sinks remain scoped, blocked, or report-only.
  - Next slice outcome: implement a single coherent generated-source runtime API across CLI and
    Python for range/sequence, rows/values, literal table, and calendar/date dimension local JSONL
    output, with deterministic blockers for unsupported expressions and non-local sinks.
  - User-visible surface: Python `ctx.range`, `ctx.from_rows`, `ctx.literal_table`, `ctx.calendar`,
    SQL literal/VALUES helpers, CLI generated-source commands, use cases, website status.
  - Implementation scope: generated-source plan nodes, schema inference, deterministic seed/row
    count handling, local output bridge, typed Python report objects, CLI JSON envelope.
  - Evidence required: `input_dataset_count=0`, `source_io_performed=false`,
    `generated_source_created=true`, generated source kind/schema/row/plan digests, seed,
    determinism flag, output Native I/O certificate, fallback/external-engine fields, claim gate.
  - Acceptance: no-dataset smoke stays distinct from generated-output execution; no source Native
    I/O certificate is claimed when no source was read; every admitted builder writes local output
    and returns inspectable evidence.
  - Verification: generated-source CLI tests, Python builder tests, SQL literal/VALUES tests,
    use-case coverage, release readiness metadata.
  - Non-goals: no S3/object-store sink, Foundry production path, package publication, or broad
    SQL/DataFrame runtime claim.
  - Claim boundary: scoped local generated-source output only.
  - Fallback boundary: builders must not delegate row generation or expression evaluation to pandas,
    Polars, DuckDB, DataFusion, Spark, or external services.
  - Dependencies/blockers: local output writer registry and expression semantics for generated
    columns.
  - Ledger rule: ledger entry must list each generated-source form, output format, and blocked form.

- [ ] GAR-RUNTIME-IMPL-3B local SourceState adapter parity for common file formats
  - Source: `GAR-RUNTIME-IMPL-2A`, `GAR-IOREUSE-1A`, universal compatibility scoreboard.
  - Current state: selected local format smokes and compatibility imports exist, but common local
    input formats do not yet share one user-facing SourceState runtime contract.
  - Next slice outcome: implement a local adapter registry for CSV, JSONL/JSON, Parquet, Arrow IPC,
    Avro, and ORC posture with runtime support where dependencies and evidence exist, and stable
    blockers where they do not.
  - User-visible surface: CLI read/execute commands, Python `ctx.read_*`, capability views,
    benchmark source-format rows, use-case atlas.
  - Implementation scope: adapter registry, format detection, schema/dtype inference, content
    fingerprinting, row-count knowledge, local readers, unsupported diagnostics.
  - Evidence required: source format/location/fingerprint, schema digest, SourceState id/digest,
    row-count status, decode/materialization fields, no-fallback/no-external-engine fields.
  - Acceptance: every common local file family is either runnable with SourceState evidence or
    blocked with an actionable reason; adapter support never implies Vortex-native execution by
    itself.
  - Verification: adapter snapshot tests, CLI/Python smoke per admitted format, unsupported format
    diagnostics, traditional benchmark harness contract tests.
  - Non-goals: no object-store, database, table-format, or production universal adapter claim.
  - Claim boundary: local file adapter support per admitted format.
  - Fallback boundary: adapters cannot use external engines to parse, plan, or execute user work.
  - Dependencies/blockers: dependency/license review for non-default readers and fixture coverage.
  - Ledger rule: ledger entry must enumerate each file format and support status.

- [ ] GAR-RUNTIME-IMPL-3C unified runtime execution envelope across CLI, Python, benchmarks, and web
  - Source: `GAR-RUNTIME-IMPL-2E`, execution certificates, Native I/O certificates, benchmark
    publishing, release readiness metadata.
  - Current state: runtime commands emit useful JSON fields, but field names, certificate refs, and
    evidence-level fields can still diverge by command.
  - Next slice outcome: add a versioned execution-envelope schema and adapters so every runtime
    path emits the same required evidence, no-fallback, materialization, source, output, and claim
    fields.
  - User-visible surface: CLI JSON, Python typed reports, benchmark artifacts, website evidence
    pages, release readiness reports.
  - Implementation scope: shared schema module, report adapters, validators, compatibility aliases,
    readiness checks, generated website/evidence rendering.
  - Evidence required: execution mode, engine mode, evidence level, source/generated/output refs,
    materialization/decode refs, certificate refs, fallback/external-engine flags, claim gate.
  - Acceptance: every runtime command validates against the envelope; missing fallback, certificate,
    or claim fields fail readiness; report-only rows cannot masquerade as runtime support.
  - Verification: schema contract tests, release readiness metadata, benchmark artifact
    completeness, website readiness, Python typed-report tests.
  - Non-goals: no new runtime capability or claim upgrade from schema work alone.
  - Claim boundary: envelope standardizes evidence only.
  - Fallback boundary: every envelope must expose fallback/external-engine status explicitly.
  - Dependencies/blockers: stable field names and migration policy.
  - Ledger rule: ledger entry must record schema version and migrated surfaces.
...
- [ ] GAR-RUNTIME-IMPL-3D SQL planner and expression runtime expansion by admitted syntax family
  ...
  - User-visible surface: CLI SQL command, Python `ctx.sql`, explain output, recipes, status page.
  - Implementation scope: parser grammar, binder, logical/physical plan nodes, expression lowering,
    operator admission, unsupported diagnostics, correctness fixtures.
  - Evidence required: parser/binder/planner flags, plan digests, operator families, source/output
    certificate refs, correctness digest, unsupported diagnostic code, no-fallback fields.
  - Acceptance: every admitted syntax family executes through ShardLoom-native operators only; every
    unsupported construct is rejected before execution with a stable diagnostic.
  - Verification: parser/binder/planner tests, SQL smoke per admitted family, unsupported snapshot
    tests, release readiness metadata, website/use-case checks.
  - Non-goals: no ANSI SQL parity, catalog runtime, broad UDF support, or production SQL claim.
  - Claim boundary: scoped SQL frontend runtime per admitted syntax family.
  - Fallback boundary: no external SQL/query engine may execute residual work.
  - Dependencies/blockers: operator semantics, local input/output adapters, explain schema.
  - Ledger rule: ledger entry must list syntax families admitted and blocked.
...
- [ ] GAR-RUNTIME-IMPL-3E Python end-user workflow parity across generated, local, and Vortex inputs
  ...
  - Next slice outcome: provide one ergonomic Python workflow for generated-source output, local
    ...
    unsupported methods.
  - User-visible surface: `shardloom.context()`, query builder, typed reports, examples, README,
    website use cases.
  - Implementation scope: Python context/session, CLI bridge or native bindings, query-builder
    methods, typed reports, examples, docs.
  - Evidence required: method admission, plan digest, source/generated/prepared refs, output refs,
    correctness digest, no-fallback/no-external-engine fields, claim gate.
  - Acceptance: a non-expert can install locally, import ShardLoom, run the documented workflows,
    write local output, inspect evidence, and receive actionable blockers for unsupported calls.
  - Verification: Python unit/integration tests, example smoke, compileall, website/use-case
    coverage, release readiness metadata.
  - Non-goals: no pandas/Polars backend, notebook production claim, package publication, or broad
    DataFrame parity claim.
  - Claim boundary: scoped local Python runtime workflows only.
  - Fallback boundary: Python may orchestrate ShardLoom but must not execute compute in external
    engines.
  - Dependencies/blockers: SQL/operator coverage, generated-source builders, local writers, Vortex
    lifecycle support.
  - Ledger rule: ledger entry must include exact runnable snippets and evidence output.

- [ ] GAR-RUNTIME-IMPL-3F Vortex prepared/native lifecycle, pushdown, and write/reopen runtime
  - Source: `GAR-RUNTIME-IMPL-2B`, `GAR-RUNTIME-IMPL-1F`, `GAR-PERF-2C`, Vortex provider docs.
  - Current state: prepared/native smokes and source-backed scan evidence exist, but users still
    need a simple local lifecycle from prepare/read through compute, write, reopen, scan, and proof.
  - Next slice outcome: implement a local Vortex lifecycle path with prepare/read, filter/project/
    limit or aggregate, pushdown where admitted, local Vortex output, reopen, scan, and digest proof.
  - User-visible surface: CLI Vortex workflow command, Python helper, benchmark rows, compute-flow,
    website Field Guide/status.
  - Implementation scope: VortexPreparedState, source-backed scan builder, pushdown lowering,
    Vortex sink writer, reopen verifier, lifecycle evidence.
  - Evidence required: prepared state/artifact refs and digests, layout/encoding/stats summary,
    pushdown fields, output certificate, reopen digest, decode/materialization fields,
    no-fallback/no-external-engine fields.
  - Acceptance: admitted local Vortex workflow runs without compatibility re-import; unsupported
    Vortex features block; compatibility import timing remains separate from prepared/native timing.
  - Verification: lifecycle smoke, source-backed scan tests, writer/reopen tests, benchmark harness
    contract tests, docs/website readiness.
  - Non-goals: no object-store Vortex artifacts, blanket encoded-native claim, or performance claim.
  - Claim boundary: local Vortex lifecycle only for admitted layouts/operators.
  - Fallback boundary: upstream Vortex native APIs may be providers only through certified
    boundaries; Vortex query-engine integrations remain prohibited.
  - Dependencies/blockers: Vortex dependency/version gate, writer support, operator coverage.
  - Ledger rule: ledger entry must include artifact refs, operator scope, and reopen proof.

- [ ] GAR-RUNTIME-IMPL-3G physical operator and kernel coverage to ordinary workflow readiness
  - Source: `GAR-RUNTIME-IMPL-1E`, `GAR-RUNTIME-IMPL-1G`, RFC 0015, benchmark scenario catalog.
  - Current state: selected residual-native scenarios exist; generalized aggregates, joins, top-N,
    windows, casts, null/type/string semantics, and encoded kernels are incomplete.
  - Next slice outcome: promote operator families one at a time with decoded-reference correctness,
    encoded-kernel admission where available, benchmark rows, and deterministic blockers for
    unsupported types or encodings.
  - User-visible surface: CLI/Python/SQL/DataFrame workflows, benchmark rows, capability views.
  - Implementation scope: expression IR, native kernels, residual executors, encoded-kernel
    registry, null/type semantics, correctness fixtures.
  - Evidence required: operator family, type/null semantics, encoding id, rows scanned/selected/
    output, correctness digest, decode/materialization status, no-fallback fields.
  - Acceptance: each promoted family has edge-case tests and benchmark/correctness evidence;
    unsupported variants block before execution instead of delegating.
  - Verification: unit/property/correctness tests, differential fixtures, benchmark smoke,
    traditional benchmark harness tests.
  - Non-goals: no broad SQL/DataFrame parity or performance/superiority claim from operator
    coverage alone.
  - Claim boundary: workload-scoped operator support only.
  - Fallback boundary: external engines may be test oracles only.
  - Dependencies/blockers: fixture coverage, expression registry, benchmark row schema.
  - Ledger rule: ledger entry must list promoted operator families and remaining blockers.

- [ ] GAR-RUNTIME-IMPL-3H local output writers, replay proof, and cross-format fanout
  - Source: `GAR-RUNTIME-IMPL-2F`, `GAR-RUNTIME-IMPL-1I`, `GAR-RUNTIME-IMPL-1J`,
    OutputPlan, result-sink proof.
  - Current state: scoped local JSONL output smokes exist; general local writer registry, replay
    proof, and fanout across admitted sources are incomplete.
  - Next slice outcome: implement local writer registry and fanout for Vortex, JSONL, CSV, Parquet
    or Arrow IPC where admitted, with per-output digest and replay/correctness evidence.
  - User-visible surface: Python `.write` and `.fanout`, CLI write/fanout command, recipes,
    benchmark `io_reuse_and_fanout`, website status.
  - Implementation scope: OutputPlan builder, writer registry, schema translation, output digests,
    replay verifier, multi-output orchestration.
  - Evidence required: output plan id/digest, output format/location/schema, write timing, replay
    status, metadata fidelity/loss, correctness digest per output, no-fallback fields.
  - Acceptance: input and output formats are decoupled; unsupported writers block; one admitted
    source/prepared state can write multiple local outputs with visible timing/evidence.
  - Verification: writer smoke per format, fanout smoke, replay tests, use-case coverage, benchmark
    harness tests.
  - Non-goals: no object-store write, table commit, production sink claim, or performance claim.
  - Claim boundary: local output/fanout support per format and evidence level.
  - Fallback boundary: compatibility writes are exports, not external-engine execution.
  - Dependencies/blockers: local writer dependencies, schema mapping, replay verifier.
  - Ledger rule: ledger entry must list supported writers, fanout combinations, and blocked outputs.

- [ ] GAR-RUNTIME-IMPL-3I optimizer, fusion, session reuse, and buffer-pool promotion
  - Source: `GAR-RUNTIME-IMPL-1H`, `GAR-PERF-2B`, `GAR-PERF-2E`, `GAR-PERF-2F`,
    `GAR-PERF-2G`.
  - Current state: optimizer/fusion/session/buffer evidence is partially report-only or scoped to
    benchmark paths; ordinary user workflows do not yet benefit from a coherent session runtime.
  - Next slice outcome: implement optimizer-rule admission/explain, fused local pipelines,
    `ShardLoomSession` reuse, source/prepared caches, and safe buffer reuse for admitted local
    workflows.
  - User-visible surface: CLI/Python explain, session API, benchmark timing rows, evidence reports.
  - Implementation scope: optimizer rule registry, plan rewrite trace, fused executor, session
    cache lifecycle, buffer pool/resource metrics.
  - Evidence required: rule admitted/applied/blocked, before/after plan digests, rewrite safety,
    session/cache hit/miss, fused pipeline fields, allocation/buffer metrics, correctness digest.
  - Acceptance: rewrites preserve evidence and correctness; session reuse never returns stale data;
    fusion avoids unnecessary intermediate full-table materialization where supported.
  - Verification: plan snapshot tests, differential correctness tests, batch/session smoke,
    memory/resource report, benchmark smoke.
  - Non-goals: no hidden fast mode, daemon/service, performance claim, or optimizer parity claim.
  - Claim boundary: scoped local runtime optimization only.
  - Fallback boundary: optimizer/session cannot change provider to an external engine.
  - Dependencies/blockers: execution envelope, source/prepared-state reuse, correctness diff harness.
  - Ledger rule: ledger entry must include promoted rules, cache policy, and disabled paths.

- [ ] GAR-RUNTIME-IMPL-3J benchmark artifact refresh and claim-grade runtime evidence gate
  - Source: `GAR-RUNTIME-IMPL-1S`, `GAR-BENCH-PUB-1`, benchmark publishing runbook.
  - Current state: benchmark publishing has manifests and lane policies, but every promoted runtime
    path must be tied to fresh, complete, claim-safe artifacts.
  - Next slice outcome: add a runtime evidence gate that maps each supported runtime path to fresh
    benchmark/correctness/certificate refs and blocks stale or incomplete artifacts from public
    presentation.
  - User-visible surface: benchmark page, release readiness report, status matrix, docs.
  - Implementation scope: claim matrix schema, artifact freshness checks, lane completeness checks,
    benchmark page rendering, release validators.
  ...
  - Acceptance: no runtime path can become claim-grade without current workload-scoped evidence;
    external engines remain baseline-only and missing lanes are visible.
  - Verification: benchmark artifact completeness checker, release readiness metadata, website
    readiness, traditional benchmark harness tests.
  - Non-goals: no performance/superiority/Spark-replacement claim unless gates explicitly pass.
  - Claim boundary: evidence gate only; claims remain workload-scoped.
  - Fallback boundary: external baselines cannot satisfy ShardLoom-native evidence gates.
  - Dependencies/blockers: benchmark manifests, runtime envelopes, website generator support.
  - Ledger rule: ledger entry must list passed and blocked claims with artifact refs.

- [ ] GAR-RUNTIME-IMPL-3K object-store read runtime admission and local-emulator proof
  - Source: `GAR-RUNTIME-IMPL-1K`, `GAR-COMPAT-1C`, `GAR-SCALE-1E`.
  - Current state: object-store URI planning exists, but provider reads, credential policy, and
    byte-range/full-file reads are not runtime-supported.
  - Next slice outcome: implement object-store read stages in an approved emulator or public
    no-credential fixture profile: URI parse, effect/credential policy, listing if admitted,
    byte-range read, streaming/full-file read, and SourceState evidence.
  ...
  - Implementation scope: provider abstraction, effect gate, credential policy, request planner,
    byte-range adapter, local cache boundary, emulator tests.
  - Evidence required: provider/profile, credential status, network effect status, object version/
    ETag, byte ranges, SourceState id, Native I/O certificate, no-fallback fields.
  - Acceptance: public and authenticated reads are separate gates; no network probe or credential
    resolution runs by default; unsupported providers fail closed.
  - Verification: policy tests, mocked/emulator read smoke, source-state snapshot tests, release
    readiness, website status checks.
  ...
  - Fallback boundary: storage providers do not authorize external query engines.
  - Dependencies/blockers: security review, provider harness, dependency/license approval.
  - Ledger rule: ledger entry must record provider, credential posture, and proof refs.

- [ ] GAR-RUNTIME-IMPL-3L object-store write, table/lakehouse snapshot, append, and commit ladder
  - Source: `GAR-RUNTIME-IMPL-2H`, `GAR-RUNTIME-IMPL-2I`, `GAR-COMPAT-1D`, table/lakehouse commit
    semantics gate.
  - Current state: object-store writes, table metadata/snapshot scans, append, merge/delete, commit,
    rollback, and catalog integration remain blocked or report-only.
  - Next slice outcome: after read proof, implement staged write/commit/recovery in an approved
    provider/emulator and one local fixture-backed table metadata/snapshot operation, then one
    append or commit rehearsal where admitted.
  ...
  - Implementation scope: write staging, commit protocol, idempotency, cleanup/retry, table metadata
    adapter, snapshot reader, manifest writer or commit rehearsal, blockers.
  ...
  - Non-goals: no blanket S3/GCS/ADLS support, production Iceberg/Delta/Hudi claim, catalog service,
    or object-store-backed table production claim.
  - Claim boundary: provider/table-format operation in declared fixture/profile only.
  - Fallback boundary: no external catalog, lakehouse engine, or query engine executes work.
  - Dependencies/blockers: `GAR-RUNTIME-IMPL-3K`, dependency/license review, table fixtures.
  - Ledger rule: ledger entry must list provider, table format, operation, and blocked behaviors.

- [ ] GAR-RUNTIME-IMPL-3M scale-grade local runtime: split, bounded memory, spill, shuffle, retry
  - Source: `GAR-RUNTIME-IMPL-1M`, `GAR-RUNTIME-IMPL-1N`, `GAR-SCALE-1B` through
    `GAR-SCALE-1G`.
  - Current state: scale contracts and fields exist, but larger-than-memory, split-parallel, spill,
    shuffle, retry, and idempotent commit runtime are not claimable.
  - Next slice outcome: implement a declared-resource local scale profile with SplitManifest,
    bounded memory checks, per-split execution, spill/backpressure where admitted, shuffle evidence
    for one group-by/join family, retry/idempotency, and output commit evidence.
  - User-visible surface: scale benchmark profiles, CLI/Python execution envelopes, status page.
  - Implementation scope: split scheduler, memory budget, spill manager, shuffle plan, retry/cancel/
    recovery, output commit status, scale benchmark rows.
  - Evidence required: scale profile/status, data volume, split/file/partition counts, memory/spill
    fields, shuffle fields, retry/idempotency, output commit status, correctness digest.
  - Acceptance: larger-than-memory and split-parallel claims require real bytes and correctness
    proof; synthetic metadata cannot become runtime scale claim; distributed support remains
    separate.
  ...
  - Non-goals: no literal any-volume claim, Spark replacement claim, distributed runtime claim, or
    object-store scale claim without separate proof.
  - Claim boundary: declared local resource envelope only.
  - Fallback boundary: external engines can be baselines/oracles only.
  - Dependencies/blockers: SourceState split metadata, operator coverage, local output commit proof.
  - Ledger rule: ledger entry must list resource envelope, data volume, and claim status.

- [ ] GAR-RUNTIME-IMPL-3N live, hybrid, streaming, control-plane, and distributed runtime promotion
  - Source: RFC 0034, RFC 0035, `GAR-RUNTIME-IMPL-2J`, `GAR-SCALE-1F`.
  - Current state: batch has local evidence; live/hybrid fixtures, streaming contracts, REST/remote
    API, and distributed protocols remain scoped, blocked, or report-only.
  - Next slice outcome: implement a staged promotion ladder: local engine-mode diagnostics, local
    in-memory live/hybrid fixture workflow, opt-in loopback control plane if approved, then
    fail-closed distributed worker diagnostics until a real remote runtime is approved.
  - User-visible surface: CLI/Python engine mode status, optional local API, compute-flow, website
    status/use cases.
  - Implementation scope: engine-mode admission, local control-plane lifecycle, fixture streaming
    scheduler, API schema, blocker diagnostics, small-result boundary.
  - Evidence required: engine mode, control-plane invoked flag, live/hybrid state, checkpoint/state
    posture, network effect policy, remote worker invoked status, fallback/external-engine fields.
  - Acceptance: batch/live/hybrid labels cannot imply unsupported runtime; remote execution never
    runs accidentally; any local API is opt-in, loopback-scoped, and evidence-backed.
  - Verification: engine-mode contract tests, fixture workflow tests, API/blocker tests, website
    readiness, release readiness.
  - Non-goals: no production REST service, daemon, broker/state-store runtime, remote worker runtime,
    distributed claim, or exactly-once claim.
  - Claim boundary: fixture/local control-plane technical preview only.
  - Fallback boundary: remote APIs cannot trigger external compute.
  - Dependencies/blockers: lifecycle policy, evidence envelope, security review.
  - Ledger rule: ledger entry must record exposed API surface and blocked live/hybrid/distributed
    behaviors.

- [ ] GAR-RUNTIME-IMPL-3O external adapters, databases, UDFs, extensions, and effectful operations
  - Source: `GAR-RUNTIME-IMPL-2G`, `GAR-RUNTIME-IMPL-2K`, RFC 0011, RFC 0023, adapter/governance
    docs.
  ...
  - Evidence required: connector/extension id/version/digest, credential/network/effect status,
    import/export direction, UDF type/determinism/null contract, runtime invoked flags, no-fallback
    fields.
  - Acceptance: external systems are never fallback engines; users can inspect adapters/extensions
    safely; effectful operations are blocked by default; any UDF is local, deterministic, typed, and
    evidence-backed.
  - Verification: SQLite/local fixture smoke if admitted, manifest validation tests, UDF admission/
    blocker tests, unsupported network diagnostics, capability snapshots, release readiness.
  ...
  - Fallback boundary: connectors/extensions/UDFs must not delegate compute to external engines or
    services.
  - Dependencies/blockers: security/sandbox review, license/provenance review, fixture data.
  - Ledger rule: ledger entry must separate admitted local behaviors from denied effects/connectors.

- [ ] GAR-RUNTIME-IMPL-3P Foundry dev-stack generated-output and transform proof
  - Source: `GAR-COMMERCIAL-1E`, `GAR-IOREUSE-1G`, Foundry proof-of-use certification docs.
...
- [ ] GAR-RUNTIME-IMPL-3Q clean install, public technical-preview usability, website parity, and
      no-publication release rehearsal
  - Source: `GAR-RUNTIME-IMPL-2L`, release readiness docs, Use Case Atlas, website readiness,
    package-channel matrix.
  - Current state: repo and website are strong, but full usability requires clean install proof,
    aligned examples, fresh benchmark interpretation, status parity, and security/legal/release
    gates after runtime slices land.
  - Next slice outcome: run a no-publication technical-preview rehearsal from clean checkout/local
    package artifact through CLI/Python workflows, benchmark artifact rendering, website status,
    SECURITY/LICENSE/NOTICE checks, unsupported diagnostics, and final claim gates.
  - User-visible surface: README, docs/getting-started, website, package metadata, release report.
  ...
  - Acceptance: a non-expert can install locally, run admitted workflows, inspect evidence, and see
    unsupported paths without reading phase-plan internals; no public package is published without
  ...
  - Claim boundary: public technical preview only with workload-scoped claims.
  - Fallback boundary: release gates must fail if any supported workflow uses external fallback.
  - Dependencies/blockers: completion of admitted runtime workflows and docs/website parity.
...
that must be worked before any full-runtime readiness claim. If a broader `GAR-RUNTIME-COMPLETE-*`,
...
- [ ] GAR-RUNTIME-IMPL-4A Python group-by aggregate workflow promotion
  ...
  - Implementation scope: Python query-builder admission, CLI lowering, typed report fields,
    examples, docs.
  - Evidence required: group-by columns/count, aggregate functions, source/output refs,
    correctness digest, execution certificate ref, `fallback_attempted=false`,
    `external_engine_invoked=false`, `claim_gate_status`.
  - Acceptance: documented Python group-by smoke runs end to end; multi-key group-by, windows,
    joins, and unsupported aggregations fail with stable diagnostics.
  - Verification: Python unit/integration tests, CLI parity smoke, `python -m compileall -q
    python/src python/tests scripts examples benchmarks/traditional_analytics`, use-case checks.
  - Non-goals: no broad DataFrame runtime, SQL parity, performance claim, or external-engine
    fallback.
  - Claim boundary: scoped local CSV Python group-by aggregate fixture smoke only.
  - Fallback boundary: Python orchestrates ShardLoom runtime only; it must not compute via pandas,
    Polars, DuckDB, Spark, DataFusion, or another engine.
  - Dependencies/blockers: CLI group-by aggregate smoke, Python query-builder lowering, typed
    report fields, and fixture outputs.
...
- [ ] GAR-RUNTIME-IMPL-4B SQL order-by, limit, and top-N local runtime
  ...
  - Dependencies/blockers: expression ordering semantics, sort/top-N operator, null ordering policy,
...
- [ ] GAR-RUNTIME-IMPL-4C SQL local equi-join smoke and blocker matrix
  ...
  - Dependencies/blockers: two-source binding, local SourceState parity, hash table semantics,
    memory-budget evidence, and fixture data.
  - Ledger rule: ledger entry must record admitted join shape and blocked join families.

- [ ] GAR-RUNTIME-IMPL-4D expression, cast, null, string, and date runtime families
  - Source: RFC 0021, `GAR-RUNTIME-IMPL-3D`, `GAR-RUNTIME-IMPL-3G`.
...
- [ ] GAR-RUNTIME-IMPL-4E generated-source builders as ordinary local runtime
  - Source: `GAR-RUNTIME-IMPL-3A`, `GAR-GEN-1`, `GAR-COMPAT-1B`.
...
- [ ] GAR-RUNTIME-IMPL-4F local input adapter runtime coverage by format
  - Source: `GAR-RUNTIME-IMPL-3B`, `GAR-IOREUSE-1A`, universal compatibility scoreboard.
...
- [ ] GAR-RUNTIME-IMPL-4G local output writer registry and fanout promotion
  - Source: `GAR-RUNTIME-IMPL-3H`, OutputPlan, result-sink replay proof.
...
- [ ] GAR-RUNTIME-IMPL-4H Vortex prepare/read/write/reopen lifecycle promotion
  - Source: `GAR-RUNTIME-IMPL-3F`, Vortex provider docs, compute-flow reference.
...
- [ ] GAR-RUNTIME-IMPL-4I Vortex scan pushdown and encoded-predicate runtime completion
  - Source: `GAR-RUNTIME-IMPL-3F`, `GAR-PERF-2C`, encoded predicate provider evidence.
...
- [ ] GAR-RUNTIME-IMPL-4J encoded kernel registry execution pairs
  - Source: `GAR-RUNTIME-IMPL-3G`, `GAR-PERF-2D`, RFC 0021.
...
- [ ] GAR-RUNTIME-IMPL-4K unified execution envelope and certificate validators
  - Source: `GAR-RUNTIME-IMPL-3C`, release readiness metadata, benchmark artifact policy.
...
- [ ] GAR-RUNTIME-IMPL-4L ShardLoomSession, SourceState, PreparedState, and OutputPlan reuse runtime
  - Source: `GAR-RUNTIME-IMPL-3I`, `GAR-IOREUSE-1`, `GAR-PERF-2F`.
...
- [ ] GAR-RUNTIME-IMPL-4M benchmark refresh and runtime claim gate after each promoted workflow
  - Source: `GAR-RUNTIME-IMPL-3J`, `GAR-BENCH-PUB-1`, benchmark publishing runbook.
...
- [ ] GAR-RUNTIME-IMPL-4N object-store read admission with local emulator/public fixture proof
  - Source: `GAR-RUNTIME-IMPL-3K`, `GAR-COMPAT-1C`, `GAR-SCALE-1E`.
...
- [ ] GAR-RUNTIME-IMPL-4O object-store write and table/lakehouse commit ladder
  - Source: `GAR-RUNTIME-IMPL-3L`, table/lakehouse commit semantics gate.
  - Current state: object-store writes, table metadata/snapshot scans, append, merge/delete, commit,
...
- [ ] GAR-RUNTIME-IMPL-4P scale-grade local split, memory, spill, shuffle, and retry runtime
  - Source: `GAR-RUNTIME-IMPL-3M`, `GAR-SCALE-1`, RFC 0014, RFC 0016, RFC 0017.
...
- [ ] GAR-RUNTIME-IMPL-4Q live, hybrid, loopback control-plane, and distributed blockers
  - Source: `GAR-RUNTIME-IMPL-3N`, RFC 0034, RFC 0035, `GAR-SCALE-1F`.
...
- [ ] GAR-RUNTIME-IMPL-4R adapters, databases, UDFs, extensions, and effectful operations
  - Source: `GAR-RUNTIME-IMPL-3O`, RFC 0011, RFC 0023, adapter/governance docs.
...
- [ ] GAR-RUNTIME-IMPL-4S clean install public technical-preview usability gate
  - Source: `GAR-RUNTIME-IMPL-3Q`, public preview readiness, package-channel matrix, website
