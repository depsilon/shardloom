...
This inventory maps engine-replacement, Spark-displacement, performance-superiority, best-default,
production SQL/DataFrame, object-store/lakehouse, and managed-platform claim language to the
evidence that would be required before ShardLoom could make any public claim in that family.
...
| Performance superiority | Claim-grade workload-specific benchmark profile with complete lanes, versions, environment fingerprint, correctness evidence, certificates, Native I/O, no-fallback evidence, and release approval. | `not_claim_grade` |
