...
| Execution mode | What compute route executes? | `prepared_vortex`, `native_vortex`, `compatibility_import_certified`, `direct_compatibility_transient`, `auto`. |
...
| Direct one-shot route | `direct_compatibility_transient` | Reads/parses an admitted source and computes without persistent Vortex preparation. |
...
| Direct one-shot route | `direct_compatibility_transient` local one-shot timing, not Vortex-native. |
