...
- Crates.io version: <https://crates.io/crates/vortex/0.74.0>.
- Versioned docs: <https://docs.rs/vortex/0.74.0>.
- `cargo info vortex@0.74.0`: license `Apache-2.0`, Rust version `1.91.0`, repository
  <https://github.com/spiraldb/vortex>.

Dependency update:

- `shardloom-vortex` now requires optional `vortex = "0.74"`.
- `Cargo.lock` now resolves the upstream Vortex crate family to `0.74.0`.
- Default ShardLoom builds still do not enable upstream Vortex.
- Existing provider-version evidence has been refreshed from `0.73` to `0.74` for current Vortex
...
- Vortex `0.74.0` compatibility is a dependency/build compatibility claim only.
...
- `vortex-api-inventory` now also emits
  ...
  - `decimal128_columnar_provider_gate` uses upstream Vortex
    `ArrayRef::from_arrow(RecordBatch)` for flat Arrow Decimal128 columnar SourceState handoff,
    with precision/scale/null replay evidence and no scalar-row materialization.
