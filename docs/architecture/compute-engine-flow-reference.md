...
| Expression and operator semantics baseline | `shardloom-core` exposes `shardloom.expression_semantics.v1`, `shardloom.projection_semantics.v1`, `shardloom.filter_semantics.v1`, and `shardloom.limit_semantics.v1` for scoped in-memory row evaluation. The admitted baseline covers literals, column references, aliases, casts/try-casts, boolean `AND`/`OR`/`NOT`, `IS NULL`, `IS NOT NULL`, comparisons including bytewise binary equality/inequality, projection, filter selection, limit row-count semantics, UTF-8 string predicates/functions including scoped regex match, null coalesce/nullif/CASE, Date32 and UTC timestamp parsing/extract/arithmetic/difference including scoped interval literals inside temporal helper functions, scalar and row-value literal IN semantics, bounded scalar and row-value local-source IN-subquery semantics, scoped correlated `outer.<column>` local-source scalar/row-value IN, EXISTS, and quantified subquery filters, nested scalar IN-subquery materialization, joined and grouped/HAVING projected scalar/row-value/EXISTS/quantified subquery semantics, correlated joined and grouped/HAVING projected scalar/row-value/EXISTS/quantified subquery semantics, scoped local-source EXISTS predicate semantics, scoped local-source quantified ANY/ALL predicate semantics, HAVING-level local-source EXISTS/quantified subquery semantics, scoped SQL `UNION` / `UNION ALL` composition over admitted branch SELECTs, grouped/distinct/HAVING aggregate semantics, join/window fixture semantics, scoped binary hex literal and source-column helper projection semantics, and deterministic unsupported diagnostics with explicit null/type behavior. Advanced scalar blockers are explicit for decimal precision/scale casts, non-UTC/offset timestamp literals, timezone database conversion, arbitrary interval arithmetic outside scoped temporal helpers, and locale-aware collation; complex dtype blockers are explicit for list/array, struct/row, variant, union dtype, parent/child null policy, schema field identity, and broad binary source dtype/cast/runtime decoding outside scoped helper projections; advanced subquery blockers are explicit for scalar-left multi-column IN-subquery, unbound qualified references, or otherwise non-admitted broad ANSI subquery shapes; scoped source-qualified local subquery references, scoped local-source EXISTS, scoped quantified ANY/ALL predicates, HAVING-level local-source EXISTS/quantified subqueries, scoped correlated `outer.<column>` subquery filters, and scoped UTF-8 `RLIKE` / `REGEXP` / `REGEXP_LIKE` predicates are admitted separately. | Next slices broaden only the remaining split follow-up family: broad encoded-kernel/operator coverage. | This is a ShardLoom-native semantics contract and fixture/runtime building block, not broad SQL/DataFrame support, not blanket encoded-native operator coverage, not decimal/arbitrary-interval/timezone/collation/list/struct/variant/union-dtype/broad-binary-source admission, not locale-aware regex/collation parity, not broad ANSI subquery parity, not a production claim, and not performance evidence. Rows keep `fallback_attempted=false`, `external_engine_invoked=false`, and `claim_gate_status=not_claim_grade`. |
...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=65`
and `executable_fixture_count=55`, including scoped UTF-8 regex predicates, composed string functions, temporal arithmetic/
...
window row_number/rank/lag/ntile semantics, multi-key joins, source-qualified local subqueries, scoped SQL `X'<hex>'` binary literal projections, scoped `UNHEX(<utf8-column>)` / `FROM_BASE64(<utf8-column>)` binary helper projections, and deterministic unsupported
diagnostics for decimal, timezone, arbitrary interval arithmetic, collation, list/array, struct/row, variant,
union dtype, broad binary source dtype/casts/literals, scalar-left multi-column IN-subquery, unbound qualified references, and remaining non-admitted broad ANSI subquery shapes, and
