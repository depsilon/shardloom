...
| SQL local-source smoke | `sql-local-source-smoke` admits local CSV/JSONL/flat JSON direct-transient projection/optional-filter/limit, scalar aggregate, one-column group-by aggregate, single-key numeric `ORDER BY ... LIMIT ...` top-N, scoped `CAST(column AS dtype)` predicates for `int64`, `float64`, `utf8`, `boolean`, and `date32`, bounded `column IN (<literal>,...)` predicates over local rows, and one explicit local CSV inner equi-join shape. The aggregate slices cover `COUNT(*)`, `COUNT(column)`, `SUM`, `AVG`, `MIN`, and `MAX` across all rows or after the same scoped `WHERE` predicate. Execution certificate refs, source certificate refs, materialization boundaries, pushdown status, route fields, and claim-gate reasons are source-format-aware: CSV rows use `sql-local-source.csv.*`, flat JSON rows use `sql-local-source.json.*` plus `local_json_row_materialization_to_expression_semantics` and `not_applicable_local_json_transient`, and JSONL/NDJSON rows use `sql-local-source.jsonl.*` plus `local_jsonl_row_materialization_to_expression_semantics` and `not_applicable_local_jsonl_transient`; no-filter rows emit `filter_runtime_execution=false`, `predicate_operator_family=none`, and non-filter execution certificate suffixes such as `projection-limit`. The scalar path emits `aggregate_runtime_execution=true`; the group-by path emits `group_by_runtime_execution=true`; the top-N path emits sort key, direction, null-ordering blocker policy, and `top_n_runtime_execution=true` evidence; the cast path emits `predicate_operator_family=cast`, `cast_runtime_execution=true`, `cast_source_column`, and `cast_target_dtype` evidence when a predicate is present; the `IN` path emits `predicate_operator_family=in_predicate`, `in_predicate_runtime_execution=true`, and `in_list_value_count`, with deterministic blockers for empty, NULL, mixed DATE/non-DATE, and oversized lists; the join path emits `join_runtime_execution=true`, `join_type=inner_equi`, left/right source refs, join keys, scanned/matched/output row counts, and a scoped memory estimate. It performs ShardLoom-owned parsing, binding, planning, local CSV/JSONL/flat JSON read/parse, projection/optional-filter/limit, cast/IN predicate, aggregate, top-N, or inner hash-join evaluation, bounded inline JSONL result rendering or optional local JSONL/CSV output with format-specific sink certificate refs, and source/output/execution/materialization/no-fallback evidence. Python `ctx.read_csv(...)` and local flat `ctx.read_json(...)` wrap scoped projection/optional-filter/limit, preview/select-star, scalar aggregate, one-column group-by aggregate, and single-key numeric top-N paths through query-builder chains for local `.json`, `.jsonl`, and `.ndjson` files, including local JSONL writes and `write_csv(...)` for the scoped local CSV sink; direct Python `client.sql_local_source_smoke(...)` calls expose typed IN/join and broader SQL smoke evidence. | Later slices can add Python/DataFrame joins, nested JSON/JSONPath, Parquet/Vortex SQL sources, Parquet/Arrow/Avro/ORC/Vortex output sinks, multi-output fanout, pushdown, broader typed coercions/functions, NULL/subquery-backed IN semantics, multi-key/grouped aggregate generality, named grouped aggregate aliases, outer/semi/anti/cross/multi-key/expression joins, collation/null ordering parity, window ranking, and broader planner lowering only with source/sink/correctness evidence. | This is a scoped fixture-smoke local CSV/JSONL/flat JSON SQL/Python query-builder path. It is not broad SQL/DataFrame support, not generalized join or ordering support, not nested JSON support, not a broad output/fanout claim, not a production SQL claim, not an object-store/table source claim, and not performance evidence. |
...
CSV/JSONL/flat JSON smoke, arbitrary functions, joins, subqueries, UDFs, and table-function shapes outside
...
path for local CSV, flat JSONL/NDJSON, and flat top-level `.json` `SELECT`
...
local CSV and flat JSON/JSONL/NDJSON projection/optional-filter/limit, preview/select-star, scalar aggregate, one-column grouped
