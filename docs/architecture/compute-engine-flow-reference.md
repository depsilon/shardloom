...
`sink_artifact_digest`, no-fallback fields, and claim boundary text. The remaining fanout,
and reuse-level slices stay planned.
...
`claim_gate_status=not_claim_grade`. Runtime fanout and reuse-level slices stay planned.
...
`cache_invalidation_secret_redaction_status`, and
...
`claim_gate_status`. SourceState, VortexPreparedState, OutputPlan, report-only fanout rows, and
cache invalidation rows expose their current scoped subsets; runtime fanout and broader sink
artifact proof remain planned.
