...
through `GAR-IOREUSE-1L` cover cold-lane attribution, Vortex-native source/sink/split preparation,
differential preparation, capillary I/O with PulseWeave control, and scout ingress/triage.
`GAR-PERF-2J` and `GAR-PERF-2K` cover layout/write advice and cold-lane copy/buffer evidence.
Until those slices land, the flow must keep cold costs visible instead of treating them as query
compute or hiding them behind `auto`.
...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=37`
and `executable_fixture_count=18`, including composed string functions, temporal arithmetic/
difference, CASE, IN-list NULL semantics, bounded scalar IN subqueries with admitted WHERE/ORDER BY/
LIMIT clauses, HAVING IN-subquery predicates, COUNT(DISTINCT), hidden HAVING aggregates, mixed
window row_number/rank/lag/ntile semantics, multi-key joins, and deterministic unsupported
diagnostics for decimal, timezone, interval, regex, collation, list/array, struct/row, variant,
union, binary-source, row-value/multi-column/nested/joined/grouped/correlated subquery, and
EXISTS/ANY/ALL families. `runtime.5g-f1` now exposes the current physical operator plan, operator
...
shapes, including bounded admitted subquery WHERE/ORDER BY/LIMIT tails and HAVING over aggregate output aliases. Bounded `IN`
...
row-value, multi-column, nested, joined, grouped/HAVING-internal, correlated, and EXISTS/ANY/ALL subquery shapes remain deterministic blockers.
...
path-safety, commit mode, Vortex artifact/reopen fields when feature-gated and admitted, and
no-fallback/no-external-engine evidence. Python `ShardLoomSession` can reuse admitted local
query-builder and session-bound SQL workflow collect/write/fanout reports only when statement,
source fingerprints, and output artifact fingerprints still match; changed output artifacts
invalidate reuse.
