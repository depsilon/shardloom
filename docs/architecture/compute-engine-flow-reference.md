...
`source_metadata_snapshot_status=per_batch_source_metadata_reused`. Those statuses mean scoped
process/source-metadata reuse only; they are not a daemon, service, hidden benchmark fast mode,
encoded-native claim, or performance claim.
