...
invalidation, and reuse-level slices stay planned.
...
`claim_gate_status=not_claim_grade`. Runtime fanout, invalidation, and reuse-level slices stay
planned.
...
`claim_gate_status`. SourceState, VortexPreparedState, OutputPlan, and report-only fanout rows
expose their current scoped subsets; runtime fanout and broader sink artifact proof remain planned.
