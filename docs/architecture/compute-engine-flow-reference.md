...
and `executable_fixture_count=27`, including composed string functions, temporal arithmetic/
difference, CASE, IN-list NULL semantics, row-value literal IN predicates, row-value IN-subquery predicates, scoped quantified ANY/ALL subqueries, scoped SQL UNION
...
non-admitted quantified shapes while scoped EXISTS and scoped quantified ANY/ALL are admitted. `runtime.5g-f1` now exposes the current physical operator plan, operator
...
IN-subquery, nested, joined, grouped/HAVING-internal, correlated, and other non-admitted
