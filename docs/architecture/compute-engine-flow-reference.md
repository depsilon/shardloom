...
| In-process session runtime | `ShardLoomSessionModelReport` exists as report-only explicit session/registry posture, `traditional-analytics-vortex-batch-run` emits scoped session-backed prepared/native local-artifact evidence, and `traditional-analytics-prepare-batch-run` prepares compatibility inputs once before executing the prepared batch in the same process. Python exposes a caller-owned `ShardLoomSession` for local `vortex_ingest` prepared-state reuse plus admitted local query-builder collect/write/fanout result reuse when source, output, and prepared-artifact fingerprints still match. | Continue broadening only evidence-backed session reuse: broader OutputPlan reuse, schema/dictionary caches, buffer-pool reuse, object-store/table reuse, and non-local workflows remain gated. | Session support is local, caller-owned, and explicit-close only. It is not a daemon/service, remote server, hidden global cache, distributed cache, production claim, public SQL/DataFrame claim, or performance/superiority claim. |
...
`prepare_batch_*` evidence, then invokes the same prepared/native batch path without including
preparation in child query timing. The prepared/native batch path runs a comma-separated list of
...
multi-key group-by aggregate, aggregate aliases, multi-key numeric `ORDER BY ... LIMIT ...` top-N
shapes over projection rows and aggregate output aliases, scoped single-/multi-key inner
equi-joins, scoped computed projections and multi-key numeric top-N over joined rows, scoped
scalar/grouped join aggregates with aggregate-output top-N, and scoped
...
admitted projection/scalar/multi-key grouped/top-N/aggregate-output-top-N/join/join-computed-top-N/join-aggregate runtime for local `.json`, `.jsonl`, and
...
null ordering, collation parity, outer/expression joins, windows, broader result sinks, broad
...
aggregate, aggregate aliases, multi-key numeric top-N, aggregate-output top-N, scoped joined
computed-projection/top-N, joined aggregate-output top-N, and scoped local scalar IN-subquery
shapes. Bounded `IN`
