...
| SQL local-source smoke | `sql-local-source-smoke` admits local CSV direct-transient projection/filter/limit and scalar aggregate shapes. The aggregate slice covers `COUNT(*)`, `COUNT(column)`, `SUM`, `AVG`, `MIN`, and `MAX` after the same simple `WHERE` predicate, with one scalar output row and `execution_certificate_ref=sql-local-source.csv.aggregate-filter-limit.execution.v1`. It performs ShardLoom-owned parsing, binding, planning, local CSV read/parse, projection/filter/limit or scalar aggregate evaluation, bounded inline JSONL result rendering or optional local JSONL output, and source/output/execution/materialization/no-fallback evidence. Python `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` wraps the projection path, and `ctx.read_csv(...).filter(...).aggregate(...).limit(1).collect()/write(...)` wraps the scalar aggregate path. | Later slices can add Python `ctx.sql(...)`, Parquet/Vortex SQL sources, broader output sinks, pushdown, grouped aggregates, joins, order/top-N, and broader planner lowering only with source/sink/correctness evidence. | This is a scoped fixture-smoke local CSV SQL/Python query-builder path. It is not broad SQL/DataFrame support, not a production SQL claim, not an object-store/table source claim, and not performance evidence. |
...
path for local CSV `SELECT` projection/filter/limit and scalar aggregate shapes. That SQL path can
render bounded inline JSONL or write local JSONL output with output Native I/O certificate fields.
Python `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` and
`ctx.read_csv(...).filter(...).aggregate(...).limit(1).collect()/write(...)` wrap the same admitted
runtime. Adjacent formats, grouped aggregates, joins, windows, broader result sinks, broad
...
local CSV projection/filter/limit and scalar aggregate shapes.
