...
| I/O reuse and cross-format fanout | Prepared/native rows reuse selected source metadata and scenario-family source-state. GAR-IOREUSE-1A adds a universal local SourceState benchmark/report contract for source discovery/schema identity/fingerprints/parse-plan posture across CSV, JSONL, Parquet, Arrow IPC, Avro, and ORC rows. GAR-IOREUSE-1B adds a VortexPreparedState benchmark/report contract for prepared artifact refs/digests, preparation timing separation, source-state linkage, and scoped reuse posture. Output planning, fanout, invalidation, and sink artifact evidence remain planned. | GAR-IOREUSE-1 continues with `OutputPlan`, benchmark families for `io_reuse_and_fanout`, `output_plan_reuse`, `cross_format_output`, and `generated_source_output`, plus cache invalidation and evidence-safe reuse levels. | Input and output formats remain decoupled. SourceState and VortexPreparedState evidence are not output support by themselves, not a performance claim, and not object-store/lakehouse or Foundry production support. |
...
no-fallback fields, and claim boundary text. The remaining OutputPlan, fanout, invalidation, and
reuse-level slices stay planned.
...
`claim_gate_status`. SourceState and VortexPreparedState rows expose their current scoped subsets;
OutputPlan, fanout, and sink artifact proof remain planned.
