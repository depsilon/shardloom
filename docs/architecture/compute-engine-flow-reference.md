...
aggregate output rows, multi-key scalar `ORDER BY ... LIMIT ...` top-N
...
computed-projection/top-N, aggregate-output `HAVING`, joined aggregate-output `HAVING`/top-N, and
scoped local scalar IN-subquery
shapes. Bounded `IN`
...
counts. Scoped HAVING rows expose `having_runtime_execution`, `having_source_column`, and
before/after aggregate-row counts. Scoped string-function predicates/projections expose
