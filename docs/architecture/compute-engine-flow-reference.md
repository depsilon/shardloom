...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=47`
and `executable_fixture_count=32`, including scoped UTF-8 regex predicates, composed string functions, temporal arithmetic/
...
IN/EXISTS/quantified subquery predicates, COUNT(DISTINCT), hidden HAVING aggregates, mixed
window row_number/rank/lag/ntile semantics, multi-key joins, and deterministic unsupported
diagnostics for decimal, timezone, arbitrary interval arithmetic, collation, list/array, struct/row, variant,
