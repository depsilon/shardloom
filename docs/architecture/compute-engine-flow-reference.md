...
window row_number/rank/lag/ntile semantics, multi-key joins, source-qualified local subqueries, scoped SQL `X'<hex>'`, `BINARY '<utf8>'`, and `BLOB '<utf8>'` binary literal projections, scoped binary casts and equality/inequality cast predicates, scoped `UNHEX(<utf8-column>)` / `FROM_BASE64(<utf8-column>)` binary helper projections, scoped decimal128/decimal/numeric cast projection and predicate fixtures with exact JSONL string and CSV text boundaries, scoped same-scale decimal add/subtract/multiply projections, and deterministic unsupported
diagnostics for non-UTC timestamp/timezone, arbitrary interval arithmetic, collation, decimal division/mixed-scale coercion or typed decimal sinks, complex equality/accessors/casts/nested source decoding/flat sinks beyond scoped JSONL/result-boundary `ARRAY[...]` and `STRUCT(<source column>, ...)` projections, variant,
