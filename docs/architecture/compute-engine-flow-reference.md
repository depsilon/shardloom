...
| SQL local-source smoke | `sql-local-source-smoke` admits local CSV/JSONL direct-transient projection/filter/limit, scalar aggregate, one-column group-by aggregate, single-key numeric `ORDER BY ... LIMIT ...` top-N, scoped `CAST(column AS dtype)` predicates for `int64`, `float64`, `utf8`, `boolean`, and `date32` over local rows, and one explicit local CSV inner equi-join shape. The aggregate slices cover `COUNT(*)`, `COUNT(column)`, `SUM`, `AVG`, `MIN`, and `MAX` after the same simple `WHERE` predicate. The scalar path emits `execution_certificate_ref=sql-local-source.csv.aggregate-filter-limit.execution.v1`; the group-by path emits `execution_certificate_ref=sql-local-source.csv.group-by-aggregate-filter-limit.execution.v1` plus `group_by_runtime_execution=true` evidence; the top-N path emits `execution_certificate_ref=sql-local-source.csv.order-by-topn-filter-limit.execution.v1` plus sort key, direction, null-ordering blocker policy, and `top_n_runtime_execution=true` evidence; the cast path emits `predicate_operator_family=cast`, `cast_runtime_execution=true`, `cast_source_column`, and `cast_target_dtype` evidence; the join path emits `execution_certificate_ref=sql-local-source.csv.inner-equi-join-filter-limit.execution.v1` plus `join_runtime_execution=true`, `join_type=inner_equi`, left/right source refs, join keys, scanned/matched/output row counts, and a scoped memory estimate. It performs ShardLoom-owned parsing, binding, planning, local CSV/JSONL read/parse, projection/filter/limit, cast predicate, aggregate, top-N, or inner hash-join evaluation, bounded inline JSONL result rendering or optional local JSONL output, and source/output/execution/materialization/no-fallback evidence. Python `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)`, `ctx.read_csv(...).filter(...).aggregate(...).limit(1).collect()/write(...)`, `ctx.read_csv(...).filter(...).group_by(...).agg(...).limit(n).collect()/write(...)`, and `ctx.read_csv(...).select(...).filter(...).sort(...).limit(n).collect()/write(...)` wrap the scoped CSV projection, scalar aggregate, one-column group-by, and top-N paths; direct Python `client.sql_local_source_smoke(...)` calls expose typed join evidence. | Later slices can add Python `ctx.sql(...)`, Python/DataFrame joins, Parquet/Vortex SQL sources, broader output sinks, pushdown, broader typed coercions/functions, multi-key/grouped aggregate generality, named grouped aggregate aliases, outer/semi/anti/cross/multi-key/expression joins, collation/null ordering parity, window ranking, and broader planner lowering only with source/sink/correctness evidence. | This is a scoped fixture-smoke local CSV/JSONL SQL/Python query-builder path. It is not broad SQL/DataFrame support, not generalized join or ordering support, not a production SQL claim, not an object-store/table source claim, and not performance evidence. |
