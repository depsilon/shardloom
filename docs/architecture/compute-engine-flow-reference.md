...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=101`
and `executable_fixture_count=85`, including scoped UTF-8 regex predicates, composed string functions, temporal arithmetic/
difference, CASE, IN-list NULL semantics, row-value literal IN predicates, scalar and row-value IN/NOT IN subquery predicates, projected scalar/row-value IN/NOT IN, EXISTS/NOT EXISTS, and quantified subqueries, correlated joined and grouped/HAVING projected scalar/row-value IN/NOT IN, EXISTS/NOT EXISTS, and quantified subqueries, scoped EXISTS/NOT EXISTS and quantified ANY/ALL subqueries, scoped SQL UNION
