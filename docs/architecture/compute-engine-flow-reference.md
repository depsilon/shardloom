...
multi-key group-by aggregate, single-key numeric `ORDER BY ... LIMIT ...` top-N shapes, and scoped
...
admitted projection/scalar/multi-key grouped/top-N runtime for local `.json`, `.jsonl`, and
`.ndjson` files. Adjacent formats, nested JSON/JSONPath, named grouped aggregate aliases,
