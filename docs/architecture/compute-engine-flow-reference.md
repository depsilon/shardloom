...
multi-key group-by aggregate, aggregate aliases, multi-key scalar `ORDER BY ... LIMIT ...` top-N
shapes over projection rows, aggregate output aliases, and group keys, scoped local-source
inner/outer/semi/anti equi-joins and cross joins, scoped computed projections and multi-key scalar
top-N over joined rows, scoped
scalar/grouped join aggregates with aggregate-output/group-key top-N, and scoped
