...
| SQL local-source smoke | `sql-local-source-smoke` admits one local CSV direct-transient shape: `SELECT <columns> FROM '<local.csv>' WHERE <simple predicate> LIMIT <n>`. It performs ShardLoom-owned parsing, binding, planning, local CSV read/parse, projection/filter/limit evaluation, bounded inline JSONL result rendering or optional local JSONL output, and source/output/execution/materialization/no-fallback evidence. Python `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` wraps this same admitted path. | Later slices can add Python `ctx.sql(...)`, Parquet/Vortex SQL sources, broader output sinks, pushdown, aggregates, joins, order/top-N, and broader planner lowering only with source/sink/correctness evidence. | This is a scoped fixture-smoke local CSV SQL/Python query-builder path. It is not broad SQL/DataFrame support, not a production SQL claim, not an object-store/table source claim, and not performance evidence. |
...
path for one local CSV `SELECT` projection/filter/limit shape. That SQL path can render bounded
inline JSONL or write local JSONL output with output Native I/O certificate fields, and Python
`ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` wraps the same admitted
runtime. Adjacent formats, broader operators, broader result sinks, broad SQL/DataFrame access, and
broader transient runtime behavior still return deterministic unsupported diagnostics.
...
local CSV projection/filter/limit shape.
