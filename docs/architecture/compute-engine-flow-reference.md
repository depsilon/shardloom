...
source-state families are scoped in-process derived state, so rows report
`source_state_digest_status=not_emitted_scoped_in_memory_source_state` until GAR-IOREUSE-1A defines
universal content-addressed SourceState digests. Those statuses mean scoped
process/source-metadata/source-state reuse only; they are not a daemon, service, hidden benchmark
fast mode, encoded-native claim, or performance claim.
