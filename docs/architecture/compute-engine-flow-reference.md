...
| Source-free generated output | No-input smoke/capability behavior exists, benchmark synthetic fixtures exist, `shardloom.generated_source_certificate_contract.v1` exposes the case split, and GAR-GEN-1C adds one scoped `generated-source-user-rows-smoke`/Python `ctx.from_rows([...]).write(...)` local JSONL output path with generated-source and output evidence. Engine-native generator nodes, SQL `VALUES`/literal execution, broad DataFrame execution, object-store writes, and Foundry generated-output runtime remain unsupported/report-only. | GAR-GEN follow-through now moves to engine-native local generator nodes and the source-free SQL/DataFrame/API admission matrix. GAR-COMPAT-1B and GAR-NOVEL-1A keep compatibility, observability, and lineage alignment with the same contract. | No source Native I/O certificate is claimed when no source dataset is read. GAR-GEN-1C is fixture-smoke-only and local-output-only; it is not a broad source-free runtime, object-store/lakehouse, Foundry, SQL/DataFrame, performance, package, or production claim. |
...
| Scale claim gate | GAR-SCALE-1A adds `scale_contract_schema_version=shardloom.traditional_analytics.scale_claim_gate.v1` plus scale profile/status, volume, split, memory, spill, shuffle, retry, commit, platform, no-fallback, and claim-boundary fields. GAR-SCALE-1B adds `split_manifest_contract_schema_version=shardloom.traditional_analytics.split_manifest.v1` plus SplitManifest ID/digest, SourceState linkage, ranges, estimates, pushdown, retry/runtime/row/spill/output refs, and `split_claim_gate_status=not_split_scale_grade`. GAR-SCALE-1C adds `memory_spill_contract_schema_version=shardloom.traditional_analytics.memory_spill_backpressure.v1` plus memory budget, operator memory budget, peak memory, budget-exceeded, spill location/read/write/file/cleanup, backpressure, OOM-prevention, no-fallback, and `memory_spill_claim_gate_status=not_larger_than_memory_grade`. Current ShardLoom rows are restricted to `local_smoke` or `local_claim_grade` and report `scale_claim_gate_status=not_scale_grade`. | GAR-SCALE-1D through GAR-SCALE-1H can add shuffle/repartition, object-store/table ladder, distributed report-only protocol, scale benchmark profiles, and Foundry scale proof boundaries. | Scale, SplitManifest, and memory/spill fields are fail-closed. They do not authorize literal any-volume support, Spark-replacement, larger-than-memory execution, runtime spill, backpressure runtime, split-parallel runtime, object-store/table runtime, distributed runtime, Foundry production support, managed-platform proof, or performance/superiority claims. |
