...
| SQL local-source smoke | `sql-local-source-smoke` admits local CSV/JSONL direct-transient projection/filter/limit, scalar aggregate, one-column group-by aggregate, single-key numeric `ORDER BY ... LIMIT ...` top-N, scoped `CAST(column AS dtype)` predicates for `int64`, `float64`, `utf8`, `boolean`, and `date32` over local rows, and one explicit local CSV inner equi-join shape. The aggregate slices cover `COUNT(*)`, `COUNT(column)`, `SUM`, `AVG`, `MIN`, and `MAX` after the same simple `WHERE` predicate. Execution certificate refs, source certificate refs, materialization boundaries, pushdown status, and claim-gate reasons are source-format-aware: CSV rows use `sql-local-source.csv.*`, while JSONL/NDJSON rows use `sql-local-source.jsonl.*` plus `local_jsonl_row_materialization_to_expression_semantics` and `not_applicable_local_jsonl_transient`. The scalar path emits `aggregate_runtime_execution=true`; the group-by path emits `group_by_runtime_execution=true`; the top-N path emits sort key, direction, null-ordering blocker policy, and `top_n_runtime_execution=true` evidence; the cast path emits `predicate_operator_family=cast`, `cast_runtime_execution=true`, `cast_source_column`, and `cast_target_dtype` evidence; the join path emits `join_runtime_execution=true`, `join_type=inner_equi`, left/right source refs, join keys, scanned/matched/output row counts, and a scoped memory estimate. It performs ShardLoom-owned parsing, binding, planning, local CSV/JSONL read/parse, projection/filter/limit, cast predicate, aggregate, top-N, or inner hash-join evaluation, bounded inline JSONL result rendering or optional local JSONL output, and source/output/execution/materialization/no-fallback evidence. Python `ctx.read_csv(...)` and local flat `ctx.read_json(...)` wrap scoped projection/filter/limit, scalar aggregate, one-column group-by aggregate, and single-key numeric top-N paths through query-builder chains; direct Python `client.sql_local_source_smoke(...)` calls expose typed join and broader SQL smoke evidence. | Later slices can add Python `ctx.sql(...)`, Python/DataFrame joins, Parquet/Vortex SQL sources, broader output sinks, pushdown, broader typed coercions/functions, multi-key/grouped aggregate generality, named grouped aggregate aliases, outer/semi/anti/cross/multi-key/expression joins, collation/null ordering parity, window ranking, and broader planner lowering only with source/sink/correctness evidence. | This is a scoped fixture-smoke local CSV/JSONL SQL/Python query-builder path. It is not broad SQL/DataFrame support, not generalized join or ordering support, not a production SQL claim, not an object-store/table source claim, and not performance evidence. |
...
projection/filter/limit smoke, functions, joins, subqueries, UDFs, and optional
...
path for local CSV and flat JSONL/NDJSON `SELECT` projection/filter/limit, scalar aggregate,
...
local CSV and flat JSONL/NDJSON projection/filter/limit, scalar aggregate, one-column grouped
