...
| Scale claim gate | GAR-SCALE-1A adds `scale_contract_schema_version=shardloom.traditional_analytics.scale_claim_gate.v1` plus `scale_profile`, `scale_claim_status`, data volume, row/file/partition/split counts, memory budget, peak memory, spill, shuffle, retry, output commit, object-store/table, remote-worker, Foundry, no-fallback, and claim-boundary fields to benchmark rows. GAR-SCALE-1B adds `split_manifest_contract_schema_version=shardloom.traditional_analytics.split_manifest.v1` plus SplitManifest ID/digest, SourceState linkage, split count, split ID, byte/row ranges, estimated rows/bytes, projection mask, filter pushdown status, retry/runtime/row/spill/output refs, no-fallback fields, and `split_claim_gate_status=not_split_scale_grade`. Current ShardLoom rows are restricted to `local_smoke` or `local_claim_grade` and report `scale_claim_gate_status=not_scale_grade`. | GAR-SCALE-1C through GAR-SCALE-1H can add bounded memory/spill/backpressure, shuffle/repartition, object-store/table ladder, distributed report-only protocol, scale benchmark profiles, and Foundry scale proof boundaries. | Scale and SplitManifest fields are fail-closed. They do not authorize literal any-volume support, Spark-replacement, larger-than-memory execution, split-parallel runtime, object-store/table runtime, distributed runtime, Foundry production support, managed-platform proof, or performance/superiority claims. |
