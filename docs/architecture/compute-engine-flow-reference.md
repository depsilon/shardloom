...
`ctx.range(...).write(...)` for one ShardLoom-native range JSONL output smoke path.
...
them for scoped user-row, literal-table, and calendar local JSONL smokes, and GAR-GEN-1D emits them
for the scoped engine-native range local JSONL smoke:
...
blocker/status does not admit engine-native `sequence`, engine-native `values`, synthetic
generators, SQL literals, DataFrame expressions, object-store writes, or Foundry output.
