...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=44`
and `executable_fixture_count=26`, including composed string functions, temporal arithmetic/
difference, CASE, IN-list NULL semantics, row-value literal IN predicates, row-value IN-subquery predicates, scoped SQL UNION
composition, bounded scalar IN subqueries with admitted WHERE/ORDER BY/LIMIT clauses, HAVING
IN-subquery predicates, COUNT(DISTINCT), hidden HAVING aggregates, mixed
...
union dtype, binary-source, scalar-left multi-column IN-subquery, nested/joined/grouped/correlated subquery, and
ANY/ALL families while scoped EXISTS is admitted. `runtime.5g-f1` now exposes the current physical operator plan, operator
...
shapes, and local EXISTS/NOT EXISTS subquery shapes, including bounded admitted subquery
...
counts, and the two-valued presence result. Scoped HAVING rows expose `having_runtime_execution`,
...
IN-subquery, nested, joined, grouped/HAVING-internal, correlated, and ANY/ALL subquery shapes
