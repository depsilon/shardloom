...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=117`
and `executable_fixture_count=95`, including scoped UTF-8 regex predicates, composed string functions, temporal arithmetic/
...
IN/NOT IN, EXISTS/NOT EXISTS, and quantified subquery predicates, scoped correlated `outer.<column>` scalar/row-value IN/NOT IN, EXISTS/NOT EXISTS, and quantified filters, scoped source-qualified subquery diagnostics, scoped subquery-backed predicate/CASE projections, COUNT(DISTINCT), hidden HAVING aggregates, mixed
...
IN/NOT IN, EXISTS/NOT EXISTS, and quantified subqueries, scoped UTF-8 regex predicates, and scoped interval literals inside temporal helper functions are admitted. `runtime.5g-f1` now exposes the current physical operator plan, operator
