...
| SQL local-source smoke | `sql-local-source-smoke` admits one local CSV direct-transient shape: `SELECT <columns> FROM '<local.csv>' WHERE <simple predicate> LIMIT <n>`. It performs ShardLoom-owned parsing, binding, planning, local CSV read/parse, projection/filter/limit evaluation, bounded inline JSONL result rendering, and source/execution/materialization/no-fallback evidence. | Later slices can add Python `ctx.sql(...)`, Parquet/Vortex SQL sources, output sinks, pushdown, aggregates, joins, order/top-N, and broader planner lowering only with source/sink/correctness evidence. | This is a scoped fixture-smoke local CSV SQL path. It is not broad SQL/DataFrame support, not a production SQL claim, not an object-store/table source claim, and not performance evidence. |
