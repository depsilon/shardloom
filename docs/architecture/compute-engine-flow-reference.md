...
`ctx.read_csv(...).select(...).filter(...).sort(...).limit(n).collect()/write(...)` wrap the same
...
local CSV projection/filter/limit, scalar aggregate, one-column grouped aggregate, and single-key
numeric top-N shapes.
