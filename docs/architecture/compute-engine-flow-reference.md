...
`source_state_category_metric_*` fields. All batch source-state paths emit
`source_state_prepare_timing_scope=batch_shared_pre_scenario`. Those statuses mean scoped
...
`source_state_dimension_label_*` and `source_state_category_metric_*` fields to
show which scoped source-state family was reused. Rows that still use explicit single-scenario CLI
...
join-aggregate children, and can reuse one category/metric grouped state across distinct-count and
high-cardinality string-group/distinct children. Both reuse families emit explicit `source_state_*`
evidence and remain residual-native runtime plumbing.
