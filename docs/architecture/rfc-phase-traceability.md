...
as `metric:vortex.filter` while zero-result scans report no reader chunks. GAR-0026-S adds the
reader-generated conjunctive selection-vector bridge for supplied encoded kernel inputs and updates
provider fields to distinguish that bridge contract from the still-missing real `flag,value`
filter-column batches and encoding-specific lowering in the benchmark Vortex scan path. Broader
