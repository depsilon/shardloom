...
| RFC 0038 | Accepted | Priority 1.7 | Defines the top-level plan/execution facade catch-up: replacing the unreleased placeholder `PlanKind::NativeVortexScan` and no-op `execute()` shape with typed plan variants and `ShardLoomExecutionResult` surfaces for current local, prepared encoded, source-bound, and reader-backed Vortex primitive paths. No SQL/DataFrame runtime, object-store runtime, writes, external engine invocation, fallback execution, or legacy facade compatibility claim is authorized by the RFC alone. |
