...
| Priority 2.7 - source-backed benchmark matrix and benchmark-suite overhaul | RFC 0040 Benchmark Suite and Platform-Learning Hardening | RFC 0009 Benchmark Methodology; RFC 0015 Correctness; RFC 0025 Competitive/no-fallback; RFC 0029 Evidence; RFC 0031 Native I/O | source-backed correctness/benchmark matrix; `BenchmarkSuiteCatalogReport`; `BenchmarkConstitutionRequirementReport`; scenario taxonomy; dataset-profile matrix; `BenchmarkEnginePluginContract`; coverage-table rows; platform-inspired neutral capability reports | Report/code surfaces now define the local-first suite catalog and source-backed benchmark matrix. Benchmarks remain local/platform-neutral by default; Photon/Fabric/Snowflake and other managed systems are design references only, not benchmark dependencies or fallback execution paths. Comparative benchmark execution and measured claim rows remain blocked until later release-readiness work. |
| Priority 2.8 - crate-level posture cleanup | RFC 0041 Feature/Build Matrix and Crate Posture | RFC 0024 Release Engineering; RFC 0026 Encoded Native Reads; RFC 0031 Native I/O; RFC 0038 Facade | crate docs/export posture for executable, report-only, blocked, future, and prohibited-fallback surfaces | Docs/posture cleanup only; no runtime expansion, dependency expansion, external engine invocation, or fallback execution. |
...
| Priority 3.5 - workspace feature/build validation matrix | RFC 0041 Feature/Build Matrix and Crate Posture | RFC 0024 Release Engineering; RFC 0030 API/Deployment/Baselines; RFC 0036 Foundry Integration Pack | default/all/no-default feature checks; Vortex feature combinations; packaging/deployment; benchmark extras; future Foundry optional package surfaces; feature-disabled unsupported diagnostics | Release-readiness validation only; no release publication, runtime expansion, dependency expansion, external engine invocation, or fallback execution. |
...
| Priority 3.9 - typed command/result envelope and CLI modularity | RFC 0039 Typed Command/Result Envelope and CLI Modularity | RFC 0030 Universal API/Deployment/Baselines; RFC 0035 REST/Event/API; RFC 0037 Client/Wrapper Surface; Priority 3.7 evidence/policy hardening | typed command/result/evidence envelope; result/result_refs/artifacts/certificates/policy/lifecycle/capability payload slots; modular CLI handlers; shared renderer; golden JSON fixtures | Clean pre-release protocol/CLI replacement. No REST server, wrapper implementation, runtime expansion, external engine invocation, or fallback execution is authorized by this row. |
...
| RFC 0039 | Accepted | Priority 3.9 | Defines the typed command/result/evidence envelope and CLI modularity overhaul: replacing the early flat `(key, value)` payload model, adding typed result/artifact/certificate/policy/lifecycle/capability slots, centralizing rendering/diagnostics/fallback reporting, and splitting command handlers by capability family. No REST server, wrapper implementation, benchmark execution, runtime expansion, external engine invocation, or fallback execution is authorized by the RFC alone. |
| RFC 0040 | Partially implemented | Priority 2.7 | Defines the benchmark-suite and platform-learning hardening lane: local-first benchmark suite catalog, scenario taxonomy, dataset profiles, benchmark constitutions, plugin-based local engines, coverage tables, and neutral capability reports inspired by Photon/Fabric/Snowflake lessons. Current implementation adds `BenchmarkSuiteCatalogReport`, `BenchmarkConstitutionRequirementReport`, local optional engine plugin contracts, coverage rows, and the source-backed matrix report surface. Managed platforms are design references only; comparative benchmark execution, measured benchmark rows, performance claims, managed-platform benchmark lanes, credentials, dependencies, external engine invocation, and fallback execution remain unauthorized. |
| RFC 0041 | Accepted | Priority 2.8, Priority 3.5 | Defines the workspace feature/build validation matrix and crate-level posture cleanup: default/all/no-default feature checks, key Vortex/package/benchmark/Foundry feature combinations, feature-disabled unsupported diagnostics, build evidence fields, and crate docs/export posture for executable, report-only, blocked, future, and prohibited-fallback surfaces. No package publication, dependency expansion, runtime expansion, object-store execution, external engine invocation, or fallback execution is authorized by the RFC alone. |
