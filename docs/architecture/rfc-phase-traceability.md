...
| GAR-GEN-1 | Source-free generated-output planning and `GeneratedSourceCertificate` | RFC 0031, RFC 0032, RFC 0033, RFC 0036, RFC 0037, RFC 0039 | Planned source-free generated-output lane distinguishing `no_dataset_smoke`, `user_generated_source`, and `engine_native_generated_source`; future report/runtime slices must expose generated-source fields, no source Native I/O certificate when no source dataset was read, output sink evidence, no-fallback/no-external-engine fields, SQL/DataFrame report-only boundaries, and S3/object-store/Foundry blockers. |
