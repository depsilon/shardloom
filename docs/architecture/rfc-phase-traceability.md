...
| GAR-GEN-1 | Source-free generated-output planning and `GeneratedSourceCertificate` | RFC 0031, RFC 0032, RFC 0033, RFC 0036, RFC 0037, RFC 0039 | Partially completed source-free generated-output lane. GAR-GEN-1A/1B add `shardloom.generated_source_certificate_contract.v1`, CLI/Python capability fields, and Python typed accessors that distinguish `no_dataset_smoke`, `user_generated_source`, and `engine_native_generated_source`; current rows keep no source Native I/O certificate when no source dataset was read, no-dataset smoke as `smoke_only`, generated-source runtime as report-only, no-fallback/no-external-engine fields, SQL/DataFrame report-only boundaries, and S3/object-store/Foundry blockers. Future runtime slices must add generated rows, output sink evidence, materialization/decode evidence, and scoped output certificates before any generated-output claim. |
