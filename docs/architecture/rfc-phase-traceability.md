...
| RFC 0021 | Partially implemented | GAR-0021-A, GAR-RUNTIME-IMPL-4D-S1, GAR-RUNTIME-IMPL-4D-S2, GAR-RUNTIME-IMPL-4D-S3, GAR-RUNTIME-IMPL-4D-S4, Ongoing | Expression/kernel architecture exists in principle. GAR-0021-A closes the approximate aggregate/sketch admission contract with report-only `approx_count_distinct`/alias surfaces, sketch-state/merge/serialization, encoded-strategy, exact-reference, benchmark, execution-certificate, Native I/O, no-fallback, and claim-boundary fields. GAR-0021-H adds explicit prepared/native source-backed scan evidence fields, but residual operators remain ShardLoom-native unless a later slice admits an encoded/native provider. GAR-RUNTIME-IMPL-4D-S1 adds scoped ShardLoom-native UTF-8 string predicate runtime for `utf8_starts_with` / `utf8_contains` / `utf8_ends_with` and local SQL `LIKE 'prefix%'` / `LIKE '%contains%'` / `LIKE '%suffix'` lowering with deterministic unsupported diagnostics for other LIKE shapes. GAR-RUNTIME-IMPL-4D-S2 adds scoped ISO Date32 parsing/formatting, `date_year` / `date_month` / `date_day`, UTF-8/Date32 casts, local SQL `DATE 'YYYY-MM-DD'` predicates, CSV ISO date inference, and deterministic invalid-date/non-Date32 blockers. GAR-RUNTIME-IMPL-4D-S3 adds scoped local SQL logical `AND` predicate runtime over admitted leaves; GAR-RUNTIME-IMPL-4D-S4 adds scoped logical `OR` over admitted leaves while preserving `AND` precedence inside `OR` branches. Full expression, function, and kernel coverage remains ongoing. |
