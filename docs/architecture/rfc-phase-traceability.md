...
`docs/architecture/phased-execution-plan.md` is the source of truth for the Planned queue, completed phase ledger, deferred work, and CG closeout state. This document maps phase and CG work to governing RFCs; it may record historical traceability, but it must not introduce a competing current queue.
