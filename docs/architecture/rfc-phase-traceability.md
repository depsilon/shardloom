...
| RFC 0024 | Partially implemented | 10D, 12A, 12B, GAR-COMMERCIAL-1A, GAR-COMMERCIAL-1B, Ongoing | Release/API compatibility policy exists; continues as a cross-phase enforcement concern. GAR-COMMERCIAL-1A/B add planned adoption and package-channel readiness follow-through: one-command local proof plus a GitHub pre-release/TestPyPI/PyPI/Homebrew/Scoop/winget/conda-forge/GHCR/crates.io-future matrix that remains blocked without channel-specific install, uninstall, clean-install, smoke, SBOM/checksum/provenance, and rollback/yank evidence. |
