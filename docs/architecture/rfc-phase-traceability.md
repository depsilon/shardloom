...
| RFC 0021 | Partially implemented | Ongoing | Expression/kernel architecture exists in principle. GAR-0021-H adds explicit prepared/native source-backed scan evidence fields, but residual operators remain ShardLoom-native unless a later slice admits an encoded/native provider. Full kernel coverage remains ongoing. |
