...
| RFC 0021 | Partially implemented | GAR-0021-A, GAR-RUNTIME-IMPL-4D-S1, Ongoing | Expression/kernel architecture exists in principle. GAR-0021-A closes the approximate aggregate/sketch admission contract with report-only `approx_count_distinct`/alias surfaces, sketch-state/merge/serialization, encoded-strategy, exact-reference, benchmark, execution-certificate, Native I/O, no-fallback, and claim-boundary fields. GAR-0021-H adds explicit prepared/native source-backed scan evidence fields, but residual operators remain ShardLoom-native unless a later slice admits an encoded/native provider. GAR-RUNTIME-IMPL-4D-S1 adds scoped ShardLoom-native UTF-8 string predicate runtime for `utf8_starts_with` / `utf8_contains` and local SQL `LIKE 'prefix%'` / `LIKE '%contains%'` lowering with deterministic unsupported diagnostics for other LIKE shapes. Full expression, function, and kernel coverage remains ongoing. |
