...
| GAR-NOVEL-1 | Evidence-native generated execution, lineage, observability, and confidence | RFC 0018, RFC 0029, RFC 0031, RFC 0032, RFC 0033, RFC 0035, RFC 0036, RFC 0037, RFC 0039, RFC 0040 | Planned cross-surface lane aligning GeneratedSourceCertificate/source-free generated-output evidence with report-only OpenLineage facets, OpenTelemetry trace mapping, and Bayesian claim-confidence/regression fields. Export and confidence surfaces are opt-in/report-only, no-network/no-backend by default, and cannot upgrade runtime support, production readiness, performance, Spark-replacement, SQL/DataFrame, Foundry, object-store/lakehouse, or package claims without separate scoped evidence and no-fallback/no-external-engine proof. |
