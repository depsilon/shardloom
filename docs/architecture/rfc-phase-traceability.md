...
| RFC 0007 | Partially implemented | 10B-14B, GAR-0007-A, GAR-0007-B | Translation contracts are present. GAR-0007-A adds `shardloom.compatibility_output_writer_matrix.v1` through `translation-plan`, distinguishing the scoped native Vortex reference writer, local fixture-smoke compatibility output writers, Iceberg/Delta table-commit blockers, feature gates, evidence refs, metadata-loss posture, and no-fallback/no-external-engine fields. GAR-0007-B hardens feature-gated traditional analytics smoke evidence for CSV, JSONL, Parquet, Arrow IPC, Avro, and ORC compatibility outputs from Vortex-derived tables. Production sink APIs, object-store output, broad write methods, catalog/table integration, lakehouse commits, and performance claims remain gated. |
