...
| Phase 11A — Spill policy turns real | RFC 0014 Memory Management, Spill, and OOM Safety | RFC 0017 Fault Tolerance, Cancellation, and Recovery; RFC 0008 Object-Store Runtime and Distributed Task Model | memory budgets; memory reservations; spill policies; spill file refs; cleanup expectations; deterministic fail-before-OOM; no fallback execution | Keep spill posture local/native and deterministic; do not add object-store spill during this phase. |
| Phase 11B — Recovery, cancellation, retry | RFC 0017 Fault Tolerance, Cancellation, and Recovery | RFC 0014 Memory Management, Spill, and OOM Safety; RFC 0019 Security, Secrets, Governance, Agent Safety | task attempt identity; retry eligibility; cancellation state; cleanup required; partial output; ambiguous commit; spill cleanup; external-effect retry safety; no fallback execution | Recovery policy must remain explicit, diagnosable, and safe for effectful boundaries. |
| Phase 12A — Native Vortex write intent to staged output | RFC 0005 Vortex-Native File IO and Output Contract | RFC 0004 Native Dataset Manifest, Snapshot, Incremental; RFC 0020 Schema Evolution, Catalog, Table Compatibility; RFC 0024 Release Engineering | Vortex output highest fidelity; write intent; staged output; schema compatibility; metadata preservation; no object-store write unless explicitly in phase; no fallback execution | Preserve native Vortex write fidelity and stage/validate before commit semantics. |
