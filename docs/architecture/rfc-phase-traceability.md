...
| Priority 7.4 - claim-grade compute-engine completion | RFC 0014 Memory Management; RFC 0015 Correctness and Semantics; RFC 0016 Optimizer/Adaptive Execution; RFC 0021 Expression Engine; RFC 0025 Competitive/no-fallback; RFC 0029 Certificates and Benchmarks; RFC 0031 Native I/O; RFC 0033 User Workflow; RFC 0038 Top-Level Plan/Execution Facade; RFC 0040 Benchmark Suite | RFC 0012 Diagnostics; RFC 0018 Observability; RFC 0024 Release Engineering; RFC 0032 Capability Surface; RFC 0041 Feature/Build Matrix; RFC 0042 Vortex Runtime Utilization | `compute-capability-matrix`; operator-family ladder; ShardLoomNative semantic conformance suite; DataFrame/SQL unsupported parity; execution artifact richness; measured source-backed benchmark rows; benchmark taxonomy execution; sink/write/replay proof; first workload-certified compute workflow; local scheduler/runtime; memory/spill operator maturity; Vortex layout/write advisor feedback | P7.4.1 adds a report-only machine-readable compute capability matrix and operator-family ladder for current, planned, unsupported, and future certification states without runtime effects. P7.4.2 adds the first side-effect-free ShardLoomNative semantic fixture suite plus report-only DataFrame/SQL unsupported parity for expression, grouping, aggregate, sort/limit, and SQL parse/bind/plan/execute gaps. P7.4.3 adds artifact-rich top-level execution result preservation across provider reports, inline artifacts, evidence slots, certificate refs, Native I/O refs, provider versions, lifecycle status, source/split refs, representation transitions, and no-fallback policy fields. P7.4.4 begins benchmark/source-backed closeout with fixture-smoke measured rows for all 15 eligible prepared/source-bound/reader-backed encoded matrix rows and expands executable local dataset fixtures across wide, very-wide, null-heavy, many/few file-shape, date-partitioned, clustered, schema-drift, dirty CSV, nested JSON, CDC overlay, skewed, and high-cardinality shapes while keeping claim closeout blocked. P7.4.5 completes the first workload-certified local Vortex analytics workflow by combining opt-in source replay with `--write-result-vortex`: ShardLoom writes the computed result envelope as `result.vortex`, re-opens it, verifies result JSON and materialized rows, emits source/result artifact digests, schema summaries, benchmark/coverage refs, source replay and result-sink Native I/O certificate status, commit/cleanup fields, separated compute/write timing, and CLI/Python invocation. P7.4.6 adds deterministic local task-graph scheduling, bounded queue/backpressure fields, retry/cancellation gates, memory reservation evidence, fail-before-OOM status, operator spill blockers, and certified runtime execution certificate evidence for the same scoped workload. P7.4.7 adds report-only Vortex layout/write advisor feedback from the certified local workflow evidence while keeping layout execution and improvement claims blocked. Remaining P7.4 work is P7.4.4 benchmark closeout: extra-column/multi-file/incremental taxonomy scenarios where feasible, selected local comparative reruns, deterministic unsupported rows for unsupported scenarios, and promotion into claim-grade versus not-claim-grade workload-scoped compute evidence. External engines remain reference/oracle/baseline-only; unsupported paths require stable diagnostics and required future evidence; broad SQL/DataFrame/performance/Spark-displacement/production claims remain blocked until workload constitutions have correctness, benchmark, certificate, Native I/O, materialization, and no-fallback evidence. |
...
`join + aggregate`, `row number window`, `high-cardinality string group/distinct`, and
`top-N per group` through the local Vortex import/replay/result-sink path. The update preserves
Native I/O certificate, runtime execution certificate, materialization/decode-boundary, and
`fallback_attempted=false` evidence. Extra-column, many-file, dirty CSV, nested JSON, CDC, and
partition-pruning support remain blocked until those input contracts are certified.
...
contains workload-scoped correctness, benchmark, execution-certificate, Native I/O,
