...
| --- | --- | --- | --- | --- |
| PERF-01 through PERF-12; conditional PERF-13 | RFC 0044 Resident Runtime and Resource Ownership | RFC 0013 Streaming/Zero-Copy; RFC 0014 Memory/Spill; RFC 0015 Correctness; RFC 0017 Recovery; RFC 0021 Kernels; RFC 0031 Native I/O; RFC 0042 Vortex Runtime | Supplied performance overhaul, active PERF queue, provider boundaries, ownership and generation checks, complete-output measurements, explicit durable versus memory-visible publication; September 8 execution-aware native artifact topology under PERF-03/04/05/07/09/10/12 | Maintainer-adopted 2026-09-05 and extended 2026-09-08. The continuation ledger distinguishes frozen native measurements from prepared/spill/export acceptance. Topology and its dependent local-reduction, local-Top-K and preparation stages are parked after negative measurements; the experimental runtime and unbuilt Python prototype are excluded from the retained-runtime shipping batch. The topology design records separate correctness and retain/drop gates if later resumed. All full PERF packets remain open; no global allocator, whole-family, topology-performance or competitor claim follows from foundation tests. |
...
| GAR-0004-A - CDC/manifest transaction planning gate (complete) | RFC 0004 Native Dataset Manifest, Snapshot, Incremental | RFC 0008 Object-Store Runtime; RFC 0012 Diagnostics; RFC 0017 Recovery; RFC 0020 Schema/Table Compatibility; RFC 0028 Commit/Lakehouse | `CdcManifestTransactionGateReport` exposes CDC read intent, CDC write intent, generalized manifest serialization, manifest metadata reads, object-store commit, table/catalog commit, transaction execution, and unsupported commit diagnostic rows through table-intelligence and CDC CLI output; unsupported rows carry deterministic info diagnostics, required evidence, `claim_gate_status=not_claim_grade`, no I/O, no external engine, and no fallback | Report-only gate; no metadata reads, data reads, manifest serialization execution, object-store commits, table/catalog commits, transaction writes, CDC execution, credentials, or table/lakehouse claims. |
| GAR-0005-B - object-store Vortex I/O and upstream write integration gate (complete) | RFC 0005 Vortex-Native File IO and Output Contract | RFC 0008 Object-Store Runtime; RFC 0012 Diagnostics; RFC 0019 Security/Governance; RFC 0031 Native I/O; RFC 0042 Vortex Runtime Utilization | `VortexObjectStoreIoGateReport` exposes object-store Vortex read/write provider, credential policy, range request budget, write idempotency, upstream sink API, Native I/O certificate, and unsupported diagnostic rows through `vortex-api-inventory` and `object-store-request-plan`; rows carry deterministic info diagnostics, required evidence, `support_status=unsupported`, `claim_gate_status=not_claim_grade`, no provider/network probe, no credential resolution, no object-store I/O, no write I/O, no external engine, and no fallback | Report-only gate; no network I/O, credentials, object-store provider execution, upstream Vortex object-store writes, generalized schema/encoding writes, table/catalog integration, lakehouse output, dependency expansion, or writer/performance claims. |
| GAR-0020-A - table/catalog metadata admission gate (complete) | RFC 0020 Schema Evolution, Catalog Integration, and Table Compatibility | RFC 0004 Native Dataset Manifest; RFC 0008 Object-Store Runtime; RFC 0012 Diagnostics; RFC 0019 Security/Governance; RFC 0041 Feature/Build Matrix | `CatalogMetadataIntegrationGateReport` exposes catalog resolution, snapshot/manifest reads, table metadata reads, partition/delete/CDC metadata reads, table-format dependency admission, commit recovery metadata binding, and metadata cache invalidation through `cg9-catalog-metadata-gate` and `table-intelligence-plan`; blocked rows carry deterministic info diagnostics, required evidence, `support_status=unsupported`, `claim_gate_status=not_claim_grade`, no catalog/object-store/data/write I/O, no credentials, no external engine, and no fallback | Report-only gate; no catalog resolution runtime, metadata reads, data reads, external table-format dependencies, credentials, table/catalog writes, lakehouse runtime, or production table/catalog claims. |
| GAR-0020-C - local manifest-backed table metadata read smoke (complete) | RFC 0020 Schema Evolution, Catalog Integration, and Table Compatibility | RFC 0004 Native Dataset Manifest; RFC 0012 Diagnostics; RFC 0031 Native I/O; RFC 0041 Feature/Build Matrix | `LocalTableMetadataReadSmokeReport` and `local-table-metadata-read-smoke` expose one in-memory local manifest-backed metadata summary from typed catalog, manifest, snapshot, schema, partition, native Vortex file, and segment metadata; the report includes a stable metadata digest, evidence refs, deterministic blocked-path diagnostics, `support_status=runtime_supported`, `claim_gate_status=scoped_local_metadata_smoke_only`, no object-store/data/write/credential I/O, no external engine, and no fallback | Scoped fixture smoke only; no filesystem manifest parser, object-store reads, credentials, data scans, table metadata writes, external table-format dependencies, table/lakehouse runtime, production SQL/DataFrame/catalog claims, or performance claims. |
| GAR-0020-B - delete/CDC/maintenance execution matrix (complete) | RFC 0020 Schema Evolution, Catalog Integration, and Table Compatibility | RFC 0004 Native Dataset Manifest; RFC 0012 Diagnostics; RFC 0016 Layout/Compaction; RFC 0028 Commit/Lakehouse | `TableMaintenanceExecutionMatrixReport` is embedded in `table-intelligence-plan` and exposes row-level support posture for file-level delete compatibility, segment/row/position/equality delete execution, append-only and metadata-only CDC planning, CDC update/delete/tombstone execution, compaction planning/execution, table metadata writes, and table-maintenance commits; report-only rows carry existing report refs, unsupported rows carry deterministic diagnostics, and every row records required fixture, commit semantics, evidence, no-fallback, no-external-engine, and claim-gate fields | Matrix/diagnostic slice only; no delete/tombstone runtime, CDC execution, compaction writes, table metadata writes, object-store I/O, lakehouse/catalog commits, external engine invocation, fallback execution, or production table-format claims. |
...
| CG-12.1 — native-first plan portability report foundation | RFC 0030 Universal API, Plan Portability, Import/Deployment, and External Baselines; RFC 0022 Plan IR and Substrait-Compatible Interoperability | RFC 0010 Developer Experience; RFC 0012 Diagnostics; RFC 0024 Release/API Compatibility | `PlanPortabilityReport` records native-first direction, interop format, native plan schema version, supported/native-only/Substrait-like/lossy/unsupported/residual construct lists, metadata-loss boundaries, redaction requirement, parser/import/export/runtime/probe/read/write side-effect fields, and no fallback execution; `GAR-0022-A` adds `shardloom.substrait_report_only_contract.v1` for Substrait request diagnostics | Establishes CG-12 report-only evidence through `plan-ir`, `plan-import`, and `plan-export` without real serialization, external engine execution, filesystem/network probing, Substrait dependency adoption, or fallback execution. |
...
| CG-21 implementation lanes — user data workflow and ETL surface | RFC 0033 User Data Workflow and ETL Surface | RFC 0032 Capability Surface; RFC 0031 Native I/O; RFC 0030 API/Deployment; RFC 0029 Correctness/Benchmarks/Certificates; RFC 0012 Diagnostics | install/import; context/capability API; source/sink registry; DataFrame/query builder; SQL workflow; pandas/Arrow/NumPy boundaries; data contracts; local structured adapters; output/commit UX; object-store UX; table/catalog UX; observability; benchmark/migration UX; notebooks; UDFs; unstructured/media; governance; workload scorecards | Priority 4 completed local workflow/report lanes for import, context, capability, source/sink planning, query-builder planning, local adapter posture, output/remote blockers, and quickstart proof. GAR-0032-B adds a Python typed method capability matrix for advertised DataFrame/query-builder methods, blockers, evidence requirements, claim boundaries, and no-fallback/no-external-engine posture. Mature DataFrame runtime methods, SQL, joins, aggregations, windows, schema/data-quality runtime, object-store/table runtime, production ETL certification, package publication, external engine fallback, and claims remain blocked unless named in Planned follow-up slices. |
...
GAR-RUNTIME-IMPL-6D front-door benchmark publication closeout: RFC 0009, RFC 0029, RFC 0033,
...
path using an explicit generated delta sidecar imported into a separate Vortex artifact, and the
prepared/native row now scans projected base/delta Vortex columns without full fact-table
materialization. GAR-0021-H adds explicit `source_backed_scan_*` fields to prepared/native rows so
source roles, source refs/digests, projected columns, provider scope, Native I/O certificate status,
materialization boundary rows, residual executor, and no-fallback/no-external-engine evidence are
visible without inferring them from scattered fields. GAR-0026-Q adds explicit
`encoded_predicate_provider_*` blocker fields for the prepared/native `selective filter` row so
Vortex scan filter pushdown is not mistaken for an admitted encoded predicate provider. GAR-0026-R
adds reader-backed bridge diagnostics showing non-empty filtered scans expose projected chunks such
as `metric:vortex.filter` while zero-result scans report no filtered metric reader chunks.
GAR-0026-S adds the reader-generated conjunctive selection-vector bridge for supplied encoded
kernel inputs. GAR-0026-T adds a scoped filter-column probe that observes real `flag,value` reader
chunks without decode/materialization. GAR-0026-U lowers the observed `flag:fastlanes.bitpacked`
and `value:vortex.sequence` chunks into admitted ShardLoom kernel inputs and intersects their
selection vectors. GAR-0026-V consumes that admitted bridge selection vector for scoped
prepared/native selective-filter metric `row_count` and `metric_sum` evidence while keeping metric
aggregation residual-native and encoded-native claims false. Broader
incremental-state execution, table/catalog/object-store partition-pruning, object-store multi-file,
general source API runtime, general JSON execution, and encoded predicate provider claims remain
separately blocked.
...
GAR-FLOW-2H completion update: `traditional-analytics-vortex-batch-run` now computes one
fact/dimension/CDC Vortex source metadata snapshot per batch invocation and reuses artifact
size/digest evidence across child prepared/native scenario reports. The emitted
`source_metadata_snapshot_*` fields make this runtime-plumbing reuse explicit while preserving
per-scenario typed envelopes, Native I/O certificates, result-sink evidence, and
`fallback_attempted=false` / `external_engine_invoked=false`; it is not a performance or
encoded-native claim.

GAR-FLOW-2I completion update: `traditional-analytics-vortex-batch-run` now reuses one
dimension-label lookup state across hash-join and join-aggregate child scenarios when both are
present. The emitted `source_state_reuse_*`, `source_state_prepare_micros`, and
`source_state_prepare_timing_scope=batch_shared_session_open_only_deferred_family_build_reported_separately`
fields keep shared session-open setup visible while deferred family-build timing is reported
separately. This preserves child scenario envelopes and no-fallback/no-external-engine evidence and
remains scoped source-state runtime plumbing, not a performance, encoded-native, SQL/DataFrame,
object-store, lakehouse, production, or Spark-displacement claim.

GAR-FLOW-2J completion update: `traditional-analytics-vortex-batch-run` now reuses one
`category,metric` grouped source state across distinct-count and high-cardinality
string-group/distinct child scenarios when both are present. The emitted aggregate
`source_state_reuse_*` fields plus family-specific `source_state_category_metric_*` fields keep
shared setup visible while preserving child scenario envelopes and no-fallback/no-external-engine
evidence. This is scoped source-state runtime plumbing, not a performance, encoded-native,
SQL/DataFrame, object-store, lakehouse, production, or Spark-displacement claim.

GAR-FLOW-2K completion update: `traditional-analytics-vortex-batch-run` now reuses one
`group_key,id,metric` ranked source state across sort/top-k, top-N per group, and row-number/window
child scenarios when multiple ranked consumers are present. The emitted aggregate
`source_state_reuse_*` fields plus family-specific `source_state_ranked_metric_*` fields keep shared
setup visible while preserving child scenario envelopes and no-fallback/no-external-engine evidence.
This is scoped residual source-state runtime plumbing, not a distributed sort, performance,
encoded-native, SQL/DataFrame, object-store, lakehouse, production, or Spark-displacement claim.

GAR-FLOW-2L completion update: `traditional-analytics-vortex-batch-run` now reuses one
`group_key,category,metric` grouped source state across group-by aggregation and multi-key group-by
child scenarios when both are present. The emitted aggregate `source_state_reuse_*` fields plus
family-specific `source_state_group_category_metric_*` fields keep shared setup visible while
preserving child scenario envelopes and no-fallback/no-external-engine evidence. This is scoped
residual source-state runtime plumbing, not a performance, encoded-native, SQL/DataFrame,
object-store, lakehouse, production, or Spark-displacement claim.

GAR-FLOW-2M completion update: `traditional-analytics-vortex-batch-run` now reuses one
`raw_event_time,dirty_numeric,dirty_flag` dirty-input cleanup source state across
clean/cast/filter/write and malformed timestamp / dirty CSV child scenarios when both are present.
The emitted aggregate `source_state_reuse_*` fields plus family-specific
`source_state_dirty_input_*` fields keep shared dirty-column parsing and timestamp validation setup
visible while preserving child scenario envelopes and no-fallback/no-external-engine evidence. This
is scoped residual source-state runtime plumbing, not a performance, encoded-native,
SQL/DataFrame, object-store, lakehouse, production, or Spark-displacement claim.

GAR-FLOW-2N completion update: `traditional-analytics-vortex-batch-run` now reuses one
filtered `id,value,metric` source state across selective-filter and filter/projection/limit child
scenarios when both are present. The emitted aggregate `source_state_reuse_*` fields plus
family-specific `source_state_selective_filter_*` fields keep shared predicate/filter setup visible
while preserving child scenario envelopes and no-fallback/no-external-engine evidence. Selective
rows retain scoped `encoded_predicate_provider_*` evidence, but shared-state metric aggregation is
reported as `batch_source_state_metric_aggregation_used` and remains residual-native. This is
scoped residual source-state runtime plumbing, not a performance, encoded-native, SQL/DataFrame,
object-store, lakehouse, production, or Spark-displacement claim.
...
| Priority 3.5 / CG-14 / GAR-0014-A - memory runtime hardening and spill/OOM promotion gate | RFC 0014 Memory Management, Spill, and OOM Safety | RFC 0016 Optimizer/Adaptive Execution; RFC 0017 Fault Tolerance/Recovery; RFC 0018 Observability; RFC 0024 Release Engineering; RFC 0025 Competitive/no-fallback; RFC 0029 Certificates; RFC 0043 Security/Path Safety | `MemoryRuntimeHardeningGateReport`; `gar_id=GAR-0014-A`; existing memory admission, pre-OOM memory guard fixture, operator spill declarations, spill reservation, spill lifecycle, and dynamic runtime-promotion report refs; required reservation-release, pressure-reaction, native-spill-read/write, spill-cleanup, allocator, production fail-before-OOM, execution-certificate, Native I/O, benchmark, no-fallback, and path-safety evidence refs | Report-only gate plus bounded local `pre-oom-memory-guard-smoke` fixture; no allocator runtime, resource-derived chunk sizing runtime, adaptive parallelism runtime, broad reservation release runtime, pressure reaction runtime, native spill IO, cleanup execution, object-store IO, data reads, writes, claim publication, external engine invocation, or fallback execution is authorized by this row. |
...
| --- | --- | --- | --- |
| RFC 0001 | Partially implemented | 0-3, Ongoing | Foundational architecture and no-fallback direction established; GAR-0001A-A adds a report-only SQL/DataFrame planner-readiness matrix through CLI capability fields and Python capability accessors, with deterministic diagnostics, `claim_gate_status=not_claim_grade`, no parser/binder/planner/runtime execution, no external engine, and no fallback. GAR-0001A-B adds `global-architecture-gate`, a release/readiness runtime-claim gate for distributed, object-store, and lakehouse claims with deterministic blockers, required evidence refs, `claim_gate_status=not_claim_grade`, no credentials, no I/O, no runtime execution, no external engine, and no fallback. GAR-0001B-A adds `shardloom.engine_replacement_claim_inventory.v1` and `docs/architecture/engine-replacement-claim-inventory.md`, mapping replacement claim families to required runtime, output, correctness, benchmark, certificate, Native I/O, and no-fallback evidence while keeping `claim_gate_status=not_claim_grade`, no replacement claim, no benchmark rerun, no runtime execution, no external engine, and no fallback. Executable SQL/DataFrame runtime, distributed runtime, object-store execution, lakehouse output, and engine-replacement claims remain gated by later evidence. |
| RFC 0002 | Partially implemented | 2-6, Ongoing | Core contract framing in place; implementation depth still increases by phase. |
| RFC 0003 | Partially implemented | 3-10C, Ongoing | Planning/runtime skeletons exist. GAR-0003-A adds `vortex_segment_extraction_admission_ref` and `shardloom.vortex_segment_extraction_admission.v1` through `vortex-api-inventory`, admitting local non-null sparse patch/fill reader chunks into ShardLoom run-length encoded kernel inputs with correctness/no-fallback evidence while keeping nullable/generalized sparse segment extraction gated. GAR-0003-B adds `shardloom.materialization_policy.v1`, Python typed accessors, and benchmark `materialization_policy_ref` coverage so encoded-native, residual-native, materialized-temporary, and unsupported paths expose decode/materialization posture and claim boundaries. Broad production Vortex segment extraction and wider operator coverage remain phased. |
| RFC 0004 | Partially implemented | 12A, 12B, 13A, 13B, GAR-0004-A | Manifest/snapshot/incremental model present conceptually; CG-9.5 CDC incremental planning and CG-9.7 compaction planning evidence exist for declared metadata; GAR-0004-A adds a report-only CDC/manifest transaction gate with deterministic unsupported diagnostics for write, manifest, metadata-read, commit, and transaction lanes; advanced write/commit, CDC execution, manifest serialization execution, metadata reads, and maintenance execution behavior remain planned. |
| RFC 0005 | Partially implemented | 12A, GAR-0005-A, GAR-0005-B, PROD-READY-1A, Ongoing | Vortex-native output contract is established. GAR-0005-A adds `shardloom.vortex_local_io_coverage.v1` through `vortex-api-inventory`, classifying the scoped local primitive scan reader lane, feature-gated flat scalar/columnar Vortex ingest writer lanes, the feature-gated native CountAll output payload writer lane, generalized local writer blockers, claim boundaries, and no-fallback/no-external-engine fields. `shardloom-cli/vortex-write` now directly enables the existing feature-gated native count-payload writer and scoped local Vortex ingest paths. PROD-READY-1A adds `shardloom.vortex_native_writer_schema_certification.v1`, separating scoped flat scalar, typed complex source-free, flat columnar, nullable flat columnar, flat Arrow Decimal128 columnar, flat Arrow dictionary utf8/binary columnar, and flat Arrow dictionary integer/finite-float columnar local writer runtime evidence from boolean/decimal/interleave-preserving dictionary layout and JSON/WKB extension provider candidates, plus the blocked generalized schema/encoding writer, while recording provider decisions, DType/validity/encoding scopes, metadata/statistics preservation posture, materialization boundaries, replay evidence, deterministic unsupported diagnostics, and no-fallback fields. GAR-0005-B adds `shardloom.vortex_object_store_io_gate.v1` through `vortex-api-inventory` and `object-store-request-plan`, explicitly blocking object-store Vortex providers, credentials, range budgets, write idempotency, upstream sink APIs, and Native I/O certificates until evidence exists. Full generalized schema/encoding payload writing, table/catalog, object-store runtime execution, lakehouse, SQL/DataFrame, and performance claims remain gated. |
...
| RFC 0007 | Partially implemented | 10B-14B, GAR-0007-A, GAR-0007-B | Translation contracts are present. GAR-0007-A adds `shardloom.compatibility_output_writer_matrix.v1` through `translation-plan`, distinguishing the local native Vortex reference writer, admitted local compatibility-export writers, Iceberg/Delta table-commit blockers, feature gates, evidence refs, metadata-loss posture, and no-fallback/no-external-engine fields. GAR-0007-B hardens Vortex-derived writer evidence for CSV, JSONL, Parquet, Arrow IPC, Avro, and ORC compatibility outputs. Object-store output, catalog/table integration, lakehouse commits, broad nested/complex sink shapes, production claims, and performance claims remain gated. |
| RFC 0008 | Partially implemented | 11A, 14A, 14A.3, 14B, GAR-0008-A, GAR-0008-B, GAR-0012-B | CG-10.1 object-store range planning, CG-10.2 request coalescing, CG-10.3 commit protocol planning, CG-10.4 distributed scheduling planning, and CG-10.5 checkpoint/retry/idempotency planning evidence exist for declared byte ranges, commit signals, task shapes, and reliability evidence. GAR-0008-A adds `shardloom.object_store_byte_range_provider_gate.v1` through `object-store-request-plan` and `cg10-object-store-runtime-gate`, explicitly blocking byte-range provider runtime until provider capability policy, credential-effect policy, request-budget policy, retry policy, idempotency-key contract, execution certificate, Native I/O certificate, and benchmark evidence exist. GAR-0008-B adds `shardloom.object_store_runtime_blocker_matrix.v1` for coordinator start, worker start, task execution, checkpoint writes, retry attempts, cleanup execution, and commit-record writes, with `SL_OBJECT_STORE_UNSUPPORTED`, row-specific blockers, no I/O, no fallback, and no external engine invocation. GAR-0012-B propagates those object-store/distributed blocker rows through info-level JSON envelope diagnostics, text summary fields, and Python typed result access while keeping the gate report-only. Object-store IO, provider probes, credential resolution, retry execution, checkpoint writes, distributed runtime, and object-store commit execution remain planned. |
| RFC 0009 | Partially implemented | 2-10C, Ongoing | Core policy scaffolding exists; deeper behavior and tooling continue to mature. GAR-0009-A adds `shardloom.spark_displacement_benchmark_evidence_matrix.v1` and `docs/architecture/spark-displacement-benchmark-evidence-matrix.md`, mapping Spark-displacement benchmark workloads to baseline/oracle lanes, correctness refs, timing refs, environment refs, execution-mode refs, missing evidence, and `claim_gate_status=not_claim_grade` while keeping external engines baseline-only with no benchmark rerun, no public performance claim, no Spark-displacement claim, no external engine invocation, and no fallback. |
| RFC 0010 | Partially implemented | 10C, 10D, Priority 3.6, GAR-0010-A, Ongoing | Developer/agent usability direction is represented by CLI/Python/agent protocol surfaces, `RfcCoverageFollowThroughReport`, and the Python `CapabilityPosture` view added by GAR-0010-A; every new CLI, Python, future REST, capability, diagnostic, benchmark, and certificate surface must remain deterministic, machine-readable, human-readable, side-effect-explicit, and safe for import/discovery/dry-run workflows before execution/write permissions. |
| RFC 0011 | Partially implemented | Priority 3.6, GAR-0011-A, Ongoing (post-core), CG-21P | Modular extensibility is accepted as the boundary contract for SQL, UDFs, unstructured/media, LLM/API/model effects, embeddings, vector operations, and agent-facing extension discovery. `RfcCoverageFollowThroughReport` records typed/effect/materialization metadata and sandbox/governance/correctness/certificate prerequisites. GAR-0011-A adds `shardloom.extension_manifest_effect_capability_matrix.v1` and `docs/architecture/extension-manifest-effect-capability-matrix.md` so extension manifest families, required permissions, sandbox posture, effect metadata, materialization boundaries, deterministic blockers, `claim_gate_status=not_claim_grade`, `fallback_attempted=false`, and `external_engine_invoked=false` are emitted through extension, UDF, and security/governance capability surfaces. Runtime extension loading, UDF execution, API/LLM/embedding/vector/model effects, connector/object-store/catalog extension runtime, dependency expansion, external effects, external engine invocation, and fallback execution remain deferred and must preserve explicit effect/materialization/cost/redaction/certificate/no-fallback boundaries. |
...
| RFC 0014 | Partially implemented | 10B, 11A, 11B, 13B, 14A, CG-14, GAR-0014-A | Memory/spill/OOM policies are partially scaffolded; CG-14.1 adaptive memory boundary evidence, runtime memory reservation admission evidence, bounded pre-OOM guard fixture evidence, operator memory/spill declaration gate evidence, and the CG-14/GAR-0014-A memory runtime hardening gate exist with explicit evidence refs and path-safety refs. Allocator runtime, resource-derived runtime chunk sizing, adaptive parallelism runtime, broad reservation release runtime, pressure reaction runtime, native spill read/write, spill cleanup execution, and large-workload claim publication remain planned and evidence-gated. |
| RFC 0015 | Partially implemented | GAR-0015-A, Ongoing | Correctness-first posture present; deeper differential/fuzz coverage continues over time. GAR-0015-A adds the `property-string-utf8-predicate-consistency` generated property fixture, `string_utf8_predicate_consistency` fuzz seed target, and `string-semantics` deferred fixture-family artifact slot so string predicate correctness has an explicit report-only gap with no query execution, no decoded-reference execution, no external engine invocation, no fallback, and no benchmark/performance claim. |
...
| RFC 0017 | Partially implemented | 11A, 11B, 12B, 14A, 14B, GAR-0017-A | Recovery, cancellation, retry, commit, checkpoint, and cleanup planning surfaces exist. GAR-0017-A adds an explicit report-only execution-gate split for request validation, cancellation signals, retry allowance, checkpoint writes, cleanup execution, and commit execution; broad retry/cancellation/checkpoint/cleanup/commit/recovery runtime remains blocked behind provider, credential, commit-semantics, execution-certificate, Native I/O, and no-fallback evidence. |
| RFC 0018 | Partially implemented | 10D, 14A, 14B, GAR-0018-A, GAR-NOVEL-1C, Ongoing | Observability foundations exist. GAR-0018-A adds a report-only runtime-introspection artifact that exposes the local benchmark stage-timing schema while keeping live profiling, profile artifacts, debug bundles, metrics exporters, trace backends, and distributed runtime introspection blocked with no runtime collection, no external engine invocation, and no fallback. GAR-NOVEL-1C adds `shardloom.opentelemetry_trace_export_contract.v1` as the report-only OpenTelemetry trace/span mapping lane; no SDK/exporter/backend/network integration, trace emission, metric emission, log emission, or telemetry claim is authorized without a later opt-in evidence slice. |
| RFC 0019 | Partially implemented | 11B, 13A, GAR-0019-A, GAR-0019-B, Ongoing | Security/governance guardrails exist. GAR-0019-A adds `shardloom.credential_policy_enforcement_gate.v1` and `docs/architecture/credential-policy-enforcement-gate.md` so credential reference inventory, secret loading, environment/file/external-manager/cloud-IAM providers, workspace policy, runtime permission checks, redaction policy, deterministic unsupported diagnostics, `claim_gate_status=not_claim_grade`, `credential_resolution_performed=false`, `secret_loading_performed=false`, `fallback_attempted=false`, and `external_engine_invoked=false` are visible through `security-governance-evidence-gate` and `capabilities security-governance`. GAR-0019-B adds `shardloom.sandbox_governance_readiness_gate.v1` and `docs/architecture/sandbox-governance-runtime-readiness.md` so sandbox profile inventory, filesystem/network/environment/secret/process permissions, resource limits, timeout enforcement, audit log, dependency isolation, deterministic unsupported diagnostics, `sandbox_runtime_supported=false`, `extension_code_executed=false`, `udf_code_executed=false`, `fallback_attempted=false`, and `external_engine_invoked=false` are visible through security/governance surfaces. Credential resolution, secret loading, production policy enforcement, sandbox execution, plugin/UDF execution, external effects, external engine invocation, and fallback execution remain blocked until later evidence-bearing runtime slices. |
| RFC 0020 | Partially implemented | 12A, 13A, 13B, GAR-0020-A, GAR-0020-B, GAR-0020-C, Priority 3.6 | CG-9.1 schema-evolution, CG-9.2 partition-evolution, CG-9.3 delete/tombstone, CG-9.4 aggregate table compatibility, CG-9.5 CDC incremental planning, CG-9.6 layout-health dependency, CG-9.7 compaction planning dependency evidence, `RfcCoverageFollowThroughReport`, GAR-0020-A deterministic catalog/table metadata admission diagnostics, GAR-0020-B delete/CDC/maintenance execution matrix diagnostics, and GAR-0020-C scoped local manifest-backed table metadata read smoke exist for typed transitions and table/catalog promotion gates; broader catalog/table metadata integration, delete/tombstone execution, CDC execution, compaction execution, layout/compaction integration, update/delete/merge claims, and write/commit certification remain planned. |
| RFC 0021 | Partially implemented | GAR-0021-A, GAR-RUNTIME-IMPL-4D-S1, GAR-RUNTIME-IMPL-4D-S2, GAR-RUNTIME-IMPL-4D-S3, GAR-RUNTIME-IMPL-4D-S4, GAR-RUNTIME-IMPL-4D-S5, GAR-RUNTIME-IMPL-4D-S6, GAR-RUNTIME-IMPL-4D/5G closeout, GAR-RUNTIME-IMPL-4D-F1, GAR-RUNTIME-IMPL-4D-F2, GAR-RUNTIME-IMPL-4D-F3, Ongoing follow-ups | Expression/kernel architecture exists in principle. GAR-0021-A closes the approximate aggregate/sketch admission contract with report-only `approx_count_distinct`/alias surfaces, sketch-state/merge/serialization, encoded-strategy, exact-reference, benchmark, execution-certificate, Native I/O, no-fallback, and claim-boundary fields. GAR-0021-H adds explicit prepared/native source-backed scan evidence fields, but residual operators remain ShardLoom-native unless a later slice admits an encoded/native provider. GAR-RUNTIME-IMPL-4D-S1 adds scoped ShardLoom-native UTF-8 string predicate runtime for `utf8_starts_with` / `utf8_contains` / `utf8_ends_with` and local SQL `LIKE 'prefix%'` / `LIKE '%contains%'` / `LIKE '%suffix'` lowering with deterministic unsupported diagnostics for other LIKE shapes. GAR-RUNTIME-IMPL-4D-S2 adds scoped ISO Date32 parsing/formatting, `date_year` / `date_month` / `date_day`, UTF-8/Date32 casts, local SQL `DATE 'YYYY-MM-DD'` predicates, CSV ISO date inference, and deterministic invalid-date/non-Date32 blockers. GAR-RUNTIME-IMPL-4D-S3 adds scoped local SQL logical `AND` predicate runtime over admitted leaves; GAR-RUNTIME-IMPL-4D-S4 adds scoped logical `OR` over admitted leaves while preserving `AND` precedence inside `OR` branches; GAR-RUNTIME-IMPL-4D-S5 adds scoped logical `NOT` over admitted leaves while preserving `NOT`/`AND`/`OR` precedence for unparenthesized scoped predicates; GAR-RUNTIME-IMPL-4D-S6 adds balanced grouping parentheses over admitted predicate leaves and emits deterministic blockers for unbalanced predicate groups. The 4D/5G closeout adds bytewise binary equality, promotes UTC timestamp/Date32/binary semantic conformance rows, and expands the admitted semantics matrix to release-gate string composition, temporal arithmetic/difference, CASE, IN/subquery, distinct aggregate, hidden HAVING aggregate, window, and join fixtures. GAR-RUNTIME-IMPL-4D-F1 closes advanced scalar residuals by adding executed conformance fixtures and matrix unsupported rows for decimal precision/scale casts, non-UTC timestamp literals, timezone database conversion, interval arithmetic outside scoped temporal helpers, and locale-aware collation; scoped ANSI interval literals inside temporal helpers and scoped UTF-8 regex predicates are now promoted separately through ShardLoom-owned lowering, and later GAR-RUNTIME-IMPL-6D evidence promotes scoped `decimal128(p,s)` / `decimal(p,s)` / `numeric(p,s)` casts through exact fixed-scale projection/predicate runtime, exact exponent normalization, scoped mixed-scale decimal add/subtract/multiply, mixed-scale comparison, and exact fixed-scale division runtime while keeping non-exact division, broad SQL-standard coercion beyond exact exponent notation, decimal/float comparison, and typed sinks blocked. GAR-RUNTIME-IMPL-4D-F2 originally closed complex dtype residuals as deterministic blockers for list/array construction/access/equality, struct/row construction/equality/access, variant access, SQL UNION/union dtype semantics, parent/child null behavior, schema field identity, and binary SQL source/runtime decoding while preserving admitted bytewise binary scalar equality; later GAR-RUNTIME-IMPL-6D promotes scoped `ARRAY[...]` literal and `STRUCT(<source column>, ...)` projections through the JSONL/result boundary and CSV JSON-text output, then adds scoped `SELECT DISTINCT` and `UNION DISTINCT` structural equality over those already-materialized projection values, followed by scoped canonical structural `ORDER BY` over those complex result-boundary values. GAR-RUNTIME-IMPL-4D-F3 closes the advanced predicate/subquery residual by executing bounded local scalar IN subqueries with optional admitted WHERE/ORDER BY/LIMIT tails, executing HAVING IN-subquery predicates over aggregate output rows, and adding deterministic no-fallback blockers for multi-column scalar-left, nested, joined, grouped/HAVING-internal, correlated, EXISTS, and quantified subquery shapes that were not yet admitted. GAR-RUNTIME-IMPL-6D promotes scoped UTF-8 `LIKE` `%` / `_` wildcard predicates, row-value literal `IN`/`NOT IN`, bounded row-value local-source `IN (SELECT ...)` / `NOT IN (SELECT ...)`, nested scalar IN-subqueries, joined and grouped/HAVING projected scalar/row-value IN/NOT IN, EXISTS/NOT EXISTS, and quantified subqueries, scoped top-level SQL `UNION` / `UNION ALL` composition, scoped local-source `EXISTS` / `NOT EXISTS`, scoped quantified `ANY` / `ALL`, scoped source-qualified local subquery references, scoped correlated `outer.<column>` scalar/row-value/EXISTS/NOT EXISTS/quantified subquery filters and correlated projected negative-subquery shapes, scoped UTF-8 regex predicates, scoped ANSI interval literals inside temporal helper functions, scoped decimal exponent notation that normalizes exactly to the target scale, scoped JSONL/result-boundary and CSV JSON-text `ARRAY[...]` and `STRUCT(<source column>, ...)` projections, scoped structural DISTINCT/UNION DISTINCT equality for those complex result rows, scoped feature-gated Arrow list/large-list/fixed-size-list and struct source decoding through JSONL and CSV JSON-text result boundaries, and scoped typed nested compatibility sink preservation through Parquet, Arrow IPC, Avro, and scoped local Vortex when a stable nested Arrow dtype can be inferred from non-null nested values or carried from source child-schema evidence, while keeping scalar-left multi-column, unbound qualified or broad correlated subquery shapes, locale/collation, arbitrary interval arithmetic, non-exact decimal division, broad SQL-standard decimal coercion beyond exact exponent notation, decimal/float comparison, complex accessors/casts/subquery membership/broad nested ordering, all-null typed nested sink columns without child-schema evidence, ORC nested output, all-null typed nested Vortex output without child-schema evidence, and union dtype casts blocked separately. Broad encoded-kernel/operator coverage remains the explicit follow-up runtime item rather than a hidden parent gap. |
| RFC 0022 | Partially implemented | Priority 3.6, GAR-0022-A, Ongoing (interop track) | Native plan serialization/import/export, imported-plan capability gates, `RfcCoverageFollowThroughReport`, and the `shardloom.substrait_report_only_contract.v1` Substrait report-only import/export contract exist as dependency-free safety surfaces. Imported-plan execution, real Substrait/non-native format parsers/exporters, Substrait dependency adoption, external-engine bridging, and fallback execution remain blocked. |
| RFC 0023 | Partially implemented | Priority 3.6, GAR-0023-A, Ongoing (extension track) | Extension manifests, inspection reports, sandbox-policy vocabulary, and `RfcCoverageFollowThroughReport` exist as inspection-only surfaces. GAR-0023-A adds `shardloom.plugin_abi_udf_sandbox_blocker.v1` and `docs/architecture/plugin-abi-udf-sandbox-blocker.md` so plugin ABI inventory, dynamic library loading, Rust/WASM/Python/SQL/external/table-function UDFs, plugin lifecycle transitions, sandbox evidence binding, license/provenance attestation, and unsupported diagnostics are emitted through extension, UDF, and security/governance capability surfaces with `claim_gate_status=not_claim_grade`, `abi_loading_supported=false`, `dynamic_loading_performed=false`, `extension_code_executed=false`, `udf_execution_performed=false`, `fallback_attempted=false`, and `external_engine_invoked=false`. Plugin/UDF execution, unsafe Python/external extension behavior, extension code loading, signing enforcement, resource-limit enforcement, dependency expansion, external effects, external engine invocation, and fallback execution remain blocked. |
| RFC 0024 | Partially implemented | 10D, 12A, 12B, GAR-COMMERCIAL-1A, GAR-COMMERCIAL-1B, GAR-0024-A, Ongoing | Release/API compatibility policy exists; continues as a cross-phase enforcement concern. GAR-COMMERCIAL-1A completes the one-command local technical-preview proof path: source artifact build, exact local wheel install in a clean virtual environment, installed-wheel CLI/Python smoke, scoped generated-source local JSONL/CSV output smokes, compatibility/prepared-Vortex benchmark smoke, provenance/SBOM/checksum dry run, and transcript fields that keep public package release, tags, secrets, fallback dependencies, and production/performance claims false. GAR-COMMERCIAL-1B completes the machine-readable package-channel readiness matrix and validator: GitHub pre-release, TestPyPI, PyPI, Homebrew tap, Scoop, winget, conda-forge, GHCR container, and future crates.io public API crates are all represented with install/uninstall commands, proof requirements, authorization/provenance posture, rollback/yank policies, blockers, and claim boundaries. GAR-0024-A adds `shardloom.publication_api_schema_stability_gate.v1`, a fail-closed release-plan and hard-release-readiness gate for API compatibility windows, schema compatibility windows, package identities, signing policy, checksum manifest, SBOM bundle, and publication approval. v0.1.8 GitHub/TestPyPI/PyPI/Homebrew publication is proof-backed for selected-channel package access, but no production API/schema stability claim, broad public package claim, external engine invocation, or fallback execution is authorized by package availability. Future package channels remain blocked without channel-specific install, uninstall, clean-install, smoke, SBOM/checksum/provenance, authorization, and rollback/yank/delete/deprecate evidence; PyPI/TestPyPI require Trusted Publisher/OIDC, internal Rust crates remain unpublished, and package access does not imply production readiness. |
| RFC 0025 | Planned | CG-1 through CG-23, GAR-0001B-A, GAR-0025-A, Ongoing | Competitive Engine Track policy is documented; CG-21 is the user data workflow and ETL surface, CG-22 is the three-engine certified data execution fabric, and CG-23 is the REST, event, and remote API surface; implementation remains gate-specific and evidence-gated. GAR-0001B-A adds the engine-replacement claim inventory so competitive, Spark-displacement, best-default, production SQL/DataFrame, object-store/lakehouse, managed-platform, and performance/superiority language remains blocked until per-claim evidence is attached. GAR-0025-A adds `shardloom.competitive_replacement_sufficiency_gate.v1`, requiring correctness, benchmark, Native I/O, execution-certificate, capability-coverage, no-fallback, and release/publication evidence before any replacement claim can pass. No replacement claim, Spark-displacement claim, superiority claim, production-platform claim, runtime expansion, benchmark rerun, external engine invocation, or fallback execution is authorized. |
| RFC 0026 | Partially implemented | CG-1, CG-2, CG-13 | Encoded-read and query-primitive readiness contracts exist; CG-13.1 encoded path selection evidence exists for count/filter/project candidates; GAR-0026-A adds a scoped prepared/native `filter + projection + limit` residual-native Vortex scan path with filter/projection pushdown and bounded top-N state; GAR-0026-B adds a scoped prepared/native `group by aggregation` residual-native Vortex scan path with projection pushdown and grouped residual state; GAR-0026-C extends that pattern to scoped prepared/native `multi-key group by` with composite-key residual state; GAR-0026-D adds scoped prepared/native `hash join` with projected dimension/fact scans and bounded residual join state; GAR-0026-E adds scoped prepared/native `join + aggregate` with projected dimension/fact scans, fact-side filter pushdown, and bounded residual grouped aggregation; GAR-0026-F adds scoped prepared/native `top-N per group` with projected fact scans and bounded residual per-group ranking state; GAR-0026-G adds scoped prepared/native `row number window` with projected fact scans and bounded residual rank-1 state; GAR-0026-H adds scoped prepared/native `high-cardinality string group/distinct` with projected fact scans and residual string grouping state; GAR-0026-I adds scoped prepared/native `partition pruning` with projected `event_date`/`metric` scans, local date-range filter pushdown, and residual scalar aggregation; GAR-0026-J adds scoped prepared/native `sort and top-k` with projected `id`/`metric` scans and bounded residual global top-k state; GAR-0026-K adds scoped prepared/native `distinct count` with projected `category` scans and residual distinct state; GAR-0026-L adds scoped prepared/native `null-heavy aggregate` with projected `nullable_metric_00` scans and residual null-skipping aggregate state; GAR-0026-M adds scoped prepared/native `clean/cast/filter/write` with projected `raw_event_time`/`dirty_numeric`/`dirty_flag` scans and residual cleanup/filter/aggregate state; GAR-0026-N adds scoped prepared/native `malformed timestamp / dirty CSV` with projected `raw_event_time`/`dirty_numeric` scans and residual validation/parse/aggregate state; GAR-0026-O adds scoped prepared/native `nested JSON field scan` with projected `nested_payload` scans and residual generated-field extraction state; GAR-0026-P adds scoped prepared/native `small change over large base` with projected base `id`/`metric` scans, projected CDC delta `id`/`op`/`value`/`metric`/`effective_ts` scans, and residual overlay state; GAR-0021-H adds reusable `source_backed_scan_*` evidence fields over those prepared/native scan boundaries; GAR-0026-Q adds `encoded_predicate_provider_*` blocker fields for the prepared/native `selective filter` row, keeping Vortex scan filter pushdown distinct from an admitted encoded predicate provider; GAR-0026-R adds reader-backed bridge diagnostics showing projected chunks such as `metric:vortex.filter` on non-empty filtered scans and no filtered metric chunks on zero-result scans; GAR-0026-S adds the reader-generated conjunctive selection-vector bridge for supplied encoded kernel inputs; GAR-0026-T adds real `flag,value` filter-column probe evidence; GAR-0026-U lowers observed `flag:fastlanes.bitpacked` and `value:vortex.sequence` chunks into admitted kernel inputs and intersects their selection vectors; GAR-0026-V consumes the admitted bridge selection vector for scoped selective-filter metric `row_count` and `metric_sum` evidence while keeping metric aggregation residual-native and encoded-native claims false; real generalized encoded execution and production compressed-execution claims remain gated. |
...
| RFC 0028 | Partially implemented | CG-3, CG-4, CG-9, CG-10, GAR-0028-A | Output/commit readiness contracts exist; first native count-result payload path is complete; first local committed-manifest execution path is complete; local committed-manifest recovery diagnostics, first local rollback cleanup path, and scoped local sidecar recovery replay proof are complete; `GAR-0028-A` expands the commit promotion gate with deterministic blockers for generalized manifest serialization, generalized sink commit, object-store commit, table/catalog commit, lakehouse transaction commit, Foundry dataset transaction commit, upstream Vortex write API execution, live/hybrid checkpoint commit, and output-payload fidelity claims. Broad payload, generalized recovery, table/catalog, lakehouse, object-store, Foundry, and upstream Vortex write runtime execution remains blocked and not claim-grade. |
| RFC 0029 | Partially implemented | CG-5, CG-6, CG-16, CG-17, GAR-0029-A, GAR-NOVEL-1D | CG-16.1 local encoded count certificate, CG-16.2 execution-certificate evidence surface, CG-17.1 stateful reuse boundary report, and the CG-17 promotion gate for stable reuse keys/invalidation/manifest-diff/state-certificate/reuse-benchmark blockers exist; broader correctness, benchmark, certificate, cache read/write/replay, and incremental execution evidence remain future gate work. GAR-0029-A adds `shardloom.cg5_cg6_stateful_reuse_evidence_expansion.v1` to `correctness-harness-plan`, `benchmark-claim-evidence-plan`, and `stateful-reuse-plan`, tying CG-5 correctness, CG-6 benchmark, CG-16 execution-certificate linkage, CG-19 Native I/O linkage, CG-17 boundary/invalidation evidence, reuse benchmark constitution, and public claim attachment into one fail-closed blocker report. No benchmark rerun, cache read/write/replay, incremental execution, stateful reuse runtime, performance claim, superiority claim, external engine invocation, or fallback execution is authorized. GAR-NOVEL-1D adds `shardloom.traditional_analytics.bayesian_claim_confidence.v1` as a report-only/not-fit advisory Bayesian claim-confidence schema with posterior distribution, credible interval, regression probability, minimum-run, benchmark population, release policy, no-fallback, and no-external-engine fields; it can only block future release/performance claims after a fitted model and release gate exist and cannot upgrade claim status alone. |
| RFC 0030 | Partially implemented | CG-11, CG-12, CG-18, GAR-0030-A, GAR-COMMERCIAL-1A, GAR-COMMERCIAL-1B | CG-11.1 stable CLI/API JSON protocol foundation, CG-11.2 thin Python wrapper foundation, and CG-11.3/CG-11.4 source-tree Python client surfaces exist through `CliApiJsonProtocolReport`, `PythonWrapperFoundationReport`, `api-compat-plan`, `python-wrapper-plan`, and the zero-dependency `python/` client; CG-12.1 plan portability report foundation, CG-12.4 native plan serialization, and CG-12.5 imported-plan capability gate exist through `PlanPortabilityReport`, `NativePlanDocument`, `ImportedPlanCapabilityGateReport`, `plan-ir`, `plan-import native`, and `plan-export native`; CG-18.1 plus the Priority 3.5 CG-18 harness maturity surface exist through `UniversalHarnessReport`, `universal-harness-plan`, and `docs/architecture/universal-import-deployment-baseline-harness.md`; GAR-0030-A adds explicit `universal_harness_execution_gate_status`, execution allowed/attempted flags, and required/attached/missing evidence refs so harness execution remains blocked until capability, execution-certificate, Native I/O, policy/no-fallback, output, correctness, and benchmark evidence is attached; GAR-COMMERCIAL-1A packages local build/install, installed-wheel smoke, generated-source local output smokes, prepared/native tiny benchmark evidence, provenance dry run, and evidence inspection into one user-facing flow; GAR-COMMERCIAL-1B adds package-channel readiness metadata and release-gate validation for external distribution channels while keeping all channels blocked until proof exists. Imported-plan execution, non-native format parsers/exporters, actual harness execution, package/container publication, Foundry invocation, external baseline runner execution, and comparison dataset materialization remain staged. |
| RFC 0031 | Partially implemented | CG-19, GAR-RUNTIME-IMPL-4F-S1 | CG-19.1 native I/O envelope report exists through `NativeIoEnvelopeReport` and `native-io-envelope-plan`; GAR-0031A adds a source/sink coverage matrix for local Vortex, compatibility import, object-store/range-read, table/catalog, streaming, unstructured/media, and external-adapter families with support status, certificate refs, deterministic unsupported diagnostics, blockers, future evidence, no-fallback fields, and claim boundaries. GAR-GEN-1A/1B now expose generated-source/output certificate semantics through `shardloom.generated_source_certificate_contract.v1`: no source Native I/O certificate is claimed when `input_dataset_count=0`, no-dataset smoke remains non-generated-output, and output Native I/O evidence remains required for generated output. GAR-RUNTIME-IMPL-4F-S1 adds scoped flat JSONL/NDJSON local input runtime to `sql-local-source-smoke` with SourceState-style id/digest/reuse posture, schema/content fingerprints, deterministic nested-value blockers, and no fallback/external engine execution. GAR-RUNTIME-IMPL-4F now also includes feature-gated flat scalar local Parquet admission through the same SQL local-source smoke while default builds fail closed. Source/sink runtime certificate emission, broad adapter runtime, broader Parquet type/nesting support, Arrow/Avro/ORC reads, object-store I/O, table/catalog paths, and fallback remain absent outside existing certified local lanes. |
| RFC 0032 | Partially implemented | CG-20, GAR-0032-A, GAR-0032-C, GAR-0032-D, GAR-0032-E | CG-20.1 world-class sufficiency reporting exists through `WorldClassSufficiencyReport` and `world-class-sufficiency-plan`; CG-20.2 user-surface capability discovery exposes report-only `capabilities` scopes for ETL, Python, DataFrame/notebook, UDF, universal/event/API adapters, unstructured/media, API, observability, deployment, extension, and security/governance dimensions; GAR-0001A-A adds a SQL/DataFrame planner-readiness matrix to `capabilities sql`, `capabilities dataframe`, and Python `CapabilityView`; GAR-0032-A documents SQL parser/binder readiness and strengthens `workflow-unsupported-plan sql-parse|sql-bind|sql-plan|sql-execute --format json` with unsupported support status, not-claim-grade gate status, and explicit parser/binder/planner not-executed fields; GAR-0032-C adds `shardloom.external_effect_blocker_matrix.v1` to effect-budget and capability views so UDF, API, LLM, embedding, vector, plugin, media, and network-effect rows stay blocked with policy-required diagnostics, denied-by-default effect status, no runtime execution, no external engine invocation, and no fallback; GAR-0032-D adds `shardloom.unstructured_adapter_capability_matrix.v1` to unstructured-media, universal-adapter, event/API/SaaS, and API-surface capability views so document/media/vector/adapter/source-sink metadata rows remain report-only or blocked with no runtime execution, no source/sink I/O, no model/network side effect, no external engine invocation, and no fallback; GAR-0032-E adds the `shardloom.best_default_certification_gate.v1` best-default certification gate to world-class sufficiency and certification capability views, explicitly requiring workload constitution, correctness, benchmark, execution-certificate, Native I/O, materialization/decode, no-fallback policy, release/security, UX/install, capability snapshot, scorecard, and dossier evidence while keeping best-default language blocked and `claim_gate_status=not_claim_grade`; GAR-0032-B adds an exported Python DataFrame/query-builder method capability matrix over advertised source declarations, lazy filter/select/limit/group-by declarations, unsupported diagnostics, required evidence, claim boundaries, and no-fallback/no-external-engine posture; GAR-GEN-1A/1B add source-free generated-output contract fields to Python, SQL, DataFrame, universal-adapter, and API capability scopes for `no_dataset_smoke`, `user_generated_source`, and `engine_native_generated_source`; GAR-GEN-1C/1D add scoped local JSONL user-row and range generated-output smokes; GAR-GEN-1E adds `shardloom.generated_source_api_admission.v1` for Python, SQL, DataFrame, and API source-free admission rows without executing broad SQL/DataFrame runtime; CG-20.3 adds Python live ETL smoke helpers; CG-20.7 adds a feature-gated local compatibility-file-to-Vortex bridge for CSV, JSONL, Parquet, Arrow IPC, Avro, and ORC; CG-20.11 adds local Conda recipe scaffolds for the CLI/Python/metapackage split; P7.4.2 adds the first ShardLoomNative semantic conformance fixture suite. Real SQL parser/binder execution, operators, functions, production adapters, full semantic conformance coverage, migration analyzers, DataFrame/notebook runtime beyond scoped generated-output smokes, UDF runtime, production ETL, universal-adapter runtime/certification, unstructured/media runtime, Conda build/install publication, correctness, benchmark, and best-default certification evidence remain staged. |
| RFC 0033 | Partially implemented | CG-21, GAR-COMMERCIAL-1C, GAR-COMMERCIAL-1F | Defines the user data workflow and ETL surface: install/import, capability discovery, local structured ETL, DataFrame/query-builder, SQL, pandas/Arrow boundaries, data contracts, transformations, joins, aggregations, windows, incremental ETL, writes, object stores, table/catalog UX, remote inputs, logs/events, unstructured/media references, UDFs, observability, migration, benchmarks, governance, notebooks, deployment, adapter maturity, lane sequencing, MVP scope, unsupported diagnostics, and certification disqualifiers. Priority 4 implements local workflow/report and quickstart proof surfaces. P7/Audit-F9 adds report-only unsupported diagnostics and Python helper parity for `from_pandas`, Arrow table/IPC input and output boundaries, NumPy/Python-object materialization, schema discovery/description/validation, data-quality summary/quarantine, and notebook preview/display. P7.4.2 extends parity to `with_column`, `group_by`, `agg`, `sort`, `limit`, and SQL parse/bind/plan/execute diagnostics. GAR-0032-B exposes a typed DataFrame method matrix so Python users and agents can inspect method support, blockers, evidence requirements, and claim boundaries without executing, reading, materializing, writing, or invoking external engines. GAR-0033-A adds `shardloom.etl_workflow_capability_matrix.v1` so CLI/Python users can inspect local ready/smoke workflow rows, report-only SQL/DataFrame/data-quality posture, and blocked object-store/table/production ETL rows with no-fallback/no-external-engine fields and `claim_gate_status=not_claim_grade`. GAR-GEN-1 is the planned source-free generated-output lane that separates no-dataset smoke from user-generated rows and ShardLoom-native generator nodes. GAR-COMMERCIAL-1C adds a generated buyer-facing website/status matrix so users can inspect support, blockers, planned work, and not-planned claims without reading the phase plan; GAR-COMMERCIAL-1F adds `shardloom.workflow_recipe_library.v1` and validated copyable workflow recipes for local smokes, generated-output smokes, prepared/native direction, messy-data fixtures, result-sink proof, blocked object-store diagnostics, Foundry-style local proof, and benchmark interpretation. Mature DataFrame/SQL/pandas/Arrow/write/object-store/table runtime and production ETL certification remain blocked or report-only until later evidence-bearing slices. |
...
| RFC 0035 | Partially implemented | CG-23, GAR-NOVEL-1B, GAR-NOVEL-1C, GAR-COMMERCIAL-1D | Defines the REST, event, and remote API surface: REST control plane, data plane, event plane, capability/discovery endpoints, plan/explain/dry-run endpoints, async query lifecycle, result delivery policies, problem+json errors, engine-aware request behavior, live/hybrid event APIs, API maturity ladder, OpenAPI/AsyncAPI/CloudEvents/OpenTelemetry/OpenLineage/Flight/ADBC/MCP references, security/governance policy, certification blockers, and no-fallback boundaries. Priority 6 implements contract/report-only lanes through API-A9 columnar data-plane standards, while API-A2 remains no-listener discovery reporting and API-A10 production-certified workload API remains blocked until workload-scoped evidence exists. GAR-NOVEL-1B adds `shardloom.openlineage_facet_mapping.v1` as a report-only observability capability mapping execution mode, no-fallback, Native I/O, materialization, claim gate, generated-source, and Vortex artifact evidence into future ShardLoom-owned OpenLineage custom facet placeholders; GAR-NOVEL-1C adds `shardloom.opentelemetry_trace_export_contract.v1` as a report-only observability capability mapping request admission, source read, compatibility parse, Vortex import/scan, operator compute, result sink, evidence render, and claim gate timing/evidence fields into future OpenTelemetry internal span placeholders; GAR-COMMERCIAL-1D adds `shardloom.enterprise_evidence_export_pack.v1` to package ShardLoom JSON, OpenLineage facet payload previews, OpenTelemetry span/metric payload previews, optional Markdown summary, and redaction report into a report-only opt-in local evidence pack. No lineage event emission, schema publication, telemetry exporter, trace/metric/log emission, OTLP/collector/backend configuration, HTTP listener, dependency expansion, external engine invocation, or fallback execution is authorized by those rows alone. |
| RFC 0036 | Partially implemented | Priority 8, Priority 9, GAR-COMMERCIAL-1E, GAR-0036-A | Defines the optional Foundry Integration Pack and availability surface: Conda-first/PyPI-friendly/GitHub-release-backed distribution, provenance artifacts, external proof examples, shardloom-foundry helper package, Foundry maturity ladder, datasets, transactions, branches, incremental runs, Data Health, lineage, schedules, Data Connection, S3-compatible dataset access, virtual tables, external compute boundary reports, Iceberg posture, media sets, `FoundryMediaSetSource`, `FoundryVirtualMediaSetSource`, `FoundryMediaSetSink`, `FoundryMediaExtractionBoundaryReport`, `FoundryModelCallBoundaryReport`, `FoundryEmbeddingBoundaryReport`, `FoundryAipLogicBoundaryReport`, `FoundryUnstructuredWorkflowCertificate`, Ontology/AIP/Functions/model/scenario surfaces, BYOC, Compute Modules, Marketplace starter products, governance, and Foundry benchmark schemas. Priority 9 now adds local Foundry integration-pack readiness docs and `shardloom.foundry_proof_of_use_report.v1` for source-checkout proof. GAR-COMMERCIAL-1E adds `shardloom.foundry_dev_stack_starter_kit.v1`, exact local proof commands, staged-input posture, generated-output/no-dataset separation, local certificate-style output, and explicit blockers for real Foundry output API evidence while keeping Foundry runtime/compute/Spark/output APIs disabled. GAR-0036-A adds `shardloom.foundry_package_proof_boundary_matrix.v1` and `docs/foundry/package-proof-boundary-matrix.md` so package publication, Artifact Repository publication, service invocation, Compute Modules, virtual tables, dataset transactions, and F10 deployment are explicit blocked/report-only rows with no Foundry runtime, compute, Spark, fallback, external engine, or public Foundry claim. Full Foundry package/platform installation, invocation, external compute execution, virtual-table native execution, dependency expansion, fallback execution, and platform claims remain unauthorized until real Foundry evidence exists. |
| RFC 0037 | Accepted | Priority 3.8, GAR-0010-A, GAR-0037-A | Defines the client/wrapper/SDK/ecosystem integration surface: one canonical protocol, transport adapters, client core, language SDKs, Python ecosystem wrappers, workflow wrappers, MCP posture, Flight/ADBC/JDBC/ODBC/BI posture, wrapper maturity W0-W7, `WrapperCapabilityReport`, `ProtocolSurfaceParityReport`, golden contract fixtures, and wrapper no-fallback invariants. GAR-0010-A adds Python typed capability posture accessors over the existing CLI JSON envelope without creating a separate execution path. GAR-0037-A adds `shardloom.wrapper_connector_implementation_registry.v1` through `capabilities api-surfaces --format json` and Python `ctx.wrapper_connector_registry()` so current local Python wrapper rows, report-only generated clients, and blocked DB-API/SQLAlchemy/Ibis/orchestration/MCP/Flight/ADBC/JDBC/ODBC/BI/Grafana/Foundry rows remain explicit. No generated client, API server, data-plane bridge, dependency, external engine invocation, or fallback execution is authorized by the RFC alone. |
| RFC 0038 | Partially implemented | Priority 1.7; GAR-0038-A | Top-level plan variants, `ShardLoomExecutionResult`, provider-neutral blocked dispatch, `ShardLoomExecutionProvider`, and Vortex provider-side dispatch now exist for local primitive and prepared/source-backed/reader-backed encoded report surfaces. P7.4.3 preserves provider reports as artifact-rich execution results with provider version, lifecycle, inline artifact, evidence-slot, certificate, Native I/O, materialization, residual-boundary, representation-transition, source/split, and no-fallback fields. GAR-0038-A adds a typed `facade_compatibility_matrix` artifact plus Python view accessors that classify executable provider-dispatched paths, report-only paths, unsupported SQL/DataFrame/object-store/write runtimes, removed/unsupported legacy placeholders, and prohibited external-engine fallback. SQL/DataFrame runtime, object-store runtime, writes, external engine invocation, fallback execution, and executable legacy facade shims remain unauthorized. |
| RFC 0039 | Partially implemented | Priority 3.9, GAR-0039-A, GAR-0039-B | Defines the typed command/result/evidence envelope and CLI modularity overhaul. Current implementation adds `shardloom.output.v2`, typed result/result_refs/artifacts/artifact_refs/certificates/policy/lifecycle/capability_snapshot slots, API/Python protocol reporting, Python typed-payload parsing, shared CLI routing for common policy/lifecycle/capability fields, conservative typed ref routing for explicit result/artifact/certificate refs, the shared `typed_envelope` and `cli_output` CLI modules, `command_family` lifecycle taxonomy, the status/capabilities, REST/API planning, packaging/deployment, benchmark planning, benchmark runtime including `vortex-count-benchmark`, operational hardening/security, diagnostics, evidence/certificate planning, workflow/table planning, engine/runtime planning, extension/UDF planning, prepared/source-backed encoded-read, and first Vortex primitive execution and planning handler modules, including `vortex-count-where`, `vortex-project`, `vortex-filter`, `vortex-filter-project`, `vortex-run`, `vortex-local-exec`, `vortex-bounded-local-exec`, `vortex-query-trace`, `vortex-metadata-plan`, `vortex-pruning-plan`, `vortex-metadata-probe`, and `vortex-api-inventory`, and typed-envelope contract snapshots while retaining `fields` as a temporary legacy mirror. P7.4.3 adds artifact-rich top-level execution result envelopes and Python result views for current provider evidence. GAR-0039-A migrates the API-surface capability family further into typed payloads by attaching an inline `api_surface_capability_report` artifact for the wrapper/connector registry, adding typed `capability_snapshot` fields for registry counts and claim boundaries, and making Python `OutputEnvelope.field_map` prefer typed payload fields before the legacy mirror. GAR-0039-B centralizes typed-envelope integration-test helpers and adds an inline `universal_harness_report` artifact for the Foundry optional-harness boundary. Remaining command-family result migration, remaining golden fixtures, and additional physical handler splits remain planned. No REST server, wrapper ecosystem implementation, benchmark execution, runtime expansion, extension dynamic loading, UDF execution, external service invocation, external engine invocation, or fallback execution is authorized by the RFC alone. |
| RFC 0040 | Partially implemented | Priority 2.7, GAR-0040-A | Defines the benchmark-suite and platform-learning hardening lane: local-first benchmark suite catalog, scenario taxonomy, dataset profiles, benchmark constitutions, plugin-based local engines, coverage tables, and neutral capability reports inspired by Photon/Fabric/Snowflake lessons. Current implementation adds `BenchmarkSuiteCatalogReport`, `BenchmarkConstitutionRequirementReport`, local optional engine plugin contracts, coverage rows, the source-backed matrix report surface, fixture-smoke measured source-backed matrix rows, the machine-readable scenario catalog, taxonomy/constitution metadata in the executable local analytics runner, opt-in taxonomy-extra local scenarios, generated dataset profile metadata including wide/very-wide/null-heavy/date-partitioned/clustered shapes, and support/coverage table output separate from timings. GAR-0040-A adds `shardloom.comparative_rerun_managed_platform_gate.v1` to benchmark and release claim surfaces, separating local full comparative reruns, external baseline/oracle rows, managed-platform design-reference rows, credential policy, claim-grade artifact publication, and no-fallback/external-execution boundaries while keeping reruns, credential resolution, network probes, managed dependencies, managed-platform execution, public performance claims, external engine invocation, and fallback execution blocked. Managed platforms remain design references only; full comparative benchmark reruns, source-backed claim-grade promotion, performance claims, managed-platform benchmark lanes, credentials, new managed dependencies, external engine fallback, and fallback execution remain unauthorized. |
| RFC 0041 | Partially implemented | Priority 2.8, Priority 3.5, GAR-0041-A | Defines the workspace feature/build validation matrix and crate-level posture cleanup: default/all/no-default feature checks, key Vortex/package/benchmark/Foundry feature combinations, feature-disabled unsupported diagnostics, build evidence fields, and crate docs/export posture for executable, report-only, blocked, future, and prohibited-fallback surfaces. Current implementation completes the crate-posture docs/export cleanup, adds `WorkspaceFeatureBuildMatrixReport` plus matrix documentation, and adds `shardloom.per_claim_evidence_attachment_matrix.v1` to release-plan and hard release-readiness surfaces so public release, package, performance/superiority, Spark-displacement, engine-replacement, production SQL/DataFrame, object-store/lakehouse, and Foundry/platform claims each name required test, benchmark, certificate, Native I/O, security, provenance, unsupported-path, no-fallback, and approval evidence. Public claims remain blocked with `per_claim_evidence_attachment_matrix_claim_gate_status=not_claim_grade` until every row has attached passing evidence and human approval. No package publication, dependency expansion, runtime expansion, object-store execution, external engine invocation, or fallback execution is authorized by the RFC alone. |
...
| RFC 0043 | Partially implemented | P8.0, P8.4, GAR-0043-A, GAR-0043-B, Priority 9 | Defines security, vulnerability, exploit, and supply-chain hardening for release and platform readiness: SEC-0 through SEC-9, malicious input and workspace safety, evidence artifact safety, dependency/advisory gates, SBOM/checksum/provenance/attestation requirements, vulnerability disclosure, compromised package response, Scorecard/CodeQL/Dependabot posture, and no-fallback security blockers. Current implementation completes P8.0 by adding the RFC, threat model, expanded security policy, supply-chain response doc, dependency audit release-gate report, report-level runtime input/workspace/evidence artifact safety contracts, local SBOM/checksum/provenance dry-run generation, PyPI workflow policy snapshots, CodeQL, Scorecard, Dependabot, open-source security posture docs, known unsupported paths, release security gate aggregation, and contract tests. P8.4 adds `scripts/check_release_readiness.py` and `shardloom.hard_release_readiness_gate.v1`. GAR-0043-A adds `scripts/check_release_architecture_tracker.py`, `docs/release/release-architecture-tracker-gate.md`, `shardloom.release_architecture_tracker_report.v1`, and hard release-readiness integration so unchecked Global Architecture Review/phased-plan items, RFC traceability, known unsupported paths, release security refs, provenance refs, and per-claim evidence refs fail closed with `architecture_tracker_status=blocked`, `claim_gate_status=not_claim_grade`, `public_release_claim_allowed=false`, `public_package_claim_allowed=false`, `fallback_attempted=false`, and `external_engine_invoked=false`. GAR-0043-B adds `scripts/final_release_rehearsal.py`, `docs/release/final-release-rehearsal.md`, `shardloom.final_release_rehearsal_report.v1`, and `shardloom.local_publication_attestation_plan.v1` for a local no-publication rehearsal that records package artifact, checksum, SBOM, provenance, security, architecture-tracker, package-channel, unsupported-path, per-claim, and publication/API/schema refs while keeping `rehearsal_status=passed`, `claim_gate_status=not_claim_grade`, `publication_authorization_status=approved_channel_proof_passed`, `publication_human_approved=true`, `public_release_claim_allowed=false`, `public_package_claim_allowed=false`, `signing_key_used=false`, `publication_attempted=false`, `tag_created=false`, `fallback_attempted=false`, and `external_engine_invoked=false`. v0.1.8 GitHub/TestPyPI/PyPI/Homebrew package access is proof-backed; production claims, future package-channel claims, signing, uploaded attestations, external engine invocation, and fallback execution remain unauthorized. |
...
| GAR-0010-A | Python API ergonomics and typed capability view | RFC 0010, RFC 0037, RFC 0039 | `CapabilityPosture` and `CapabilityView` accessors expose support status, claim-gate status, runtime/effect/no-fallback/external-engine booleans, blockers, required evidence, gates, and materialization boundaries from existing capability `OutputEnvelope` fields; Python tests lock no-scraping access without changing runtime behavior, package publication, DataFrame/SQL execution, external engines, or fallback execution. |
| GAR-0010-B | DataFrame/notebook and package surface readiness report | RFC 0010, RFC 0024, RFC 0032 | `shardloom.dataframe_notebook_package_readiness.v1` exposes local Python package metadata, editable-install smoke, DataFrame method matrix, notebook display blockers, public package publication blockers, and deterministic unsupported diagnostics through `capabilities python`, `capabilities dataframe`, `capabilities notebook`, `capabilities deployment`, `capabilities api-surfaces`, Python `CapabilityView.dataframe_notebook_package_readiness`, and `ctx.dataframe_notebook_package_readiness()`; the report distinguishes local package smoke from runtime/publication support and preserves `fallback_attempted=false`, `external_engine_invoked=false`, and `claim_gate_status=not_claim_grade` without adding package publication, broad DataFrame runtime, notebook runtime, external engines, or fallback execution. |
| GAR-0032-B | Python DataFrame method capability matrix | RFC 0032, RFC 0037, RFC 0033 | `DataFrameMethodCapability` and `DataFrameMethodCapabilityMatrix` expose advertised Python DataFrame/query-builder method support, blockers, required evidence, materialization boundaries, claim gates, and no-fallback/no-external-engine posture over the existing typed capability view; Python tests lock report-only behavior without adding DataFrame execution, dataset reads, materialization, writes, SQL runtime, external engines, or fallback execution. |
| GAR-0033-A | ETL workflow and data-quality capability matrix | RFC 0033, RFC 0032, RFC 0037 | `shardloom.etl_workflow_capability_matrix.v1` exposes local ready/smoke workflow rows, report-only SQL/DataFrame and data-quality API posture, and blocked object-store, table/lakehouse, and production ETL certification rows through `capabilities workflow --format json`, Python `ctx.etl_workflow_matrix()`, and `docs/architecture/etl-workflow-capability-matrix.md`; the matrix preserves `fallback_attempted=false`, `external_engine_invoked=false`, and `claim_gate_status=not_claim_grade` without adding runtime behavior, production ETL claims, package publication, external engines, or fallback execution. |
| GAR-0034-A | Live/hybrid fabric blocker and freshness gate | RFC 0034, RFC 0013, RFC 0014, RFC 0017, RFC 0035 | `shardloom.live_hybrid_fabric_freshness_gate.v1` exposes blocked broker, durable checkpoint/state-store, unbounded scheduler, hybrid object-store commit, hybrid table/catalog snapshot, and exactly-once rows plus fixture-smoke freshness and report-only baseline/oracle rows through `engine-capability-matrix --format json`, `capabilities engines --format json`, Python `ctx.engine_capability_matrix()`, and `docs/architecture/live-hybrid-fabric-freshness-gate.md`; the gate preserves `fallback_attempted=false`, `external_engine_invoked=false`, and `claim_gate_status=not_claim_grade` without adding broker/state-store/object-store/table runtime, production live/hybrid freshness, exactly-once, performance, Spark-displacement, external baseline runtime, or fallback execution. |
| GAR-0035-A | REST server/runtime unsupported contract | RFC 0035, RFC 0039, RFC 0037, RFC 0034 | `shardloom.rest_api_runtime_unsupported_contract.v1` exposes blocked HTTP listener, remote execution, Flight/ADBC transport, external broker, and dependency-expanded server rows plus report-only OpenAPI discovery, plan-preview, and result-delivery rows through `rest-api-contract-plan --format json`, Python `ctx.rest_api_contract_plan()`, and `docs/architecture/rest-server-runtime-unsupported-contract.md`; the gate preserves `server_started=false`, `network_listener_opened=false`, `fallback_attempted=false`, `external_engine_invoked=false`, and `claim_gate_status=not_claim_grade` without adding REST server runtime, remote execution, transport runtime, broker integration, package publication, production API, performance, Spark-displacement, external engine, or fallback execution. |
| GAR-0037-A | Wrapper and connector implementation registry | RFC 0037, RFC 0030, RFC 0032, RFC 0035, RFC 0036, RFC 0039 | `shardloom.wrapper_connector_implementation_registry.v1` exposes ready-local Python CLI JSON and typed capability/helper rows, report-only generated client/report viewer/Foundry wrapper rows, and blocked DB-API, SQLAlchemy, Ibis, dbt, Airflow, Dagster, Prefect, MCP, Flight SQL, ADBC, JDBC/ODBC, BI, and Grafana rows through `capabilities api-surfaces --format json`, Python `ctx.capabilities().wrapper_connector_registry`, Python `ctx.wrapper_connector_registry()`, and `docs/architecture/wrapper-connector-implementation-registry.md`; the registry preserves `dependency_expansion_allowed=false`, `network_listener_started=false`, `data_plane_bridge_supported=false`, `fallback_attempted=false`, `external_engine_invoked=false`, and `claim_gate_status=not_claim_grade` without adding generated clients, connectors, REST server runtime, Flight/ADBC transport, Foundry package, package publication, production API, production SQL/DataFrame support, performance, Spark-displacement, external engine, or fallback execution. |
| GAR-COMPAT-1 | Universal compatibility completion matrix | RFC 0007, RFC 0008, RFC 0020, RFC 0028, RFC 0030, RFC 0031, RFC 0032, RFC 0033, RFC 0035, RFC 0036, RFC 0037 | Completed compatibility coverage lane. GAR-COMPAT-1A adds `shardloom.universal_compatibility_coverage_scoreboard.v1`, CLI `capabilities compatibility --format json`, Python `ctx.compatibility_scoreboard()`, website/status rendering, and release-readiness checks for CSV, JSONL/JSON, Parquet, Arrow IPC, Avro, ORC, Excel, SQLite, Postgres/MySQL, JDBC/ODBC, S3/GCS/ADLS, Iceberg/Delta/Hudi, Vortex, generated/source-free outputs, Python rows/DataFrame, SQL VALUES/literals, REST/Flight/ADBC, and Foundry. GAR-COMPAT-1B adds `shardloom.universal_compatibility.generated_output_contract.v1` with compatibility/status rows for no-dataset smoke, scoped Python `ctx.from_rows(...).write(...)`, `ctx.literal_table(...).write(...)`, `ctx.calendar(...).write(...)`, `ctx.range(...).write(...)`, `ctx.sequence(...).write(...)`, `ctx.sql_values(...).write(...)`, `ctx.sql_literal_select(...).write(...)`, `ctx.sql("SELECT * FROM generate_series/range(...)").write(...)`, scoped range-projection `ctx.sql("SELECT value AS id, value + 1 AS next FROM range(...)").write(...)`, scoped `ctx.dataframe_source_free_projection("lit(...).alias('name')").write(...)`, generated-row literal `with_column`, and generated-range int64 `with_column` local JSONL/CSV smokes, plus local-output-only generated-source posture. GAR-COMPAT-1C adds `shardloom.universal_compatibility.object_store_admission_ladder.v1` with S3/GCS/ADLS rows for URI parse, credential policy, public no-credential read, authenticated read, byte-range read, full-file read, local cache, write staging, and commit protocol, preserving `credential_resolution_performed=false`, `network_probe_allowed=false`, `provider_probe_allowed=false`, `object_store_io=false`, `write_io=false`, `fallback_attempted=false`, and `external_engine_invoked=false`. GAR-COMPAT-1D adds `shardloom.universal_compatibility.table_format_boundary_matrix.v1` with Iceberg/Delta/Hudi rows for metadata read, table scan, snapshot/time-travel, partition evolution, delete/tombstone, append, merge/update/delete, commit, rollback, catalog interaction, and object-store coupling, preserving `catalog_io_allowed=false`, `object_store_io_allowed=false`, `table_metadata_read_allowed=false`, `table_data_read_allowed=false`, `write_io_allowed=false`, `commit_allowed=false`, `rollback_allowed=false`, `fallback_attempted=false`, and `external_engine_invoked=false`. GAR-COMPAT-1E adds `shardloom.universal_compatibility.database_warehouse_boundary_matrix.v1` with SQLite, Postgres, MySQL, JDBC/ODBC, Snowflake, BigQuery, and Databricks SQL rows for import/export/query-pushdown posture. SQLite is the only local fixture exception through `sqlite-local-import-export-smoke`; the matrix still preserves `credential_resolution_performed=false`, `network_probe_performed=false`, `driver_loaded=false`, `query_pushdown_supported=false`, `fallback_attempted=false`, and `external_engine_invoked=false` while network database/warehouse rows remain blocked. Rows distinguish runtime-supported, smoke-supported, report-only, blocked, and not-planned posture with blocker IDs, required future evidence, claim boundaries, `fallback_attempted=false`, and `external_engine_invoked=false`; no source Native I/O certificate is claimed when no source dataset is read and output Native I/O evidence remains required for output claims. Future slices must keep the scoreboard as a capability map and cannot promote object-store, table/lakehouse, database/warehouse, broad SQL/DataFrame, Foundry, package, performance, production, or Spark-replacement claims without scoped evidence and no-fallback/no-external-engine proof. |
| GAR-GEN-1 | Source-free generated-output planning and `GeneratedSourceCertificate` | RFC 0031, RFC 0032, RFC 0033, RFC 0036, RFC 0037, RFC 0039 | Completed scoped source-free generated-output lane. GAR-GEN-1A/1B add `shardloom.generated_source_certificate_contract.v1`, CLI/Python capability fields, and Python typed accessors that distinguish `no_dataset_smoke`, `user_generated_source`, and `engine_native_generated_source`; scoped `generated-source-user-rows`, `generated-source-range`, `generated-source-sequence`, `generated-source-sql`, Python `ctx.from_rows([...]).write(...)`, `ctx.from_rows([...]).with_column(literal).write(...)`, `ctx.literal_table([...]).write(...)`, `ctx.calendar(...).write(...)`, `ctx.range(...).write(...)`, `ctx.range(...).with_column(int64_expression).write(...)`, `ctx.sequence(...).write(...)`, `ctx.sql_values(...).write(...)`, `ctx.sql_literal_select(...).write(...)`, `ctx.sql("SELECT * FROM generate_series/range(...)").write(...)`, scoped range-projection `ctx.sql("SELECT value AS id, value + 1 AS next FROM range(...)").write(...)`, scoped `ctx.dataframe_source_free_projection("lit(...).alias('name')").write(...)`, and scoped `ctx.dataframe_generated_with_column("name", "lit(...)").write(...)` now support local JSONL/CSV output with generated-source certificate status, output Native I/O certificate status, materialization/decode boundary fields, sink-artifact evidence, and no-fallback/no-external-engine evidence. GAR-GEN-1E adds `shardloom.generated_source_api_admission.v1` and Python typed accessors for `python_ctx_from_rows`, `python_ctx_range`, `python_ctx_sequence`, `python_ctx_literal_table`, `python_ctx_calendar`, `python_generated_source_write`, SQL literal `SELECT`, SQL `VALUES`, scoped SQL source-free range projection, SQL `generate_series`/`range`, scoped DataFrame literal projection, and scoped generated `with_column` posture. Current rows keep no source Native I/O certificate when no source dataset was read and no-dataset smoke as `smoke_only`. Broader expression-backed SQL/DataFrame runtime, engine-native values/synthetic generators, object-store/lakehouse output, Foundry output, production/package claims, and performance claims remain blocked by their owning broad rows rather than by the scoped generated-output lane. |
| GAR-PERF-1 | End-to-end prepared/native performance architecture | RFC 0026, RFC 0029, RFC 0040, RFC 0041, RFC 0042 | Completed prepared/native performance architecture lane covering a post-source-state-reuse benchmark refresh, complete source-state reuse family coverage, fused filter/project/limit plus selection-vector follow-through, and `shardloom.traditional_analytics.bayesian_advisor.v1` report-only confidence/uncertainty fields for future mode/reuse/sizing/layout advisor decisions. Benchmark rows remain local pre-release evidence and cannot authorize performance, superiority, Spark-displacement, production, SQL/DataFrame, object-store/lakehouse, or encoded-native claims without workload-scoped evidence and claim gates. |
| GAR-PERF-2 | Evidence-level runtime tiering, evidence-aware optimizer, scan pushdown completion, encoded kernel registry, fused operator pipelines, session runtime, allocation/buffer reuse, optimized build profiles/PGO, and native microbenchmark suite | RFC 0016, RFC 0022, RFC 0026, RFC 0029, RFC 0039, RFC 0040, RFC 0041, RFC 0042 | Partially completed performance-runtime evidence lane. Completed slices cover the `minimal_runtime`/`certified`/`full_replay` evidence-level contract, report-only evidence-aware logical optimizer rule registry and trace rows, Vortex Scan API filter/projection/limit pushdown completion across prepared/native scenario families, scoped compressed/encoded kernel registry rows for selective-filter bitpacked, sequence, and constant inputs plus dictionary group-by over real Vortex reader chunks, local non-null sparse patch/fill reader chunks lowered into encoded run-length inputs, scoped FSST string-contains helpers using Vortex native LikeKernel, shared masked UTF-8 helpers using Vortex 0.75 `Mask::all_true`/`all_false`/`true_count` metadata, shared local-open layout-reader cache evidence through Vortex 0.75 `with_layout_reader_cache()`, current-runtime drop decisions for Vortex 0.75 grouped kernels on flat-column routes and `byte_length()` on dictionary-derived length/transform routes, and deterministic blockers for sorted/min-max, nullable/generalized sparse, TurboQuant/vector, and broader operators, scoped fused local prepared/native residual-pipeline evidence for filter/projection/limit, filter/aggregate, and top-k/projection plus deterministic filter/group-by blockers, in-process `ShardLoomSession` runtime over scoped prepared/native local artifacts, allocation profiling/scoped buffer-pool blocker evidence for result buffers, temporary vectors, hash tables, dictionary/string state, and source-state arrays, optimized `release-lto`/`release-pgo`/`release-native-benchmark` build-profile evidence with portable-release and target-CPU-native boundaries, and first-class native microbenchmark rows for Vortex scan-only, filter predicate-only, projection-only, group-by kernel, hash-join kernel, top-k, result-sink write, and evidence-render primitives with deterministic blockers for missing isolated primitives. The lane separates evidence level, optimizer trace, scan pushdown, kernel admission, fusion/materialization avoidance, proof overhead, session/cache/buffer reuse, allocation/resource evidence, build-profile/PGO/native-codegen evidence, and subsystem/kernel evidence from execution mode and end-to-end benchmark claims, keeps `fallback_attempted=false` and `external_engine_invoked=false` visible in every row, treats `minimal_runtime` as `not_claim_grade` unless a future scoped gate explicitly approves promotion, forbids daemon/service/remote-server claims, hidden optimizer fallback, hidden global pools, unsafe lifetime shortcuts, hidden release `RUSTFLAGS`, and `target-cpu=native` portable release artifacts, keeps optimizer rule admission, scan pushdown, kernel registry admission, fused residual pipelines, buffer-reuse evidence, and optimized build profiles independent from encoded-native operator or performance claims, requires before/after plan digests and evidence preservation before any optimizer rewrite promotion, requires end-to-end encoded evidence before `encoded_native_claim_allowed=true`, requires stronger independent fused/unfused runtime certificates before claim-grade fusion promotion, requires buffer reuse to preserve correctness and evidence parity, and requires PGO/native benchmark lanes to record reproducibility and claim gates. |
| GAR-IOREUSE-1 | Reusable I/O state, Vortex preparation, and flexible output fanout | RFC 0029, RFC 0031, RFC 0033, RFC 0036, RFC 0039, RFC 0040, RFC 0042 | Partially completed I/O reuse and benchmark lane covering the stable path `InputAdapter -> SourceState -> VortexPreparedState -> ExecutionPlan -> OutputPlan -> SinkArtifact`. Completed GAR-IOREUSE-1A adds the universal local SourceState benchmark/report contract with source identity, digest, source-format/location/fingerprint/schema fields, parse/decode plan digest, reuse hit/reason, materialization policy refs, no-fallback/no-external-engine fields, and `source_state_claim_gate_status=not_claim_grade` across CSV, JSONL, Parquet, Arrow IPC, Avro, and ORC rows. Completed GAR-IOREUSE-1B adds the scoped local VortexPreparedState benchmark/report contract with prepared artifact refs/digests, prepared-state identity/digest, source-state linkage, preparation timing separation through `vortex_prepare_millis` and `preparation_included_in_timing`, reuse hit/reason, Native I/O/materialization refs, no-fallback/no-external-engine fields, and `prepared_state_claim_gate_status=not_claim_grade`. Completed GAR-IOREUSE-1C adds the scoped local OutputPlan benchmark/report contract with output plan identity/digest, target format/schema/location posture, metadata preservation/materialization fields, local Vortex write/replay refs, sink artifact refs/digests, output Native I/O status, no-fallback/no-external-engine fields, and `output_plan_claim_gate_status=not_claim_grade`. Completed GAR-IOREUSE-1D adds the report-only fanout benchmark matrix with required fanout case IDs, requested/blocked output formats, deterministic blocker IDs/reasons, timing/reuse columns, `fanout_output_count=0`, no-fallback/no-external-engine fields, and `claim_gate_status=not_claim_grade`. Completed GAR-IOREUSE-1E adds the cache invalidation/fingerprint benchmark contract with source fingerprint kind, source content digest, source mtime/size, object ETag posture, manifest version, schema digest, plan digest, output-plan digest, cache-valid status, invalidation reason, credential-redaction status, no-fallback/no-external-engine fields, and `cache_invalidation_claim_gate_status=not_claim_grade`. Completed GAR-IOREUSE-1F adds the evidence-safe reuse-level contract and `reuse_level_matrix` with normalized discovery, schema, parse-plan, prepared-state, operator-source-state, output-plan, and result-replay reuse status, hit/allowed/digest/blocker fields, evidence-level linkage, output-format separation, invalidation reasons, `claim_grade_requirements_met=false`, no-fallback/no-external-engine fields, and `claim_gate_status=not_claim_grade`. Completed GAR-IOREUSE-1G adds report-only Foundry no-input generated-output fanout posture through `shardloom.foundry_generated_output_fanout_posture.v1` with `generated_output_execution_performed=false`, `generated_source_created=false`, generated-source/output certificates not emitted, `fanout_output_count=0`, no direct S3/object-store writes, no Foundry Spark, no fallback, no external engine, and `claim_gate_status=not_claim_grade`. The lane keeps input and output formats decoupled, separates source discovery/schema/parse timing from Vortex preparation, operator compute, output planning, output write, output replay timing, and fingerprint/invalidation posture, requires source/prepared/output reuse-hit and fanout-count fields, forbids persistent caches, object-store runtime, and lakehouse/table commit claims unless separately admitted, blocks hidden fast modes and external engine fallback, prevents reuse evidence from upgrading claim status by itself, and keeps Foundry generated-output posture report-only unless real Foundry output API evidence exists. |
| GAR-SCALE-1 | Spark-level scale contract and any-volume execution readiness | RFC 0008, RFC 0013, RFC 0014, RFC 0016, RFC 0017, RFC 0020, RFC 0026, RFC 0029, RFC 0031, RFC 0034, RFC 0035, RFC 0036 | Completed scale-readiness contract lane that forbids literal any-volume and Spark-replacement claims. GAR-SCALE-1A adds the report-only `shardloom.traditional_analytics.scale_claim_gate.v1` contract with scale classes, `scale_profile`, `scale_claim_status`, volume/row/file/partition/split counts, memory budget/peak memory, spill, shuffle, retry, output commit, object-store/table, remote-worker, Foundry, no-fallback, no-external-engine, and claim-boundary fields while keeping current ShardLoom rows restricted to `local_smoke` or `local_claim_grade` with `scale_claim_gate_status=not_scale_grade`. GAR-SCALE-1B adds the report-only `shardloom.traditional_analytics.split_manifest.v1` contract with SplitManifest IDs/digests, SourceState linkage, split counts/IDs, byte/row ranges, estimated rows/bytes, projection masks, filter pushdown posture, retry/runtime/row/spill/output refs, no-fallback/no-external-engine fields, and `split_claim_gate_status=not_split_scale_grade`. GAR-SCALE-1C adds the fail-closed `shardloom.traditional_analytics.memory_spill_backpressure.v1` contract with memory budget, operator memory budget, peak memory, memory-budget-exceeded, spill location/read/write/file/cleanup, backpressure, OOM-prevention, no-fallback/no-external-engine fields, and `memory_spill_claim_gate_status=not_larger_than_memory_grade` while keeping runtime spill/backpressure and larger-than-memory claims blocked. GAR-SCALE-1D adds the report-only `shardloom.traditional_analytics.shuffle_repartition.v1` contract with shuffle requirement/strategy, partitioning, local-combine/global-merge, broadcast, skew, shuffle spill, retry, correctness-digest, no-fallback/no-external-engine fields, and `shuffle_claim_gate_status=not_shuffle_scale_grade` while keeping distributed shuffle, Spark-scale join, skew-handling, retryable-shuffle, partitioned-write, and performance claims blocked. GAR-SCALE-1E adds the report-only `shardloom.traditional_analytics.object_table_scale_ladder.v1` contract with object-store URI/listing/split-planning/read/write/commit statuses, table metadata/runtime/commit/rollback statuses, credential/network/ETag/commit/rollback evidence, separate object-store read/write and table runtime/commit claim gates, no-fallback/no-external-engine fields, and `object_table_ladder_claim_gate_status=not_object_table_scale_grade` while keeping object-store runtime, table runtime, table commit, credential, network, lakehouse, and production claims blocked. GAR-SCALE-1F adds the report-only `shardloom.traditional_analytics.distributed_protocol.v1` contract with coordinator, worker, task lease, task attempt, split, retry, result-fragment, merge, no-fallback/no-external-engine fields, `distributed_claim_status=report_only`, and `distributed_claim_gate_status=not_distributed_runtime_grade` while keeping daemon/service/remote-worker/cluster/network/managed-platform claims blocked. GAR-SCALE-1G adds `shardloom.traditional_analytics.scale_benchmark_profile.v1` with scale profile definitions, required scale scenarios and metrics, row-level profile status, synthetic metadata-only boundaries, public-leaderboard separation, actual-large-volume proof requirements, no-fallback/no-external-engine fields, and `scale_benchmark_claim_gate_status=not_scale_benchmark_grade`. GAR-SCALE-1H adds `shardloom.foundry_scale_proof_boundary.v1` to the local Foundry proof report with Foundry runtime/compute/Spark, input/output dataset counts, staged input bytes, execution mode, split count, memory budget, evidence dataset, no-fallback/no-external-engine fields, and `claim_gate_status=not_foundry_scale_grade`. Later runtime slices must not promote object-store/table/distributed/Foundry/runtime scale claims from report-only protocol rows, synthetic metadata, local orchestration, or managed-platform wrappers alone. |
| GAR-NOVEL-1 | Evidence-native generated execution, lineage, observability, and confidence | RFC 0018, RFC 0029, RFC 0031, RFC 0032, RFC 0033, RFC 0035, RFC 0036, RFC 0037, RFC 0039, RFC 0040 | Completed cross-surface report-only lane aligning GeneratedSourceCertificate/source-free generated-output evidence with report-only OpenLineage facets, OpenTelemetry trace mapping, and Bayesian claim-confidence posture. Completed rows expose `shardloom.generated_source_evidence_alignment.v1`, `shardloom.openlineage_facet_mapping.v1`, `shardloom.opentelemetry_trace_export_contract.v1`, and `shardloom.traditional_analytics.bayesian_claim_confidence.v1`. Export and confidence surfaces are opt-in/report-only, no-network/no-backend by default, posterior confidence is not fit, credible intervals and regression probabilities are not computed, and these surfaces cannot upgrade runtime support, production readiness, performance, Spark-replacement, SQL/DataFrame, Foundry, object-store/lakehouse, or package claims without separate scoped evidence and no-fallback/no-external-engine proof. |
| GAR-COMMERCIAL-1 | Adoption and commercial-readiness friction reduction | RFC 0024, RFC 0030, RFC 0033, RFC 0035, RFC 0036, RFC 0037, RFC 0039, RFC 0043 | Completed adoption/commercial-readiness lane. GAR-COMMERCIAL-1A completes the one-command local install/smoke proof by running local build, exact wheel install in a clean virtual environment, installed-wheel Python/CLI smoke, scoped `ctx.from_rows(...).write(local_jsonl)` and `ctx.range(...).write(local_jsonl)` generated-source output smokes, `shardloom` plus `shardloom-prepared-vortex` benchmark smoke, provenance/SBOM/checksum dry-run evidence, and transcript safety fields. GAR-COMMERCIAL-1B completes the package-channel readiness matrix with `shardloom.package_channel_readiness_matrix.v1`, `scripts/check_package_channel_readiness.py`, hard release-gate integration, and rows for GitHub pre-release, TestPyPI, PyPI, Homebrew tap, Scoop, winget, conda-forge, GHCR container, and future crates.io public API crates. GAR-COMMERCIAL-1C completes the generated public "Can I use this?" status matrix in `website/status.html`, sourcing universal compatibility and package-channel rows while exposing `runtime-supported`, `smoke-supported`, `report-only`, `blocked`, `planned`, and `not-planned` posture plus no-fallback/no-external-engine evidence. GAR-COMMERCIAL-1D completes the report-only enterprise evidence export pack with `shardloom.enterprise_evidence_export_pack.v1`, local artifact layout, redaction/retention policy, and no-network/no-backend/no-event/no-trace/no-metric/no-log posture. GAR-COMMERCIAL-1E completes the local Foundry dev-stack starter with `shardloom.foundry_dev_stack_starter_kit.v1`, local proof commands, staged-input posture, source-free generated-output blockers, and Foundry output dataset boundaries. GAR-COMMERCIAL-1F completes the workflow recipe library with `shardloom.workflow_recipe_library.v1`, `scripts/check_workflow_recipes.py`, and recipe rows mapped to Use Case Atlas ids for local smokes, generated-output smokes, prepared/native direction, messy-data fixtures, result-sink proof, blocked object-store diagnostics, Foundry-style local proof, and benchmark interpretation. These surfaces reduce evaluation friction only; v0.1.8 GitHub/TestPyPI/PyPI/Homebrew publication proof is now recorded separately, but they do not authorize production support, external network/export backends, runtime expansion, performance/Spark-displacement claims, or fallback execution. |
| GAR-DOCS-1 | Non-expert Use Case Atlas and status matrix | RFC 0010, RFC 0012, RFC 0033, RFC 0034, RFC 0036, RFC 0037, RFC 0039 | Completed non-expert documentation lane. GAR-DOCS-1A adds `docs/use-cases/README.md` as the hub for answering whether a use case is supported, how to try it, what evidence appears, and what remains unsupported. GAR-DOCS-1B hardens `docs/use-cases/use-case-index.yml` with schema/status/reference/evidence/claim-boundary validation through `scripts/check_use_case_index.py` and `scripts/check_use_case_coverage.py`. GAR-DOCS-1C syncs use-case Markdown/data into the Astro/Starlight website through `website-src/scripts/sync-content.mjs` and `npm run build`. GAR-DOCS-1D adds all-capability documentation coverage checks. GAR-DOCS-1E adds the workflow recipe library with `shardloom.workflow_recipe_library.v1`. GAR-DOCS-1F adds non-expert field-guide glossary coverage. GAR-DOCS-1G adds exact reference/backlink validation. GAR-DOCS-1H adds the website "Can I use this?" matrix under `website/use-cases/` and status surfaces. The lane is documentation/status only and cannot upgrade runtime support, package publication, benchmark claims, production SQL/DataFrame, object-store/lakehouse, Foundry, performance, Spark-displacement, or no-fallback posture without separate scoped evidence. |
...
| GAR-0012-B | Envelope status and distributed/object-store diagnostic propagation | RFC 0012, RFC 0008, RFC 0010 | `cg10-object-store-runtime-gate` now propagates object-store/distributed runtime blocker rows as info-level `SL_OBJECT_STORE_UNSUPPORTED` envelope diagnostics, text summary fields, and Python typed result access; report-only status remains `success`; no object-store IO, distributed runtime, external engine, or fallback execution. |
