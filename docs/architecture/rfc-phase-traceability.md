...
| RFC 0042 | Partially implemented | Priority 2.6.5, Priority 2.7 | Defines Vortex runtime utilization and execution-spine hardening: utilization reporting across arrays, execution layers, Scan Source/Sink/Split, field masks, predicate ordering, layouts, I/O, sessions/registries, device residency, extension types, benchmark discipline, and Vortex integrations as baselines only. Current implementation adds report/code surfaces, tests, and the GAR-0042A `vortex-api-inventory` source/split admission proof for the scoped local Vortex scan fixture path. Generalized Source/Split runtime, field-mask/predicate-ordering evidence, layout/write evidence, object-store I/O, GPU/device execution, managed-platform benchmark lanes, external engine invocation, and fallback execution remain unauthorized. |
