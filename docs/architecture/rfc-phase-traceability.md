...
`source_state_prepare_timing_scope=batch_shared_pre_scenario` fields keep shared setup visible while
preserving child scenario envelopes and no-fallback/no-external-engine evidence. This is scoped
source-state runtime plumbing, not a performance, encoded-native, SQL/DataFrame, object-store,
lakehouse, production, or Spark-displacement claim.
