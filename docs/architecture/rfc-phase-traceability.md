...
| RFC 0018 | Partially implemented | 10D, 14A, 14B, GAR-0018-A, Ongoing | Observability foundations exist. GAR-0018-A adds a report-only runtime-introspection artifact that exposes the local benchmark stage-timing schema while keeping live profiling, profile artifacts, debug bundles, metrics exporters, trace backends, and distributed runtime introspection blocked with no runtime collection, no external engine invocation, and no fallback. |
