...
`fallback_attempted=false` evidence. Dirty CSV `clean/cast/filter/write` now executes through the
same local Vortex path with dirty-column clean/cast/filter evidence. Many-file, nested JSON, CDC,
and partition-pruning support remain blocked until those input contracts are certified.
