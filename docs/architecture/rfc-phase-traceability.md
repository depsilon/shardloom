...
as `metric:vortex.filter` while zero-result scans report no reader chunks; filter-only `flag,value`
batches, encoding-specific lowering, and conjunctive selection-vector evidence remain blocked. Broader
