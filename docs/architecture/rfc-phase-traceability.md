...
| GAR-SCALE-1 | Spark-level scale contract and any-volume execution readiness | RFC 0008, RFC 0013, RFC 0014, RFC 0016, RFC 0017, RFC 0020, RFC 0026, RFC 0029, RFC 0031, RFC 0034, RFC 0035, RFC 0036 | Planned scale-readiness lane that forbids literal any-volume and Spark-replacement claims while defining scale classes, split manifests, bounded-memory/spill/backpressure evidence, shuffle/repartition/join evidence, object-store/table scale ladders, distributed report-only protocol fields, scale benchmark profiles, synthetic metadata-only evidence boundaries, and Foundry scale proof boundaries. Future slices must keep local smoke/local claim evidence separate from larger-than-memory, split-parallel, object-store, table, distributed, Foundry, and managed-platform proof, require declared resource envelopes and no-fallback/no-external-engine evidence, and cannot promote object-store/table/distributed/Foundry/runtime scale claims from report-only protocol rows or synthetic metadata alone. |
