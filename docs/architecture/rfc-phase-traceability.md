...
| RFC 0029 | Partially implemented | CG-5, CG-6, CG-16, CG-17, GAR-NOVEL-1D | CG-16.1 local encoded count certificate, CG-16.2 execution-certificate evidence surface, CG-17.1 stateful reuse boundary report, and the CG-17 promotion gate for stable reuse keys/invalidation/manifest-diff/state-certificate/reuse-benchmark blockers exist; broader correctness, benchmark, certificate, cache read/write/replay, and incremental execution evidence remain future gate work. GAR-NOVEL-1D adds a planned advisory Bayesian claim-confidence schema that can block uncertain release/performance claims but cannot upgrade claim status alone. |
...
| GAR-NOVEL-1 | Evidence-native generated execution, lineage, observability, and confidence | RFC 0018, RFC 0029, RFC 0031, RFC 0032, RFC 0033, RFC 0035, RFC 0036, RFC 0037, RFC 0039, RFC 0040 | Partially completed cross-surface lane aligning GeneratedSourceCertificate/source-free generated-output evidence with report-only OpenLineage facets and OpenTelemetry trace mapping. Completed rows expose `shardloom.generated_source_evidence_alignment.v1`, `shardloom.openlineage_facet_mapping.v1`, and `shardloom.opentelemetry_trace_export_contract.v1`; Bayesian claim-confidence/regression fields remain planned. Export and confidence surfaces are opt-in/report-only, no-network/no-backend by default, and cannot upgrade runtime support, production readiness, performance, Spark-replacement, SQL/DataFrame, Foundry, object-store/lakehouse, or package claims without separate scoped evidence and no-fallback/no-external-engine proof. |
