...
| RFC 0005 | Partially implemented | 12A, GAR-0005-A, Ongoing | Vortex-native output contract is established. GAR-0005-A adds `shardloom.vortex_local_io_coverage.v1` through `vortex-api-inventory`, classifying the scoped local primitive scan reader lane, the feature-gated native CountAll output payload writer lane, broad local writer blockers, claim boundaries, and no-fallback/no-external-engine fields. `shardloom-cli/vortex-write` now directly enables the existing feature-gated native count-payload writer. Full staged write, generalized schema/encoding payload writing, table/catalog, object-store, lakehouse, SQL/DataFrame, and performance claims remain gated. |
