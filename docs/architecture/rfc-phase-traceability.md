...
| GAR-PERF-2 | Evidence-level runtime tiering, evidence-aware optimizer, scan pushdown completion, encoded kernel registry, fused operator pipelines, session runtime, allocation/buffer reuse, optimized build profiles/PGO, and native microbenchmark suite | RFC 0016, RFC 0022, RFC 0026, RFC 0029, RFC 0039, RFC 0040, RFC 0041, RFC 0042 | Partially completed performance-runtime evidence lane. Completed slices cover the `minimal_runtime`/`certified`/`full_replay` evidence-level contract, Vortex Scan API filter/projection/limit pushdown completion across prepared/native scenario families, scoped compressed/encoded kernel registry rows for selective-filter bitpacked and sequence inputs plus deterministic blockers for the remaining initial pairs, scoped fused local prepared/native residual-pipeline evidence for filter/projection/limit, filter/aggregate, and top-k/projection plus deterministic filter/group-by blockers, in-process `ShardLoomSession` runtime over scoped prepared/native local artifacts, and allocation profiling/scoped buffer-pool blocker evidence for result buffers, temporary vectors, hash tables, dictionary/string state, and source-state arrays. Planned slices still include evidence-aware logical optimizer rule registry and trace rows, optimized `release-lto`/`release-pgo`/`release-native-benchmark` build-profile evidence with portable-release and target-CPU-native boundaries, and native microbenchmark rows for Vortex scan-only, filter predicate-only, projection-only, group-by kernel, hash-join kernel, top-k, result-sink write, and evidence-render primitives. The lane separates evidence level, optimizer trace, scan pushdown, kernel admission, fusion/materialization avoidance, proof overhead, session/cache/buffer reuse, allocation/resource evidence, build-profile/PGO/native-codegen evidence, and subsystem/kernel evidence from execution mode and end-to-end benchmark claims, keeps `fallback_attempted=false` and `external_engine_invoked=false` visible in every row, treats `minimal_runtime` as `not_claim_grade` unless a future scoped gate explicitly approves promotion, forbids daemon/service/remote-server claims, hidden optimizer fallback, hidden global pools, unsafe lifetime shortcuts, hidden release `RUSTFLAGS`, and `target-cpu=native` portable release artifacts, keeps optimizer rule admission, scan pushdown, kernel registry admission, fused residual pipelines, buffer-reuse evidence, and optimized build profiles independent from encoded-native operator or performance claims, requires before/after plan digests and evidence preservation before optimizer rewrite promotion, requires end-to-end encoded evidence before `encoded_native_claim_allowed=true`, requires stronger independent fused/unfused runtime certificates before claim-grade fusion promotion, requires buffer reuse to preserve correctness and evidence parity, requires PGO/native benchmark lanes to record reproducibility and claim gates, and requires skipped/unsupported microbenchmark primitives to be deterministic visible rows. |
| GAR-IOREUSE-1 | Reusable I/O state, Vortex preparation, and flexible output fanout | RFC 0029, RFC 0031, RFC 0033, RFC 0036, RFC 0039, RFC 0040, RFC 0042 | Planned I/O reuse and benchmark lane covering the stable path `InputAdapter -> SourceState -> VortexPreparedState -> ExecutionPlan -> OutputPlan -> SinkArtifact`. The lane defines SourceState, VortexPreparedState, OutputPlan, cross-format fanout benchmark cases, source/prepared/output cache invalidation, evidence-safe reuse levels, and Foundry no-input generated-output fanout posture. It keeps input and output formats decoupled, separates source discovery/schema/parse timing from Vortex preparation, operator compute, output planning, output write, and output replay timing, requires source/prepared/output reuse-hit and fanout-count fields, forbids object-store runtime and lakehouse/table commit claims unless separately admitted, blocks hidden fast modes and external engine fallback, prevents reuse evidence from upgrading claim status by itself, and keeps Foundry generated-output posture report-only unless real Foundry output API evidence exists. |
