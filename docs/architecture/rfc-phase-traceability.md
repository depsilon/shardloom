...
| GAR-COMMERCIAL-1 | Adoption and commercial-readiness friction reduction | RFC 0024, RFC 0030, RFC 0033, RFC 0035, RFC 0036, RFC 0037, RFC 0039, RFC 0043 | Partially completed adoption/commercial-readiness lane. GAR-COMMERCIAL-1A completes the one-command local install/smoke proof by running local build, exact wheel install in a clean virtual environment, installed-wheel Python/CLI smoke, scoped `ctx.from_rows(...).write(local_jsonl)` and `ctx.range(...).write(local_jsonl)` generated-source output smokes, `shardloom` plus `shardloom-prepared-vortex` benchmark smoke, provenance/SBOM/checksum dry-run evidence, and transcript safety fields. GAR-COMMERCIAL-1B completes the package-channel readiness matrix with `shardloom.package_channel_readiness_matrix.v1`, `scripts/check_package_channel_readiness.py`, hard release-gate integration, and rows for GitHub pre-release, TestPyPI, PyPI, Homebrew tap, Scoop, winget, conda-forge, GHCR container, and future crates.io public API crates. GAR-COMMERCIAL-1C completes the generated public "Can I use this?" status matrix in `website/status.html`, sourcing universal compatibility and package-channel rows while exposing `runtime-supported`, `smoke-supported`, `report-only`, `blocked`, `planned`, and `not-planned` posture plus no-fallback/no-external-engine evidence. GAR-COMMERCIAL-1D completes the report-only enterprise evidence export pack with `shardloom.enterprise_evidence_export_pack.v1`, local artifact layout, redaction/retention policy, and no-network/no-backend/no-event/no-trace/no-metric/no-log posture. Remaining planned slices cover Foundry dev-stack starter and workflow recipes. These surfaces reduce evaluation friction only; they do not authorize package publication, release tags, OCI pushes, package-channel submissions, Foundry production support, external network/export backends, runtime expansion, performance/Spark-replacement claims, or fallback execution. |
