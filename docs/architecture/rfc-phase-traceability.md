...
| Phase 13A — Lakehouse table intelligence | RFC 0020 Schema Evolution, Catalog Integration, and Table Compatibility | RFC 0004 Native Dataset Manifest, Snapshot, Incremental; RFC 0019 Security | CG-9.1 schema-evolution compatibility report complete; broader schema evolution catalog/table metadata integration, partition/spec metadata, snapshots/time travel, delete/tombstone semantics, and metadata-loss propagation remain planned; no silent unsafe coercion; no fallback execution | Compatibility intelligence must remain explicit, typed, and safety-first. |
...
| RFC 0020 | Partially implemented | 12A, 13A, 13B | CG-9.1 schema-evolution compatibility evidence exists for typed schema transitions; broader catalog/table metadata integration, partition evolution, delete/tombstone semantics, and layout/compaction intelligence remain planned. |
