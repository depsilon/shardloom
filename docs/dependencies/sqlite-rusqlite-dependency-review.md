...
- Direct crate: `rusqlite = 0.40.2` in `shardloom-cli`.
- Feature posture: `default-features = false`, `features = ["bundled"]`.
- Source: Dependabot PRs <https://github.com/depsilon/shardloom/pull/1153> and
  <https://github.com/depsilon/shardloom/pull/1406>.
- `cargo info rusqlite@0.40.2` reports license `MIT`, documentation
  <https://docs.rs/rusqlite/>, repository <https://github.com/rusqlite/rusqlite>, and crates.io
  version <https://crates.io/crates/rusqlite/0.40.2>.
- Immediate host-target transitive dependency for SQLite FFI: `libsqlite3-sys = 0.38.2`, license
  `MIT` per `cargo info libsqlite3-sys@0.38.2`.
- The 0.40.2 release lowers upstream MSRV to 1.88.0; it does not broaden ShardLoom's SQLite
  fixture-smoke runtime scope.
...
- `cargo tree -p shardloom-cli --edges normal --depth 3` confirms the host-target normal tree
  includes `rusqlite v0.40.2` and `libsqlite3-sys v0.38.2` without `hashlink`,
