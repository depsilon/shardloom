...
Provide design guidance for ShardLoom's native-first plan IR and interoperability direction so plan import/export stays capability-checked, explicit about boundaries, and no-fallback.
...
- Treat Substrait-compatible thinking as an interoperability direction, not a dependency or fallback execution path.
...
- Are capability checks comprehensive (types, functions, kernels, IO, memory/spill, effects, outputs)?
...
"Propose a ShardLoom-native plan IR node contract for `<node kind>` and a capability validation path for imported Substrait-like plans that enforces explicit unsupported diagnostics, explicit effect/translation boundaries, versioned schema direction, and no fallback execution."
