...
Selected v0.2.2 package channels expose install commands. Future package channels remain blocked until their channel-specific proof exists.
...
| --- | --- | --- |
| GitHub pre-release | gh release download v0.2.2 --repo depsilon/shardloom --pattern '*' --dir <install-dir> | rm -rf <install-dir> |
| TestPyPI | python -m pip install --index-url https://test.pypi.org/simple/ --no-deps shardloom==0.2.2 | python -m pip uninstall -y shardloom |
| PyPI | python -m pip install shardloom==0.2.2 | python -m pip uninstall -y shardloom |
