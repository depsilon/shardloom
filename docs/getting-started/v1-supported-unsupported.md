...
| generated-source-user-rows, generated-source-range, generated-source-sequence, generated-source-sql | executable | default | true | true | not_claim_grade | source-free/generated local output runtime only; not a broad SQL generator, external platform sink, or performance claim |
| local-source-runtime | executable | default | true | false | local_workflow_runtime_supported | local-source SQL runtime executes admitted product workflow shapes through ShardLoom-owned planning; broad SQL/DataFrame, object-store/table SQL, external-engine fallback, and performance claims remain bounded |
...
| vortex-prepare | feature_gated | vortex-write | true | true | local_workflow_runtime_supported | local compatibility-source preparation writes Vortex artifacts with scout ingress, layout/write advisor, copy-budget, append-only differential overlay, and capillary cold-preparation evidence when feature enabled; table/object-store/CDC transaction and performance claims remain bounded |
...
| ctx.read, read_csv, read_json, read_parquet, read_arrow_ipc, read_avro, read_orc, sql, LazyFrame.collect/profile/write_vortex/write_jsonl/write_csv/fanout local-file vortex-middle route | feature_gated | release-user-surfaces for Vortex preparation/provider execution; default builds fail closed with deterministic feature-gate diagnostics | true | true | not_claim_grade | Local Python/SQL/DataFrame workflows normalize through Vortex preparation or native Vortex input before admitted native primitive/provider execution where those routes exist; provider JSONL/CSV result-summary exports, primitive JSONL/CSV row-stream/fanout exports, and scalar aggregate JSONL/CSV result-row exports are Vortex-derived with explicit decode/materialization evidence. Residual product-local operators and broad compatibility sinks block until a native/prepared Vortex route or Vortex-derived export contract exists. Direct decoded local-source smoke remains internal-only; no arbitrary SQL/DataFrame, object-store/table, fallback, or performance claim |
| read_vortex, LazyFrame.collect native Vortex primitives plus feature-gated materializing primitives | executable | default | true | false | not_claim_grade | Local Vortex count/filter/project primitives are default-supported; scalar aggregate and materializing primitive families are admitted where the release build enables local Vortex primitive execution and emit explicit decode/materialization evidence; broader Vortex operator families require admitted native provider evidence |
...
| Surface | State | Feature gate | Runtime | Write | Claim gate | Boundary |
| --- | --- | --- | --- | --- | --- | --- |
| csv, json, jsonl, ndjson | executable | default | true | false | fixture_smoke_only | flat local text SourceState, internal internal-source-smoke smoke, and vortex_ingest fixture coverage only; public Python/DataFrame runtime requires Vortex preparation or native Vortex input |
...
| parquet, arrow_ipc, avro, orc | feature_gated | universal-format-io | true | false | not_claim_grade | local flat-scalar feature-gated SourceState, internal internal-source-smoke smoke, and vortex_ingest adapters only; public Python/DataFrame runtime requires Vortex preparation or native Vortex input |
...
| vortex | feature_gated | vortex-write | true | true | not_claim_grade | Local Vortex artifact writes are admitted when the Vortex writer feature is enabled; no object-store/table commit or production claim |
...
Selected v0.2.3 package channels expose install commands. Future package channels remain blocked until their channel-specific proof exists.
...
| --- | --- | --- |
| GitHub pre-release | gh release download v0.2.3 --repo depsilon/shardloom --pattern '*' --dir <install-dir> | rm -rf <install-dir> |
| TestPyPI | python -m pip install --index-url https://test.pypi.org/simple/ --no-deps shardloom==0.2.3 | python -m pip uninstall -y shardloom |
| PyPI | python -m pip install shardloom==0.2.3 | python -m pip uninstall -y shardloom |
