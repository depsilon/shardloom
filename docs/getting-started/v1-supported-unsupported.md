...
| local-source-runtime | executable | default | true | false | local_workflow_runtime_supported | local-source SQL runtime is a ShardLoom-owned compatibility adapter/smoke command used under the Vortex-middle policy; public product workflows prepare into Vortex or use native Vortex input before admitted execution, and broad SQL/DataFrame, object-store/table SQL, external-engine fallback, and performance claims remain bounded |
