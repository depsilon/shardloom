...
| read_vortex, LazyFrame.collect native Vortex primitives plus feature-gated materializing primitives | executable | default | true | false | fixture_smoke_only | scoped local Vortex count/filter/project primitives are default-supported; scalar aggregate and materializing primitive families are admitted where the release build enables local Vortex primitive execution and emit explicit decode/materialization evidence; broader Vortex operator families require admitted native provider evidence |
