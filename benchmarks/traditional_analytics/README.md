...
prepared/native paths use Vortex scan filter/projection pushdown where applicable and avoid full
fact-table materialization; other benchmark operators still materialize Vortex-derived arrays after
scan. These rows are not broad SQL/DataFrame performance claims.
...
state plus residual grouped join output. None of these paths are encoded-native operator claims.
