...
and `filter + projection + limit` prepared/native paths use Vortex scan filter/projection pushdown
...
rows when filter/projection pushdown runs without full-table materialization; it is still not an
