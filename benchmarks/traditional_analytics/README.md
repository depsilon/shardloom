...
`join + aggregate` prepared/native paths use Vortex scan filter/projection pushdown where applicable
and avoid full fact-table materialization; other benchmark operators still materialize
Vortex-derived arrays after scan. These rows are not broad SQL/DataFrame performance claims.
...
residual grouped `(dim_label, category)` aggregation over projected fact/dimension scans. None of
these paths are encoded-native operator claims.
