...
`filter + projection + limit`, and `group by aggregation` prepared/native paths use Vortex scan
filter/projection pushdown where applicable and avoid full fact-table materialization; other
...
materialization. Neither path is an encoded-native operator claim.
