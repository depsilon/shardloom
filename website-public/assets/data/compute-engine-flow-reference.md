...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=77`
and `executable_fixture_count=66`, including scoped UTF-8 regex predicates, composed string functions, temporal arithmetic/
...
diagnostics for timezone database conversion syntax/functions/type spellings, arbitrary interval arithmetic, `COLLATE`/`ILIKE` locale and case-folding semantics, non-exact decimal division, broad ANSI decimal coercion, exponent notation, decimal/float comparison, local Vortex typed decimal output, or Avro/ORC typed decimal sinks, complex ordering/accessors/casts/subquery membership/nested source decoding/flat sinks beyond the scoped ARRAY/STRUCT result-boundary projection and structural DISTINCT/UNION DISTINCT equality rows, variant,
