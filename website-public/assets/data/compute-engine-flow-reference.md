...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=57`
and `executable_fixture_count=47`, including scoped UTF-8 regex predicates, composed string functions, temporal arithmetic/
difference, CASE, IN-list NULL semantics, row-value literal IN predicates, row-value IN-subquery predicates, projected row-value/quantified subqueries, correlated joined projected scalar/row-value/quantified subqueries, scoped quantified ANY/ALL subqueries, scoped SQL UNION
