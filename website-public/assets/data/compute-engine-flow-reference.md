...
and `executable_fixture_count=28`, including scoped UTF-8 regex predicates, composed string functions, temporal arithmetic/
...
diagnostics for decimal, timezone, interval, collation, list/array, struct/row, variant,
union dtype, binary-source, scalar-left multi-column IN-subquery, nested/joined/grouped/correlated subquery, and
non-admitted quantified shapes while scoped EXISTS, scoped quantified ANY/ALL, and scoped UTF-8 regex predicates are admitted. `runtime.5g-f1` now exposes the current physical operator plan, operator
