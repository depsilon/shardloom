...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=42`
and `executable_fixture_count=24`, including composed string functions, temporal arithmetic/
difference, CASE, IN-list NULL semantics, row-value literal IN predicates, scoped SQL UNION
...
union dtype, binary-source, row-value or multi-column IN-subquery, nested/joined/grouped/correlated subquery, and
...
scoped row-value literal `IN`/`NOT IN`, local scalar IN-subquery
...
predicates expose `in_predicate_runtime_execution` and `in_list_value_count`; row-value literal
...
mixed DATE/non-DATE lists, oversized lists/subquery sets, and broader row-value or multi-column
IN-subquery, nested, joined, grouped/HAVING-internal, correlated, and EXISTS/ANY/ALL subquery shapes
