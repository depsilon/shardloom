...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=23`
and `executable_fixture_count=16`, including composed string functions, temporal arithmetic/
difference, CASE, IN-list NULL semantics, scalar IN subqueries, COUNT(DISTINCT), hidden HAVING
aggregates, mixed window row_number/rank/lag/ntile semantics, multi-key joins, and seven
deterministic unsupported diagnostics for decimal, timezone, interval, regex, and collation
families. Broader window frames/value functions, advanced subqueries, complex dtypes, and broad
encoded-kernel coverage remain split follow-up items rather than hidden side lanes.
