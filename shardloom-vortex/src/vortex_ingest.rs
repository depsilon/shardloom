...
#[cfg(feature = "vortex-write")]
const VORTEX_PREPARED_OLAP_WRITER_SOURCE_TEXT_ZSTD_VALUES_PER_FRAME: usize = 8_192;
#[cfg(feature = "vortex-write")]
const VORTEX_PREPARED_OLAP_LARGE_REOPEN_DEFER_BYTES: u64 =
    1024 * VORTEX_PREPARED_OLAP_WRITER_ONE_MIB;
const VORTEX_PREPARED_OLAP_LAYOUT_INVENTORY_DEFERRED_STATUS: &str =
    "writer_summary_single_vortex_artifact_layout_inventory_deferred";
#[cfg(feature = "vortex-write")]
const VORTEX_PREPARED_OLAP_REOPEN_DEFERRED_STATUS: &str =
...
pub struct VortexPreparedOlapLayoutInventory {
    ...
    pub layout_size_attribution: String,
...
impl VortexPreparedOlapLayoutInventory {
    ...
    fn unavailable(status: impl Into<String>) -> Self {
        ...
        Self {
            ...
            layout_size_attribution: "not_available".to_string(),
            inventory_digest,
        }
    }
    ...
    #[cfg(feature = "vortex-write")]
    #[must_use]
    fn deferred_from_writer_summary(
        path: &Path,
        row_count: u64,
        bytes_written: u64,
        layout_write_decision: &VortexLayoutWriteRuntimeDecision,
    ) -> Self {
        let artifact_size_bytes = std::fs::metadata(path)
            .ok()
            .map_or(Some(bytes_written), |metadata| Some(metadata.len()));
        ...
        let inventory_digest = fnv64_digest_text(&format!(
            ...
            row_count,
            artifact_size_bytes.map_or_else(|| "unknown".to_string(), |value| value.to_string()),
            writer_layout,
            writer_policy,
            layout_write_decision.strategy_decision_digest
        ));
        Self {
            status: VORTEX_PREPARED_OLAP_LAYOUT_INVENTORY_DEFERRED_STATUS.to_string(),
            row_count: Some(row_count),
            segment_count: None,
            statistics_status: "deferred_until_query_open".to_string(),
            encoding_layout_status: "writer_strategy_recorded_inventory_deferred".to_string(),
            artifact_size_bytes,
            approx_footer_bytes: None,
            dtype_summary: "deferred_until_query_open".to_string(),
            root_layout_encoding: "deferred_until_query_open".to_string(),
            layout_encoding_inventory: writer_layout.to_string(),
            per_column_metadata_contract: "deferred_until_query_open".to_string(),
            segment_membership_status: "deferred_until_query_open".to_string(),
            domain_dictionary_status: "deferred_until_query_open".to_string(),
            derived_layout_stats_status: "writer_strategy_recorded_deferred_until_query_open"
                .to_string(),
            row_position_locality_status: "deferred_until_query_open".to_string(),
            layout_reader_cache_status: "deferred_for_large_artifact_public_prepare".to_string(),
            metadata_persisted_in_artifact: true,
            operator_selection_metadata_status: "deferred_until_query_open".to_string(),
    ...
    #[must_use]
    pub fn artifact_size_bytes_field(&self) -> String {
...
pub struct VortexPreparedOlapStateWriteRequest {
    ...
    pub row_position_manifest: String,
    pub dictionary_union_manifest: String,
    pub feature_gates: String,
    pub certification_level: String,
    pub certificate_refs: String,
    pub fallback_attempted: bool,
...
pub struct VortexPreparedOlapStateReport {
    ...
    pub layout_artifact_size_bytes: String,
    ...
    pub row_position_manifest: String,
    pub dictionary_union_manifest: String,
    pub admitted_query_families: String,
    pub exact_sidecar_family_count: usize,
    pub query_time_contract: String,
    pub sub_second_candidate: bool,
...
impl VortexPreparedOlapStateWriteRequest {
    /// Create a local exact OLAP prepared-state manifest request.
    ///
    /// # Errors
    /// Returns [`ShardLoomError::InvalidOperation`] when local artifact paths
    /// cannot be resolved.
    #[allow(clippy::too_many_arguments)]
    pub fn new_local(
        ...
        Ok(Self {
            ...
            prepared_artifact_path: absolute_local_path(prepared_artifact_path.as_ref())?,
            manifest_path: absolute_local_path(manifest_path.into())?,
            prepared_layout_policy: "not_declared".to_string(),
            segment_map_status: "not_declared".to_string(),
            derived_column_manifest: "none".to_string(),
            substring_index_manifest: "none".to_string(),
            aggregate_summary_manifest: "none".to_string(),
            row_position_manifest: "none".to_string(),
            dictionary_union_manifest: "none".to_string(),
            feature_gates: "vortex-write,vortex-local-primitives".to_string(),
            certification_level: "prepared_olap_contract_declared".to_string(),
            certificate_refs: "none".to_string(),
    ...
    #[must_use]
    pub fn with_prepared_layout_policy(mut self, value: impl Into<String>) -> Self {
        self.prepared_layout_policy = value.into();
        self
    }

    #[must_use]
    pub fn with_segment_map_status(mut self, value: impl Into<String>) -> Self {
        self.segment_map_status = value.into();
        self
    }

    #[must_use]
    pub fn with_derived_column_manifest(mut self, value: impl Into<String>) -> Self {
        self.derived_column_manifest = value.into();
        self
    }

    #[must_use]
    pub fn with_substring_index_manifest(mut self, value: impl Into<String>) -> Self {
        self.substring_index_manifest = value.into();
        self
    }

    #[must_use]
    pub fn with_aggregate_summary_manifest(mut self, value: impl Into<String>) -> Self {
        self.aggregate_summary_manifest = value.into();
        self
    }

    #[must_use]
    pub fn with_row_position_manifest(mut self, value: impl Into<String>) -> Self {
        self.row_position_manifest = value.into();
        self
    }

    #[must_use]
    pub fn with_dictionary_union_manifest(mut self, value: impl Into<String>) -> Self {
        self.dictionary_union_manifest = value.into();
        self
    }

    #[must_use]
    pub fn with_feature_gates(mut self, value: impl Into<String>) -> Self {
        self.feature_gates = value.into();
        self
    }

    #[must_use]
    pub fn with_certification_level(mut self, value: impl Into<String>) -> Self {
        self.certification_level = value.into();
        self
    }

    #[must_use]
    pub fn with_certificate_refs(mut self, value: impl Into<String>) -> Self {
        self.certificate_refs = value.into();
        self
    }

    #[must_use]
    pub fn with_fallback_attempted(mut self, value: bool) -> Self {
        self.fallback_attempted = value;
        self
    }

    #[must_use]
    pub fn with_external_engine_invoked(mut self, value: bool) -> Self {
...
impl VortexPreparedOlapStateReport {
    /// Return stable evidence fields for CLI/API surfaces.
    #[must_use]
    #[allow(clippy::too_many_lines)]
    pub fn evidence_fields(&self) -> Vec<(String, String)> {
        vec![
            ...
            (
                "vortex_prepared_olap_state_status".to_string(),
                self.status.clone(),
            ),
            (
            ...
            (
                "vortex_prepared_olap_state_digest".to_string(),
                self.state_digest.clone(),
            ),
            (
                "vortex_prepared_olap_state_source_state_id".to_string(),
                self.source_state_id.clone(),
            ),
            (
                "vortex_prepared_olap_state_source_state_digest".to_string(),
                self.source_state_digest.clone(),
            ),
            (
                "vortex_prepared_olap_state_source_format".to_string(),
                self.source_format.clone(),
            ),
            (
                "vortex_prepared_olap_state_source_content_digest".to_string(),
                self.source_content_digest.clone(),
            ),
            (
                "vortex_prepared_olap_state_source_size_bytes".to_string(),
                self.source_size_bytes.to_string(),
            ),
            (
                "vortex_prepared_olap_state_source_schema_digest".to_string(),
                self.source_schema_digest.clone(),
            ),
            (
                "vortex_prepared_olap_state_source_row_count".to_string(),
                self.source_row_count.to_string(),
            ),
            (
                "vortex_prepared_olap_state_prepared_artifact_ref".to_string(),
                self.prepared_artifact_ref.display().to_string(),
            ),
            (
                "vortex_prepared_olap_state_prepared_artifact_digest".to_string(),
                self.prepared_artifact_digest.clone(),
            ),
            (
                "vortex_prepared_olap_state_prepared_artifact_size_bytes".to_string(),
                self.prepared_artifact_size_bytes.to_string(),
            ),
            (
            ...
            (
                "vortex_prepared_olap_state_segment_map_status".to_string(),
            ...
            (
                "vortex_prepared_olap_state_layout_artifact_size_bytes".to_string(),
                self.layout_artifact_size_bytes.clone(),
            ),
            (
            ...
            (
                "vortex_prepared_olap_state_layout_size_attribution".to_string(),
                self.layout_size_attribution.clone(),
            ),
            (
            ...
            (
                "vortex_prepared_olap_state_admitted_query_families".to_string(),
                self.admitted_query_families.clone(),
            ),
            (
                "vortex_prepared_olap_state_exact_sidecar_family_count".to_string(),
                self.exact_sidecar_family_count.to_string(),
            ),
            (
                "vortex_prepared_olap_state_query_time_contract".to_string(),
                self.query_time_contract.clone(),
            ),
            (
                "vortex_prepared_olap_state_sub_second_candidate".to_string(),
                self.sub_second_candidate.to_string(),
            ),
            (
                "vortex_prepared_olap_state_blocker_id".to_string(),
                self.blocker_id.clone(),
            ),
            (
                "vortex_prepared_olap_state_next_action".to_string(),
                self.next_action.clone(),
            ),
            (
                "vortex_prepared_olap_state_feature_gates".to_string(),
                self.feature_gates.clone(),
            ),
            (
                "vortex_prepared_olap_state_certification_level".to_string(),
                self.certification_level.clone(),
            ),
            (
                "vortex_prepared_olap_state_certificate_refs".to_string(),
                self.certificate_refs.clone(),
            ),
            (
                "vortex_prepared_olap_state_fallback_attempted".to_string(),
                self.fallback_attempted.to_string(),
            ),
            (
                "vortex_prepared_olap_state_external_engine_invoked".to_string(),
                self.external_engine_invoked.to_string(),
            ),
        ]
    }
...
fn prepared_olap_state_status(
    ...
    } else if request.external_engine_invoked {
        "blocked_external_engine_invoked".to_string()
    } else if request.source_state_id.trim().is_empty()
        || request.source_state_digest.trim().is_empty()
        || request.source_schema_digest.trim().is_empty()
    {
...
fn prepared_olap_blocker_id(
    request: &VortexPreparedOlapStateWriteRequest,
    exact_sidecar_family_count: usize,
) -> String {
    match prepared_olap_state_status(request, exact_sidecar_family_count).as_str() {
        "prepared_olap_state_ready" => "none".to_string(),
        "blocked_fallback_attempted" => "prepared_olap_state.fallback_attempted".to_string(),
        "blocked_external_engine_invoked" => {
            "prepared_olap_state.external_engine_invoked".to_string()
        }
        "blocked_missing_source_state_evidence" => {
            "prepared_olap_state.source_state_evidence_missing".to_string()
        }
...
fn prepared_olap_admitted_query_families(request: &VortexPreparedOlapStateWriteRequest) -> String {
    ...
    if prepared_olap_metadata_ref_declared(&request.aggregate_summary_manifest) {
        ...
        {
            families.push("exact_global_count_summary");
        } else {
...
fn prepared_olap_exact_sidecar_family_count(
    request: &VortexPreparedOlapStateWriteRequest,
) -> usize {
    [
...
fn prepared_olap_state_digest(
    request: &VortexPreparedOlapStateWriteRequest,
    artifact_fingerprint: &LocalReuseFileFingerprint,
    exact_sidecar_family_count: usize,
) -> String {
    let mut digest = sha2::Sha256::new();
    let source_row_count = request.source_row_count.to_string();
    let prepared_artifact_size_bytes = artifact_fingerprint.size_bytes.to_string();
    let exact_sidecar_family_count = exact_sidecar_family_count.to_string();
    for value in [
        VORTEX_PREPARED_OLAP_STATE_SCHEMA_VERSION,
        VORTEX_PREPARED_OLAP_STATE_POLICY,
        request.source_state_id.as_str(),
        request.source_state_digest.as_str(),
        request.source_format.as_str(),
        request.source_content_digest.as_str(),
        request.source_schema_digest.as_str(),
        source_row_count.as_str(),
        artifact_fingerprint.content_digest.as_str(),
        prepared_artifact_size_bytes.as_str(),
        request.prepared_layout_policy.as_str(),
        request.segment_map_status.as_str(),
        request.derived_column_manifest.as_str(),
        request.substring_index_manifest.as_str(),
        request.aggregate_summary_manifest.as_str(),
        request.row_position_manifest.as_str(),
        request.dictionary_union_manifest.as_str(),
        request.feature_gates.as_str(),
        request.certification_level.as_str(),
        exact_sidecar_family_count.as_str(),
    ] {
        digest.update(value.as_bytes());
        digest.update(b"\0");
    }
...
fn prepared_olap_report_from_request(
    ...
    let sub_second_candidate =
        blocker_id == "none" && prepared_olap_has_sub_second_candidate_family(request);
    let state_digest =
    ...
    let layout_artifact_size_bytes = layout_inventory.artifact_size_bytes_field();
    ...
    let layout_size_attribution = layout_inventory.layout_size_attribution.clone();
    VortexPreparedOlapStateReport {
        ...
        admitted_query_families: prepared_olap_admitted_query_families(request),
        exact_sidecar_family_count,
        query_time_contract,
        sub_second_candidate,
        next_action: prepared_olap_next_action(&blocker_id),
        blocker_id,
        feature_gates: request.feature_gates.clone(),
        certification_level: request.certification_level.clone(),
...
fn prepared_olap_blocked_report(
    ...
    next_action: &str,
    manifest_digest: &str,
    artifact_fingerprint: Option<&LocalReuseFileFingerprint>,
) -> VortexPreparedOlapStateReport {
    let exact_sidecar_family_count = prepared_olap_exact_sidecar_family_count(request);
    let state_digest = artifact_fingerprint.map_or_else(
        || "none".to_string(),
        |fingerprint| prepared_olap_state_digest(request, fingerprint, exact_sidecar_family_count),
    );
    let state_id = state_digest.strip_prefix("sha256:").map_or_else(
        || "none".to_string(),
    ...
    VortexPreparedOlapStateReport {
        ...
        source_row_count: request.source_row_count,
        prepared_artifact_ref: request.prepared_artifact_path.clone(),
        prepared_artifact_digest: artifact_fingerprint.map_or_else(
            || "none".to_string(),
            |fingerprint| fingerprint.content_digest.clone(),
        ),
        ...
        admitted_query_families: prepared_olap_admitted_query_families(request),
        exact_sidecar_family_count,
        query_time_contract: "not_admitted_for_prepared_olap_query_time".to_string(),
        sub_second_candidate: false,
...
fn reuse_manifest_digest(fields: &BTreeMap<String, String>) -> String {
    ...
    for (key, value) in fields
        .iter()
        .filter(|(key, _)| !matches!(key.as_str(), "manifest_digest" | "sidecar_digest"))
    {
...
impl VortexLayoutWriteAdvisorReport {
    ...
    pub fn evidence_fields(&self) -> Vec<(String, String)> {
        ...
        Self::push_field(
            ...
            self.writer_parallelism_budget.to_string(),
        );
        Self::push_field(
            &mut fields,
...
pub struct VortexPreparedStateWriteReport {
    ...
    pub artifact_digest_source: String,
    ...
    pub writer_coalescing_policy_status: String,
...
impl VortexPreparedStateWriteReport {
    ...
    /// Return the number of source fields with an explicit compression override.
    #[must_use]
    #[cfg(feature = "vortex-write")]
    ...
    #[must_use]
    #[cfg(not(feature = "vortex-write"))]
    pub const fn writer_compression_field_count(&self) -> usize {
        0
    }

    /// Return a stable comma-separated list of source fields with compression overrides.
    #[must_use]
    #[cfg(feature = "vortex-write")]
    ...
    pub fn prepared_olap_layout_inventory_summary(&self) -> String {
        format!(
            "status={};row_count={};segment_count={};statistics={};encoding_layout={};artifact_bytes={};footer_bytes={};root_layout={};layout_encodings={};segment_membership={};domain_dictionary={};derived_stats={};row_position_locality={};layout_reader_cache={};persisted_in_artifact={};size_attribution={};digest={}",
            self.prepared_olap_layout_inventory.status,
            self.prepared_olap_layout_inventory.row_count_field(),
            self.prepared_olap_layout_inventory.segment_count_field(),
            self.prepared_olap_layout_inventory.statistics_status,
            ...
            self.prepared_olap_layout_inventory
                .metadata_persisted_in_artifact,
            self.prepared_olap_layout_inventory.layout_size_attribution,
...
fn finalize_vortex_prepared_state_write(
    ...
    let (
        ...
        let (prepared_olap_layout_inventory, reopen_scan_micros, reopen_verification_status) =
            prepared_vortex_artifact_layout_inventory_after_write(
                &input.target_path,
        ...
        let prepared_olap_layout_inventory =
            if should_defer_prepared_vortex_artifact_layout_inventory(
                ...
                VortexPreparedOlapLayoutInventory::deferred_from_writer_summary(
                    &input.target_path,
                    write_result.writer_row_count,
                    write_result.bytes_written,
                    &input.layout_write_decision,
    ...
    Ok(VortexPreparedStateWriteReport {
        ...
        artifact_digest_source: write_result.artifact_digest_source,
        ...
        writer_coalescing_policy_status: write_result.writer_coalescing_policy_status,
...
fn preparation_spine_report(
    ...
    let reopen_provider_surface = if reopen_verification_uses_writer_summary(
        reopen_verification_status,
    ) {
        "VortexSession::write_options().write(ArrayStream).summary().row_count();layout_inventory_deferred_until_query_open"
    } else if reopen_verification_uses_metadata(reopen_verification_status) {
    ...
    let native_io_certificate_refs = if reopen_verification_uses_writer_summary(
        reopen_verification_status,
    ) {
        "source_split_refs,prepared_artifact_ref,writer_summary_row_count,artifact_digest,layout_inventory_deferred"
    } else if reopen_verification_uses_metadata(reopen_verification_status) {
...
impl LocalVortexWriteContext {
    ...
    fn write_array(
        ...
        Ok(LocalVortexWriteResult {
            ...
            bytes_written,
            artifact_digest: workspace_write_report.output_digest.clone(),
            artifact_digest_source: "workspace_safe_streaming_sha256_digest".to_string(),
            digest_micros: 0,
            write_micros,
...
#[cfg(feature = "vortex-write")]
fn vortex_writer_coalescing_policy_status(
    decision: &VortexLayoutWriteRuntimeDecision,
) -> &'static str {
    if !vortex_layout_write_strategy_applies(decision) {
        return "upstream_vortex_default_writer_coalescing_policy";
    }
    if vortex_writer_block_target_bytes(decision) > 0 {
        "vortex_repartition_writer_coalescing_enabled_with_row_block_and_byte_target"
    } else {
        "vortex_row_block_writer_strategy_enabled_without_byte_target"
    }
}
...
#[cfg(feature = "vortex-write")]
fn vortex_writer_stats_concurrency(decision: &VortexLayoutWriteRuntimeDecision) -> usize {
    if vortex_layout_write_strategy_applies(decision) {
...
fn large_source_fast_zstd_text_leaf_strategy(
    ...
    let values_per_frame = row_block_size.clamp(
        1,
...
#[cfg(feature = "vortex-write")]
fn vortex_layout_size_attribution(
    dtype: &vortex::array::dtype::DType,
    artifact_size_bytes: Option<u64>,
    approx_footer_bytes: Option<usize>,
    layout_encoding_inventory: &str,
    domain_dictionary_status: &str,
) -> String {
    let mut attribution = VortexLayoutSizeAttribution::default();
    attribution.visit_dtype(dtype, None);
    format!(
        "artifact_bytes={};footer_bytes={};source_columns={};derived_columns={};derived_utf8_length_columns={};derived_url_domain_columns={};derived_time_columns={};derived_unknown_columns={};source_utf8_columns={};source_numeric_columns={};dictionary_status={};layout_encodings={};column_byte_attribution={}",
        artifact_size_bytes.map_or_else(|| "unknown".to_string(), |value| value.to_string()),
        approx_footer_bytes.map_or_else(|| "unknown".to_string(), |value| value.to_string()),
        attribution.source_columns,
        attribution.derived_columns,
        attribution.derived_utf8_length_columns,
        attribution.derived_url_domain_columns,
        attribution.derived_time_columns,
        attribution.derived_unknown_columns,
        attribution.source_utf8_columns,
        attribution.source_numeric_columns,
        domain_dictionary_status,
        compact_metadata_token(layout_encoding_inventory),
        "not_available_from_current_vortex_footer"
    )
}
...
struct VortexLayoutSizeAttribution {
    ...
    derived_utf8_length_columns: usize,
    derived_url_domain_columns: usize,
    derived_time_columns: usize,
    derived_unknown_columns: usize,
    source_utf8_columns: usize,
    source_numeric_columns: usize,
}

#[cfg(feature = "vortex-write")]
impl VortexLayoutSizeAttribution {
    fn visit_dtype(&mut self, dtype: &vortex::array::dtype::DType, field_name: Option<&str>) {
        ...
        match dtype {
            DType::Struct(fields, _) => {
                for name in fields.names().iter() {
                    let name = name.as_ref();
                    let field_dtype = fields.field(name).unwrap_or_else(|| dtype.clone());
                    self.visit_dtype(&field_dtype, Some(name));
                }
            }
            other => {
                let derived_kind = field_name.and_then(shardloom_hidden_derived_column_kind);
                if let Some(kind) = derived_kind {
                    self.derived_columns += 1;
                    match kind {
                        "utf8_length" => self.derived_utf8_length_columns += 1,
                        "url_domain" => self.derived_url_domain_columns += 1,
                        "extract_minute" | "date_trunc_minute" => self.derived_time_columns += 1,
                        _ => self.derived_unknown_columns += 1,
                    }
                    return;
                }
                self.source_columns += 1;
                match other {
                    DType::Utf8(_) => self.source_utf8_columns += 1,
                    DType::Primitive(
                        PType::U8
                        | PType::U16
                        | PType::U32
...
fn read_prepared_vortex_artifact_layout_inventory(
    ...
    let layout_size_attribution = vortex_layout_size_attribution(
        footer.dtype(),
        artifact_size_bytes,
        approx_footer_bytes,
        &layout_encoding_inventory,
...
#[cfg(feature = "vortex-write")]
fn should_defer_prepared_vortex_artifact_layout_inventory(
    row_count: u64,
    bytes_written: u64,
    layout_write_decision: &VortexLayoutWriteRuntimeDecision,
) -> bool {
    vortex_layout_write_strategy_applies(layout_write_decision)
        && (row_count >= VORTEX_PREPARED_OLAP_WRITER_LARGE_SOURCE_ROW_THRESHOLD
            || bytes_written >= VORTEX_PREPARED_OLAP_LARGE_REOPEN_DEFER_BYTES)
}

#[cfg(feature = "vortex-write")]
fn prepared_vortex_artifact_layout_inventory_after_write(
    path: &Path,
    row_count: u64,
    bytes_written: u64,
    layout_write_decision: &VortexLayoutWriteRuntimeDecision,
    ...
    if should_defer_prepared_vortex_artifact_layout_inventory(
        ...
        return Ok((
            ...
            0,
            VORTEX_PREPARED_OLAP_REOPEN_DEFERRED_STATUS.to_string(),
        ));
    }
...
#[cfg(feature = "vortex-write")]
fn reopen_verification_uses_writer_summary(status: &str) -> bool {
    status == VORTEX_PREPARED_OLAP_REOPEN_DEFERRED_STATUS
}
...
mod tests {
    ...
    fn local_flat_scalar_rows_write_and_reopen_vortex_artifact() {
        ...
        assert_eq!(
            report.artifact_digest_source,
            "workspace_safe_streaming_sha256_digest"
        );
    ...
    #[test]
    fn large_writer_summary_defers_layout_inventory_until_query_open() {
        let path = std::env::temp_dir().join(format!(
            "shardloom-vortex-ingest-large-deferred-inventory-{}-{}.vortex",
            std::process::id(),
            1
        ));
        let _ = std::fs::remove_file(&path);
        std::fs::write(&path, b"placeholder").expect("write placeholder artifact");
        ...
        advisor_input.source_byte_count = VORTEX_PREPARED_OLAP_LARGE_REOPEN_DEFER_BYTES;
        ...
        assert!(should_defer_prepared_vortex_artifact_layout_inventory(
            VORTEX_PREPARED_OLAP_WRITER_LARGE_SOURCE_ROW_THRESHOLD,
            VORTEX_PREPARED_OLAP_LARGE_REOPEN_DEFER_BYTES,
            &decision
        ));
        let inventory = VortexPreparedOlapLayoutInventory::deferred_from_writer_summary(
            &path,
            VORTEX_PREPARED_OLAP_WRITER_LARGE_SOURCE_ROW_THRESHOLD,
            VORTEX_PREPARED_OLAP_LARGE_REOPEN_DEFER_BYTES,
            &decision,
        ...
        assert_eq!(
            inventory.row_count,
            Some(VORTEX_PREPARED_OLAP_WRITER_LARGE_SOURCE_ROW_THRESHOLD)
        );
        assert_eq!(
            inventory.layout_reader_cache_status,
        ...
        assert!(reopen_verification_uses_writer_summary(
            VORTEX_PREPARED_OLAP_REOPEN_DEFERRED_STATUS
        ));
    ...
    fn local_flat_columnar_source_writes_and_reopens_vortex_artifact() {
        ...
        assert_eq!(report.stream_source_pull_micros, 0);
        assert_eq!(report.stream_array_convert_micros, 0);
        assert_eq!(
            report.stream_timing_split_status,
            "not_streaming_buffered_or_scalar_vortex_prepare"
        );
        assert_eq!(
    ...
    fn local_flat_columnar_stream_source_writes_without_buffered_batch_source() {
        ...
        assert!(
            ...
            "streaming source pull timing should be recorded"
        );
        assert!(
    ...
    fn prepared_olap_state_report_admits_single_vortex_artifact_without_sidecars() {
        ...
        assert!(report.layout_artifact_size_bytes != "unknown");
        assert!(report.layout_footer_approx_bytes != "unknown");
        assert!(report.layout_size_attribution.contains("artifact_bytes="));
        assert!(report.layout_size_attribution.contains("source_columns="));
        assert!(report.layout_size_attribution.contains("derived_columns="));
        assert!(report.layout_size_attribution.contains("footer_bytes="));
        assert!(
            report
                .layout_size_attribution
                .contains("column_byte_attribution=not_available_from_current_vortex_footer")
        );
    ...
    fn prepared_olap_state_admits_plain_vortex_artifact_with_embedded_layout_status() {
        ...
        assert_eq!(report.layout_footer_row_count, "2");
        assert_ne!(report.layout_artifact_size_bytes, "unknown");
