...
/// `.vortex` sources.
///
/// The inputs are processed sequentially by default and merged inside
/// `ShardLoom`'s local primitive runtime. This is a native partition binding,
/// not an external-engine fallback or a parallel query-engine integration.
///
/// # Errors
/// Returns an error only when internal report construction fails.
pub fn execute_vortex_local_partitioned_primitive_with_policy(
    request: &VortexQueryPrimitiveRequest,
...
#[cfg(feature = "vortex-local-primitives")]
#[allow(clippy::too_many_lines)]
#[cfg_attr(not(feature = "universal-format-io"), allow(clippy::unnecessary_wraps))]
...
#[cfg(feature = "vortex-local-primitives")]
fn row_export_selected_columns_from_chunk(
    chunk: &vortex::array::ArrayRef,
    declared_columns: &[String],
    row_indices: &[usize],
) -> Result<Vec<Vec<StatValue>>> {
    ...
    if out.iter().any(|values| values.len() != row_indices.len()) {
        return Err(ShardLoomError::InvalidOperation(
...
#[cfg(feature = "vortex-local-primitives")]
fn row_export_selected_values_from_vortex_array(
    column: &str,
    array: &vortex::array::ArrayRef,
    row_indices: &[usize],
) -> Result<Vec<StatValue>> {
    selected_stat_values_from_vortex_array(array, row_indices).ok_or_else(|| {
        ShardLoomError::InvalidOperation(format!(
            "local Vortex selected row export column '{column}' has unsupported dtype or nullable validity for scoped selected materialization; no fallback execution was attempted"
        ))
    })
}
    ...
    match predicate {
        ...
        PredicateExpr::IsNull { column } | PredicateExpr::IsNotNull { column } => {
            ...
            let current = column_values[column_index].get(row_index).ok_or_else(|| {
                ShardLoomError::InvalidOperation(
                    "local Vortex materialized null predicate row index was out of bounds; no fallback execution was attempted"
                        .to_string(),
                )
            })?;
            let is_null = matches!(current, StatValue::Null);
            Ok(matches!(predicate, PredicateExpr::IsNull { .. }) == is_null)
        }
        PredicateExpr::And(predicates) => {
...
#[cfg(feature = "vortex-local-primitives")]
#[derive(Clone)]
enum MaterializedPredicateEvaluator {
    AlwaysTrue,
    AlwaysFalse,
    IsNull {
        column_index: usize,
    ...
    And(Vec<MaterializedPredicateEvaluator>),
    Compare {
        column: String,
        column_index: usize,
        op: ComparisonOp,
...
fn fast_utf8_contains_count_from_dictionary_array(
    ...
    for code in codes {
        let Some(&matched) = dictionary_matches.get(usize::try_from(code).map_err(|error| {
            ShardLoomError::InvalidOperation(format!(
                "local Vortex dictionary code overflowed usize during UTF-8 contains count: {error}"
            ))
        })?) else {
            return Ok(None);
        };
        if matched {
            selected = selected.checked_add(1).ok_or_else(|| {
                ShardLoomError::InvalidOperation(
                    "local Vortex dictionary UTF-8 contains selected row count overflowed usize"
                        .to_string(),
                )
            })?;
        }
...
fn coerce_rewrite_value(target_value: &StatValue, value: &StatValue) -> Result<StatValue> {
    coerce_rewrite_value_for_column(None, target_value, value)
}

#[cfg(feature = "vortex-local-primitives")]
#[allow(clippy::cast_precision_loss)]
fn coerce_rewrite_value_for_column(
    column: Option<&str>,
    target_value: &StatValue,
    value: &StatValue,
) -> Result<StatValue> {
    match (target_value, value) {
        ...
        (StatValue::UInt64(_), StatValue::Utf8(value)) if column_allows_iso_date_literal(column) => {
            let value = parse_iso_date32(value).and_then(|value| {
                u64::try_from(value).map_err(|_| {
                    ShardLoomError::InvalidOperation(
                        "local Vortex expression projection negative date literal cannot target unsigned column; no fallback execution was attempted"
                            .to_string(),
                    )
                })
            })?;
        ...
        (StatValue::Float64(_), StatValue::Utf8(value)) if column_allows_iso_date_literal(column) => {
            Ok(StatValue::Float64(f64::from(parse_iso_date32(value)?)))
        }
        (StatValue::Int64(_), StatValue::Utf8(value)) if column_allows_iso_date_literal(column) => {
            Ok(StatValue::Int64(i64::from(parse_iso_date32(value)?)))
        }
        ...
        VortexQueryPrimitiveKind::CountWhere | VortexQueryPrimitiveKind::FilterPredicate => {
            ...
            let scan = read_local_vortex_scan(uri, &path, request.kind, policy, |dtype| {
                ...
                if request.kind == VortexQueryPrimitiveKind::CountWhere {
                    project_count_where_predicate_columns(&mut plan, predicate, dtype);
                }
...
fn execute_vortex_local_partitioned_primitive_enabled(
    ...
    if source_uris.len() == 1 {
        ...
        single.source_uri = source_uris.first().cloned();
    ...
    match request.kind {
        ...
        VortexQueryPrimitiveKind::SimpleAggregate => {
            let aggregate =
                read_local_vortex_simple_aggregate_partitioned_scan(&sources, request, policy)?;
            Ok(partitioned_report(
                simple_aggregate_report(request, &aggregate)?,
                source_uris.len(),
            )?)
        ...
        VortexQueryPrimitiveKind::SortRows => {
            let rows = read_local_vortex_sort_rows_partitioned_scan(&sources, request, policy)?;
            Ok(partitioned_report(
                sort_rows_report(request, &rows)?,
                source_uris.len(),
            )?)
        ...
        _ => Ok(VortexLocalPrimitiveExecutionReport::blocked(
            ...
            Diagnostic::unsupported(
                ...
                "vortex_local_partitioned_primitive",
                format!(
                    "partitioned local Vortex primitive execution does not yet support {}; no fallback execution was attempted",
                    request.kind.as_str()
                ),
...
#[cfg(feature = "vortex-local-primitives")]
fn partitioned_report(
    mut report: VortexLocalPrimitiveExecutionReport,
    source_count: usize,
) -> Result<VortexLocalPrimitiveExecutionReport> {
    report.result_summary = report.result_summary.map(|summary| {
        format!(
            "partitioned_vortex_sources={source_count};partitioned_execution_strategy=sequential_capillary_parts;{summary}"
        )
    });
    if report.state_budget.state_budget_required {
        if !report
            .state_budget
            .capillary_work_units
            .iter()
            .any(|unit| unit == "partitioned_vortex_source")
        {
            report
                .state_budget
                .capillary_work_units
                .insert(0, "partitioned_vortex_source".to_string());
        }
        if !report
            .state_budget
            .pulseweave_pressure_signals
            .iter()
            .any(|signal| signal == "partition_count")
        {
            report
                .state_budget
                .pulseweave_pressure_signals
                .insert(0, "partition_count".to_string());
        }
        report.state_budget.observed_state_items = report
            .state_budget
            .observed_state_items
            .checked_add(usize_to_u64(source_count)?)
            .ok_or_else(|| {
                ShardLoomError::InvalidOperation(
                    "partitioned local Vortex observed state count overflowed u64; no fallback execution was attempted"
                        .to_string(),
                )
            })?;
    }
...
#[cfg(feature = "vortex-local-primitives")]
struct LocalVortexPartitionSource {
    uri: DatasetUri,
    path: std::path::PathBuf,
}

#[cfg(feature = "vortex-local-primitives")]
fn partitioned_local_vortex_source_paths(
    source_uris: &[DatasetUri],
    primitive_kind: VortexQueryPrimitiveKind,
) -> Result<Vec<LocalVortexPartitionSource>> {
    source_uris
        .iter()
        .map(|uri| {
            let Some(path) = local_vortex_path(uri, primitive_kind)? else {
                return Err(ShardLoomError::InvalidOperation(format!(
                    "unsupported partitioned local Vortex primitive target: {}; provide existing local `.vortex` files only; no fallback execution was attempted",
                    uri.as_str()
                )));
            };
            Ok(LocalVortexPartitionSource {
                uri: uri.clone(),
                path,
            })
        })
...
enum ResidualPredicateMaterialization {
    None,
    PlannedNoMaterialization,
    Materialized,
}

#[cfg(feature = "vortex-local-primitives")]
impl ResidualPredicateMaterialization {
    const fn from_flags(applied: bool, materialized: bool) -> Self {
        match (applied, materialized) {
            (false, _) => Self::None,
            (true, false) => Self::PlannedNoMaterialization,
            (true, true) => Self::Materialized,
        }
    }
...
#[cfg(feature = "vortex-local-primitives")]
impl LocalVortexScan {
    fn upstream_filter_expression_used(&self) -> bool {
        self.filter_pushdown_applied
    }
...
fn row_export_state_budget_report(
    ...
    match request.kind {
        ...
        VortexQueryPrimitiveKind::DistinctRows => {
            Ok(VortexLocalPrimitiveStateBudgetReport::bounded_in_memory(
                "row_key_distinct_state+hash_row_key",
                vec![
                    "vortex_scan",
                    "row_key_state",
                    "hash_row_key_state",
        ...
        VortexQueryPrimitiveKind::DropDuplicateRows => {
            Ok(VortexLocalPrimitiveStateBudgetReport::bounded_in_memory(
                "retained_row_deduplicate_state+hash_row_key",
                vec![
                    "vortex_scan",
                    "row_key_state",
                    "hash_row_key_state",
        ...
        VortexQueryPrimitiveKind::DuplicateMaskRows => {
            Ok(VortexLocalPrimitiveStateBudgetReport::bounded_in_memory(
                "duplicate_mask_row_key_state+hash_row_key",
                vec![
                    "vortex_scan",
                    "row_key_state",
                    "hash_row_key_state",
...
fn read_local_vortex_scan(
    ...
    let source_row_count = file.row_count();
    let mut plan = configure(file.dtype())?;
    if plan.source_order_limit == Some(0) {
        return Err(ShardLoomError::InvalidOperation(
...
#[cfg(feature = "vortex-local-primitives")]
#[allow(clippy::too_many_lines)]
fn read_local_vortex_partitioned_scan(
    ...
    for source in sources {
        ...
        source_row_count = source_row_count
            ...
            .ok_or_else(|| {
                ShardLoomError::InvalidOperation(
                    "partitioned local Vortex source row count overflowed u64; no fallback execution was attempted"
                        .to_string(),
                )
            })?;
        let mut plan = configure(file.dtype())?;
        ...
        match (source_order_limit, plan.source_order_limit) {
            ...
            (Some(existing), Some(value)) if existing != value => {
                return Err(ShardLoomError::InvalidOperation(format!(
                    "partitioned local Vortex scan saw inconsistent source-order limits {existing} and {value}; no fallback execution was attempted"
                )));
            }
            _ => {}
        }
        match &projected_columns {
            None => projected_columns = Some(plan.projected_columns.clone()),
            Some(existing) if existing != &plan.projected_columns => {
                return Err(ShardLoomError::InvalidOperation(
                    "partitioned local Vortex scan saw inconsistent projected columns; no fallback execution was attempted"
                        .to_string(),
                ));
            }
            _ => {}
        }
        let current_output_columns = plan
            .output_columns
            .clone()
            .unwrap_or_else(|| plan.projected_columns.clone());
        match &output_columns {
            None => output_columns = Some(current_output_columns),
            Some(existing) if existing != &current_output_columns => {
                return Err(ShardLoomError::InvalidOperation(
                    "partitioned local Vortex scan saw inconsistent output columns; no fallback execution was attempted"
                        .to_string(),
                ));
            }
            _ => {}
        ...
        for chunk in scan.into_array_iter(&runtime).map_err(vortex_error)? {
                    ...
                    predicate.fast_count_matches_in_chunk(&chunk, &plan.projected_columns)?
                {
                    ...
                    for row_index in 0..materialized_rows {
                        if predicate.matches(&columns, row_index)? {
                            selected = selected.checked_add(1).ok_or_else(|| {
                                ShardLoomError::InvalidOperation(
                                    "partitioned local Vortex primitive residual selected row count overflowed usize"
                                        .to_string(),
                                )
                            })?;
                        }
            ...
            pre_limit_result_row_count = pre_limit_result_row_count
                ...
                .ok_or_else(|| {
                    ShardLoomError::InvalidOperation(
                        "partitioned local Vortex primitive pre-limit result row count overflowed usize"
                            .to_string(),
                    )
                })?;
            let output_rows = source_order_limit.map_or(chunk_result_rows, |limit| {
            ...
            result_row_count = result_row_count.checked_add(output_rows).ok_or_else(|| {
                ShardLoomError::InvalidOperation(
                    "partitioned local Vortex primitive result row count overflowed usize"
                        .to_string(),
                )
            })?;
            max_chunk_rows = max_chunk_rows.max(rows);
...
#[cfg(feature = "vortex-local-primitives")]
fn selected_stat_values_from_vortex_array(
    array: &vortex::array::ArrayRef,
    row_indices: &[usize],
    ...
    if let Some(values) =
        direct_selected_dictionary_stat_values_from_vortex_array(array, row_indices)
    {
        return Some(values);
    }
    if let Some(values) =
        direct_selected_non_nullable_stat_values_from_vortex_array(array, row_indices)
    {
        return Some(values);
    }
    let values = stat_values_from_vortex_array(array)?;
    row_indices
        .iter()
        .map(|&row_index| values.get(row_index).cloned())
        .collect()
}

#[cfg(feature = "vortex-local-primitives")]
fn direct_non_nullable_stat_values_from_vortex_array(
    array: &vortex::array::ArrayRef,
) -> Option<Vec<StatValue>> {
    use vortex::array::dtype::DType;

    match array.dtype() {
        DType::Primitive(_, _) => direct_non_nullable_host_primitive(array)
            .and_then(|primitive| primitive_stat_values_from_primitive_array(&primitive)),
        DType::Utf8(_) => direct_non_nullable_utf8_stat_values_from_vortex_array(array),
        _ => None,
    }
}

#[cfg(feature = "vortex-local-primitives")]
fn direct_selected_dictionary_stat_values_from_vortex_array(
    array: &vortex::array::ArrayRef,
    row_indices: &[usize],
) -> Option<Vec<StatValue>> {
    use vortex::array::arrays::dict::DictArraySlotsExt as _;
    ...
    row_indices
        .iter()
        .map(|&row_index| {
            let code = *codes.get(row_index)?;
            dictionary.get(usize::try_from(code).ok()?).cloned()
        })
        .collect()
}

#[cfg(feature = "vortex-local-primitives")]
fn direct_selected_non_nullable_stat_values_from_vortex_array(
    array: &vortex::array::ArrayRef,
    row_indices: &[usize],
) -> Option<Vec<StatValue>> {
    use vortex::array::dtype::DType;
    ...
    match array.dtype() {
        DType::Primitive(_, _) => direct_non_nullable_host_primitive(array)
            .and_then(|primitive| selected_primitive_stat_values(&primitive, row_indices)),
        DType::Utf8(_) => selected_direct_non_nullable_utf8_stat_values(array, row_indices),
        _ => None,
    }
}
...
#[cfg(feature = "vortex-local-primitives")]
fn selected_primitive_stat_values(
    ...
    match primitive.ptype() {
        PType::U8 => selected_from_slice(primitive.as_slice::<u8>(), row_indices, |value| {
            StatValue::UInt64(u64::from(value))
        }),
        PType::U16 => selected_from_slice(primitive.as_slice::<u16>(), row_indices, |value| {
            StatValue::UInt64(u64::from(value))
        }),
        PType::U32 => selected_from_slice(primitive.as_slice::<u32>(), row_indices, |value| {
            StatValue::UInt64(u64::from(value))
        }),
        PType::U64 => {
            selected_from_slice(primitive.as_slice::<u64>(), row_indices, StatValue::UInt64)
        }
        PType::I8 => selected_from_slice(primitive.as_slice::<i8>(), row_indices, |value| {
            StatValue::Int64(i64::from(value))
        }),
        PType::I16 => selected_from_slice(primitive.as_slice::<i16>(), row_indices, |value| {
            StatValue::Int64(i64::from(value))
        }),
        PType::I32 => selected_from_slice(primitive.as_slice::<i32>(), row_indices, |value| {
            StatValue::Int64(i64::from(value))
        }),
        PType::I64 => {
            selected_from_slice(primitive.as_slice::<i64>(), row_indices, StatValue::Int64)
        }
        PType::F16 => None,
        PType::F32 => selected_from_slice(primitive.as_slice::<f32>(), row_indices, |value| {
            StatValue::Float64(f64::from(value))
        }),
        PType::F64 => {
            selected_from_slice(primitive.as_slice::<f64>(), row_indices, StatValue::Float64)
        }
    }
}

#[cfg(feature = "vortex-local-primitives")]
fn selected_from_slice<T: Copy>(
    values: &[T],
    row_indices: &[usize],
    convert: impl Fn(T) -> StatValue,
) -> Option<Vec<StatValue>> {
    row_indices
        .iter()
        .map(|&row_index| values.get(row_index).copied().map(&convert))
        .collect()
}
...
#[cfg(feature = "vortex-local-primitives")]
fn selected_direct_non_nullable_utf8_stat_values(
    array: &vortex::array::ArrayRef,
    row_indices: &[usize],
) -> Option<Vec<StatValue>> {
    use vortex::array::arrays::varbinview::VarBinView;
    ...
    let utf8 = array.as_opt::<VarBinView>()?;
    match utf8.varbinview_validity() {
        Validity::NonNullable | Validity::AllValid => {}
        Validity::AllInvalid | Validity::Array(_) => return None,
    }
...
#[cfg(feature = "vortex-local-primitives")]
fn direct_non_nullable_utf8_stat_values_from_vortex_array(
    array: &vortex::array::ArrayRef,
) -> Option<Vec<StatValue>> {
    use vortex::array::VortexSessionExecute as _;
    use vortex::array::arrays::VarBinViewArray;
...
#[cfg(feature = "vortex-local-primitives")]
fn direct_non_nullable_u32_codes_from_vortex_array(
    array: &vortex::array::ArrayRef,
) -> Option<Vec<u32>> {
    direct_non_nullable_host_primitive(array)
        .and_then(|primitive| primitive_u32_codes_from_primitive_array(&primitive))
}
...
fn direct_non_nullable_u64_values_from_vortex_array(
    ...
    direct_non_nullable_host_primitive(array)
        .and_then(|primitive| primitive_u64_values_from_primitive_array(&primitive))
}
...
#[cfg(feature = "vortex-local-primitives")]
fn count_all_metadata_report(
    _source_uri: &DatasetUri,
    path: &std::path::Path,
    policy: VortexLocalPrimitiveExecutionPolicy,
) -> Result<VortexLocalPrimitiveExecutionReport> {
    use vortex::VortexSessionDefault as _;
...
fn distinct_rows_report(
    ...
    Ok(VortexLocalPrimitiveExecutionReport {
        ...
        state_budget: VortexLocalPrimitiveStateBudgetReport::bounded_in_memory(
            "row_key_distinct_state+hash_row_key",
            vec!["vortex_scan", "hash_row_key_state"],
            vec!["distinct_key_cardinality", "input_rows"],
...
fn duplicate_mask_rows_report(
    ...
    Ok(VortexLocalPrimitiveExecutionReport {
        ...
        result_summary: Some(format!(
            "duplicate_mask_rows={} subset_columns={} output_columns=duplicated keep={} row_key_state_strategy=hash_row_key_exact",
            rows,
            scan.projected_columns.join(","),
            request.duplicate_keep.as_str()
        )),
        ...
        state_budget: VortexLocalPrimitiveStateBudgetReport::bounded_in_memory(
            "duplicate_mask_row_key_state+hash_row_key",
            vec!["vortex_scan", "row_key_state", "hash_row_key_state"],
            vec!["duplicate_key_cardinality", "input_rows"],
            usize_to_u64(scan.pre_limit_result_row_count)?,
            Some(scan.source_row_count),
...
fn read_local_vortex_simple_aggregate_scan(
    ...
    for chunk in scan.into_array_iter(&runtime).map_err(vortex_error)? {
        ...
        if let Some(predicate) = residual_evaluator.as_ref() {
                ...
                && row_indices.len() < rows
            {
                ...
                for row_index in 0..row_indices.len() {
                    ...
                    if let Some(states) = grouped_states.as_mut()
                        && !states.update_count_star_direct_from_materialized_columns(
                            &columns, row_index,
                        )?
                    {
                        states.update_row(&columns, row_index)?;
            ...
            let direct_count_star_updated = if let Some(states) = grouped_states.as_mut() {
                states.update_count_star_direct_from_chunk(&chunk, &declared_columns, None)?
            } else {
                false
            };
            let materialized_rows = if direct_count_star_updated {
                rows
            } else {
                let columns = row_export_columns_from_chunk(&chunk, &declared_columns)?;
                let materialized_rows = if columns.is_empty() {
...
fn read_local_vortex_simple_aggregate_partitioned_scan(
    ...
    let declared_columns =
        local_vortex_simple_aggregate_declared_columns(&sources[0].path, request)?;
    let aggregate_has_grouping =
        !aggregate.group_by.is_empty() || !aggregate.group_expressions.is_empty();
    let mut scalar_states = if aggregate_has_grouping {
        None
...
#[cfg(feature = "vortex-local-primitives")]
fn local_vortex_simple_aggregate_declared_columns(
    path: &std::path::Path,
    request: &VortexQueryPrimitiveRequest,
) -> Result<Vec<String>> {
    use vortex::VortexSessionDefault as _;
    ...
    let mut projected_columns = aggregate.projected_columns();
    let (_pushdown_predicate, residual_predicate) = request
        .predicate
        .as_ref()
        .map_or((None, None), split_predicate_for_vortex_pushdown);
    if let Some(predicate) = residual_predicate.as_ref() {
    ...
    Ok(if plan.projected_columns.is_empty() {
        Vec::new()
    } else {
...
fn read_local_vortex_sort_rows_scan(
    ...
    let (candidate_pushdown_predicate, candidate_residual_predicate) = request
        ...
        .as_ref()
        .map_or((None, None), split_predicate_for_vortex_pushdown);
    let wide_output_second_pass =
        output_columns.len() > 8 && sort_rows.tie_policy != VortexSortTiePolicy::All;
    let force_residual_predicate_for_source_ordinals =
        wide_output_second_pass && request.predicate.is_some();
...
struct GroupedAggregateOrderCandidate {
    ...
    order_values: Vec<serde_json::Value>,
    tie_breaker: String,
}
...
impl<'a> GroupedAggregateStates<'a> {
    fn new(
        ...
        Ok(Self {
            ...
            group_columns,
            group_key_indices,
            state_template: SimpleAggregateStates::new(request, declared_columns)?,
        ...
        let entry = match self.groups.entry(key) {
            ...
            std::collections::hash_map::Entry::Vacant(entry) => {
                ...
                entry.insert(GroupedAggregateState {
                    group_values,
                    states: self.state_template.clone(),
                })
            }
        };
    ...
    fn update_count_star_direct_from_materialized_columns(
        ...
        if let Some(group) = self.groups.get_mut(&key) {
            group.states.increment_count_star_by(1)?;
            self.count_star_direct_updates = true;
            return Ok(true);
        }
        let group_values = self.group_values_for_materialized_row(columns, row_index)?;
        self.groups.insert(
            key.clone(),
        ...
        let group = self.groups.get_mut(&key).ok_or_else(|| {
            ShardLoomError::InvalidOperation(
                "local Vortex grouped aggregate count-star direct materialized group was not found; no fallback execution was attempted"
                    .to_string(),
            )
        })?;
        group.states.increment_count_star_by(1)?;
    ...
    fn update_count_star_direct_row(
        ...
        let group_values = self.group_values_for_direct_row(accessors, row_index)?;
        self.groups.insert(
            key.clone(),
            GroupedAggregateState {
                group_values,
        ...
        let group = self.groups.get_mut(&key).ok_or_else(|| {
            ShardLoomError::InvalidOperation(
                "local Vortex grouped aggregate count-star direct group was not found; no fallback execution was attempted"
                    .to_string(),
            )
        })?;
        group.states.increment_count_star_by(1)
    ...
    fn grouped_key_for_materialized_row(
        ...
        self.group_key_indices
            ...
            .map(|index| {
                ...
                Ok(AggregateDistinctValue::from(&value))
            })
            .collect::<Result<Vec<_>>>()
            .map(AggregateGroupKey)
    }
    ...
    fn update_single_group_fast_path(
        ...
        if let Some(group) = self.groups.get_mut(&key) {
            group.states.update_row(columns, row_index)?;
            return Ok(true);
        }
        ...
        let group = self.groups.get_mut(&key).ok_or_else(|| {
            ShardLoomError::InvalidOperation(
                "local Vortex grouped aggregate fast-path inserted group was not found; no fallback execution was attempted"
                    .to_string(),
            )
        })?;
        group.states.update_row(columns, row_index)?;
    ...
    fn result_row_count_and_summary(&self, limit: Option<usize>) -> Result<(usize, String)> {
        ...
        let rows = candidates
            ...
            .map(|candidate| {
                ...
                self.result_row_for_group(group).map(serde_json::Value::Object)
            })
            .collect::<Result<Vec<_>>>()?;
        let functions = self
            .groups
    ...
    fn source_order_result_row_count_and_summary(
        ...
        for key in &self.group_order {
            ...
            let row = self.result_row_for_group(group)?;
    ...
    fn ordered_candidates(&self) -> Result<Vec<GroupedAggregateOrderCandidate>> {
        ...
        for (key, group) in &self.groups {
            if !self.group_matches_having(group)? {
                continue;
            }
            let order_values = self
                ...
                .order_by
                .iter()
                .map(|order| self.result_value_for_group_column(group, &order.column))
                .collect::<Result<Vec<_>>>()?;
            candidates.push(GroupedAggregateOrderCandidate {
                key: key.clone(),
                order_values,
                tie_breaker: key.tie_breaker(),
            });
        }
        Ok(candidates)
    }
    ...
    fn group_matches_having(&self, group: &GroupedAggregateState) -> Result<bool> {
        for expression in &self.request.having {
            let left = self.result_value_for_group_column(group, &expression.column)?;
    ...
    fn result_value_for_group_column(
        ...
        if let Some(value) = group.states.result_value_for_alias(column)? {
            return Ok(value);
        }
        if let Some(value) = self
            .group_columns
            .iter()
            .zip(&group.group_values)
    ...
    fn compare_ordered_candidates(
        ...
        left.tie_breaker.cmp(&right.tie_breaker)
    }
...
struct AggregateGroupKey(Vec<AggregateDistinctValue>);
...
impl AggregateGroupKey {
    ...
    fn tie_breaker(&self) -> String {
        self.0
            .iter()
            .map(AggregateDistinctValue::tie_breaker)
            .collect::<Vec<_>>()
            .join("\u{1f}")
    }
}
...
#[cfg(feature = "vortex-local-primitives")]
fn stat_value_utf8_group_key(value: &str) -> String {
    format!("s:{}", value.replace('\u{1f}', "\\u001f"))
}
...
fn aggregate_direct_column_accessor(
    ...
    if let Some(utf8) = direct_non_nullable_host_utf8(array) {
        let mut values = Vec::with_capacity(utf8.len());
        ...
        return Ok(AggregateDirectColumnAccessor::Utf8(values));
    }
...
#[cfg(feature = "vortex-local-primitives")]
fn direct_non_nullable_host_utf8(
    array: &vortex::array::ArrayRef,
) -> Option<vortex::array::ArrayView<'_, vortex::array::arrays::varbinview::VarBinView>> {
    use vortex::array::arrays::varbinview::VarBinView;
    use vortex::array::arrays::varbinview::VarBinViewArrayExt as _;
    use vortex::array::validity::Validity;
    ...
    let utf8 = array.as_opt::<VarBinView>()?;
    match utf8.varbinview_validity() {
        Validity::NonNullable | Validity::AllValid => Some(utf8),
        Validity::AllInvalid | Validity::Array(_) => None,
    }
}
...
fn aggregate_group_column_distinct_value_for_direct_row(
        ...
        && matches!(group_column.transform, AggregateValueTransform::Identity)
    {
        ...
        return aggregate_direct_distinct_value(accessor, row_index);
    }
    let value =
        aggregate_group_column_stat_value_for_direct_row(group_column, accessors, row_index)?;
    Ok(AggregateDistinctValue::from(&value))
...
fn aggregate_direct_stat_value(
    ...
    match accessor {
        ...
        AggregateDirectColumnAccessor::Utf8(values) => values
            .get(row_index)
            .cloned()
            .map(StatValue::Utf8)
            .ok_or_else(|| {
...
fn aggregate_direct_distinct_value(
    ...
    match accessor {
        ...
        AggregateDirectColumnAccessor::Utf8(values) => values
            .get(row_index)
            .cloned()
            .map(AggregateDistinctValue::Utf8)
            .ok_or_else(|| {
                ShardLoomError::InvalidOperation(
...
fn aggregate_url_domain_str(value: &str) -> &str {
    ...
    without_www.split('/').next().unwrap_or_default()
}
...
impl AggregateDistinctValue {
    fn tie_breaker(&self) -> String {
        match self {
            ...
            Self::Int64(value) => format!("i:{value}"),
            Self::UInt64(value) => format!("u:{value}"),
            Self::Float64Bits(value) => format!("f:{value:016x}"),
            Self::Utf8(value) => stat_value_utf8_group_key(value),
        }
    }
}
...
mod tests {
    ...
    fn distinct_rows_materialize_bounded_vortex_rows_without_fallback() {
        ...
        assert!(summary.contains("distinct_rows=2"));
        assert!(summary.contains("projected_columns=value"));
        assert!(summary.contains("distinct_state_strategy=hash_row_key_exact"));
        assert!(
            report
                .state_budget
                .state_family
                .contains("row_key_distinct_state+hash_row_key")
        );
        assert!(
            report
                .state_budget
    ...
    fn local_primitive_certificates_cover_broader_runtime_fixtures() {
        ...
        for (fixture_id, mut request) in cases {
            ...
            assert_eq!(
                ...
                fixture_id != "vortex-local-count-all-struct-five",
                "{fixture_id}"
            );
    ...
    fn simple_aggregate_skips_null_measures_without_fallback() {
        ...
        let request = VortexQueryPrimitiveRequest::simple_aggregate(
            ...
            VortexSimpleAggregateRequest::new(vec![
                ...
                crate::VortexSimpleAggregateMeasure::new(
                    ...
                    "non_null_amount".to_string(),
                ),
                crate::VortexSimpleAggregateMeasure::new(
                    "count_distinct",
                    Some(ColumnRef::new("amount").expect("column")),
                    "unique_amount".to_string(),
                ),
                crate::VortexSimpleAggregateMeasure::new(
                    "sum",
                    Some(ColumnRef::new("amount").expect("column")),
        ...
        let values = payload["values"].as_object().expect("values");
        assert_eq!(payload["rows"], serde_json::json!(1));
        assert_eq!(values["rows"], serde_json::json!(3));
        assert_eq!(values["non_null_amount"], serde_json::json!(0));
        assert_eq!(values["unique_amount"], serde_json::json!(0));
        assert!(values["sum_amount"].is_null());
        assert!(values["avg_amount"].is_null());
        assert!(values["min_amount"].is_null());
        assert!(values["max_amount"].is_null());
    }
    ...
    fn partitioned_local_primitive_counts_sources_without_fallback() {
        ...
        assert!(summary.ends_with(";10"));
    }

    #[test]
    fn partitioned_local_primitive_aggregate_uses_one_state_without_fallback() {
        let left_path = unique_vortex_path("partitioned-aggregate-left");
        let right_path = unique_vortex_path("partitioned-aggregate-right");
        write_struct_fixture(&left_path).expect("left fixture");
        write_struct_fixture(&right_path).expect("right fixture");
        let source_uris = vec![
            DatasetUri::new(left_path.display().to_string()).expect("left uri"),
        ...
        assert!(summary.contains("\"sum_metric\":300.0"));
        assert!(summary.contains("\"avg_metric\":30.0"));
    }
    ...
    fn grouped_aggregate_filters_orders_and_limits_rows_without_fallback() {
        ...
        assert!(
            report
                .state_budget
                .capillary_work_units
                .contains(&"capillary_ordered_candidate_selection".to_string())
        );
        assert!(
            report
                .state_budget
    ...
    fn grouped_aggregate_source_order_limit_avoids_full_group_sort_without_fallback() {
        ...
        assert_eq!(
            report.state_budget.state_family,
            "grouped_aggregate_state+topk+count_star_direct"
        );
        assert!(
            report
                .state_budget
                .capillary_work_units
                .contains(&"count_star_direct_group_update".to_string())
        );
        assert!(
            report
                .state_budget
    ...
    fn grouped_aggregate_prunes_functionally_dependent_offset_keys_without_dropping_outputs() {
        let path = unique_vortex_path("grouped-aggregate-functional-key");
        write_pivot_struct_fixture(&path).expect("fixture");
        let request = VortexQueryPrimitiveRequest::simple_aggregate(
            DatasetUri::new(path.display().to_string()).expect("uri"),
            VortexSimpleAggregateRequest::grouped(
                vec![ColumnRef::new("amount").expect("column")],
                vec![crate::VortexSimpleAggregateMeasure::new(
                    "count",
                    None,
                    "rows".to_string(),
        ...
        assert!(
            report
                .state_budget
                .capillary_work_units
                .contains(&"functional_dependency_key_pruning".to_string())
        );
        assert!(
            report
                .state_budget
                .pulseweave_pressure_signals
                .contains(&"derived_group_key_width_reduction".to_string())
        );
        let summary = report.result_summary.expect("summary");
        assert!(
            summary.contains(
                "\"aggregate_key_encoding_mode\":\"typed_hash_key_functional_dependency\""
            )
        );
        assert!(summary.contains("\"amount\":10"));
        assert!(summary.contains("\"amount_minus_1\":9"));
        assert!(summary.contains("\"amount_minus_2\":8"));
        assert!(summary.contains("\"amount\":5"));
        assert!(summary.contains("\"amount_minus_1\":4"));
        assert!(summary.contains("\"amount_minus_2\":3"));
    }
