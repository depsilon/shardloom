...
impl Selection {
    pub(super) fn new(memory: &LiveMemoryPool, limit: usize) -> Result<Self> {
        ...
        if rows.capacity() > limit {
            return Err(failed("UTF8 DISTINCT selection exceeded reserved capacity"));
        }
        Ok(Self {
            rows,
            limit,
    ...
    pub(super) fn insert(&mut self, text: &str, count: u64) -> Result<()> {
        if self.limit == 0
            || (self.rows.len() == self.limit
                && self.rows.peek().is_some_and(|worst| {
        ...
        let lease = self
            .memory
            .reserve(u64::try_from(text.len()).map_err(|_| failed("text capacity overflowed"))?)?;
        let mut owned = String::new();
        owned
            .try_reserve_exact(text.len())
            .map_err(|error| failed(&error.to_string()))?;
        if owned.capacity() > text.len() {
            return Err(failed("UTF8 DISTINCT text exceeded reserved capacity"));
        }
        owned.push_str(text);
        let row = Row {
            text: owned,
        ...
        if self.rows.len() == self.limit {
            *self
                .rows
                .peek_mut()
                .ok_or_else(|| failed("UTF8 DISTINCT selection lost worst row"))? = row;
        } else {
            self.rows.push(row);
        }
        Ok(())
    ...
    pub(super) fn finish(self) -> Rows {
        Rows {
            rows: self.rows.into_sorted_vec(),
            lease: self.lease,
        }
    }
}
...
impl Rows {
    pub(super) fn reserved_bytes(&self) -> u64 {
        self.lease.bytes()
            + self
                .rows
                .iter()
                .map(|row| row.text_lease.bytes())
                .sum::<u64>()
    }
    pub(super) fn visit_utf8<'a>(
        &'a self,
        mut visit: impl FnMut(&'a str, u64) -> Result<()>,
    ) -> Result<()> {
        for row in &self.rows {
            visit(&row.text, row.count)?;
        }
        Ok(())
    }
    ...
    pub(super) fn text_bytes(&self) -> usize {
        self.rows.iter().map(|row| row.text.len()).sum()
    }
}
...
mod tests {
    ...
    fn values(rows: &Rows) -> Vec<(String, u64)> {
        ...
        rows.visit_utf8(|key, count| {
            result.push((key.to_owned(), count));
            Ok(())
        })
        .unwrap();
        result
