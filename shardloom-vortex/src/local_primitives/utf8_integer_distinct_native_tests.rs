...
use super::*;
use crate::{VortexAggregateSpillPolicy, local_primitives as runtime, resident_session::ResidentVortexSession};
use runtime::prepared_aggregate::{
    PreparedAggregateDisposition, UnretainedVortexAggregate,
    prepare_aggregate_for_optional_reuse, prepare_aggregate_in_session,
};
use std::{fs, path::PathBuf, sync::atomic::{AtomicU64, Ordering}};
use vortex::{
    VortexSessionDefault as _,
    file::WriteOptionsSessionExt as _,
    io::{runtime::{BlockingRuntime as _, single::SingleThreadRuntime}, session::RuntimeSessionExt as _},
...
impl Fixture {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "shardloom-focused-utf8-distinct-{}-{}", std::process::id(),
    ...
    fn write(&self, name: &str, chunks: Vec<ArrayRef>) -> PathBuf {
        ...
        let mut writer = session.write_options()
            .with_strategy(runtime::native_flat_layout::SequentialNativeFlatLayout::strategy(chunks.len()))
            .with_file_statistics(Vec::new())
            .blocking(&runtime).writer(&mut file, chunks[0].dtype().clone());
        let expected_rows = chunks.iter().map(|array| array.len() as u64).sum::<u64>();
        for array in chunks { writer.push(array).unwrap(); }
...
fn payload(report: &runtime::VortexLocalPrimitiveExecutionReport) -> serde_json::Value {
    ...
    serde_json::from_str(report.result_summary.as_ref().unwrap().rsplit_once(" values=").unwrap().1).unwrap()
}
fn unretained(request: &VortexQueryPrimitiveRequest, parallelism: usize) -> UnretainedVortexAggregate {
    match prepare_aggregate_for_optional_reuse(request, policy(parallelism)).unwrap() {
        ...
        Some(PreparedAggregateDisposition::Reusable(_)) => panic!("focused port must not widen retained UTF8 admission"),
        None => panic!("ordinary UTF8 DISTINCT request must retain its one-shot source disposition"),
    }
}
...
fn utf8_integer_distinct_native_ordinary_unretained_complete_values_and_worker_grants() {
    ...
    for parallelism in [1, 2, 4, 8, 12] {
        for offset in [0, 1, 20] {
            ...
            let report = runtime::execute_vortex_local_primitive_with_policy(&request, policy(parallelism)).unwrap();
            ...
            assert_eq!(report.rows_projected, Some(reference.as_array().unwrap().len() as u64));
            ...
            for _ in 0..2 {
                ...
                assert!(executed.native_io_certificate.source_pushdown_report.proof_basis
...
fn utf8_integer_distinct_native_dictionary_domains_and_slices_keep_complete_values() {
    let fixture = Fixture::new();
    let arrays = vec![
        chunk(dictionary(&[2, 0, 1, 2], &["東京", "", "東京"]), integers(&[i64::MIN, i64::MIN, i64::MAX, 7])),
        chunk(dictionary(&[0, 1, 2, 0, 1], &["unused", "東京", ""]), integers(&[99, 7, i64::MAX, 99, i64::MIN]))
            .slice(1..3).unwrap(),
    ];
...
fn utf8_integer_distinct_native_admission_pressure_and_late_generation_release_exactly() {
    ...
    for pressure in [false, true, false] {
        ...
        let scan = source.with_native_execution(|file, session, runtime| {
            runtime::read_prepared_vortex_simple_aggregate_scan(
                uri, &request, policy(4), file, session, runtime, Some(resident.memory()), None,
            )
        }).unwrap();
        ...
        if pressure {
            ...
            assert!(work["aggregate_provider_cpu_scope"].as_str().unwrap()
...
fn utf8_integer_distinct_native_nullable_schema_keeps_existing_ordinary_route() {
    ...
    let report = runtime::execute_vortex_local_primitive_with_policy(&request, policy(2)).unwrap();
    let work = payload(&report);
    assert!(work.get("aggregate_workers_partition_complete_pairs").is_none());
    let actual = work["values"].as_array().unwrap().iter().map(|row| {
        (row[GROUP].as_str().map(str::to_owned), row[COUNT].as_u64().unwrap())
...
fn utf8_integer_distinct_native_explicit_spill_remains_outside_integer_spill_admission() {
    let fixture = Fixture::new();
    let path = fixture.write("source.vortex", vec![chunk(strings(&["x"]), integers(&[1]))]);
    let workspace = fixture.0.join("spill");
    fs::create_dir(&workspace).unwrap();
    let mut request = query(&path, 0, 3);
    request.simple_aggregate = Some(request.simple_aggregate.take().unwrap()
    ...
    let source = resident.prepare_file(&path).unwrap();
    assert!(!runtime::exact_distinct_pairs::workers::request_schema_may_be_admitted(&request, source.dtype()));
    assert!(prepare_aggregate_for_optional_reuse(&request, policy(1)).unwrap().is_none());
