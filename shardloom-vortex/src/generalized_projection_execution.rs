...
const REPORT_ID: &str = "vortex.cg2.generalized-projection.local-scan-pushdown";

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum VortexGeneralizedProjectionExecutionStatus {
    FeatureDisabled,
...
impl VortexGeneralizedProjectionExecutionStatus {
    #[must_use]
...
pub struct VortexGeneralizedProjectionExecutionReport {
...
impl VortexGeneralizedProjectionExecutionReport {
    fn unsupported(request: &VortexQueryPrimitiveRequest) -> Self {
        ...
        diagnostics.push(Diagnostic::not_implemented(
            ...
            format!(
                "generalized projection execution only supports ProjectColumns and FilterAndProject, got {}",
                request.kind.as_str()
            ),
            "Use ProjectColumns or FilterAndProject while broader encoded projection kernels remain separate work.",
        ));
        Self {
    ...
    fn from_local_report(
        ...
        let safe = generalized_projection_local_scan_pushdown_safe(&local_primitive_report)
            && native_io_safe
            && request
                .diagnostics
                .iter()
        ...
        {
            VortexGeneralizedProjectionExecutionStatus::FeatureDisabled
        } else if safe {
            VortexGeneralizedProjectionExecutionStatus::ExecutedLocalScanPushdown
        } else {
            VortexGeneralizedProjectionExecutionStatus::BlockedUnsafeEvidence
        };
        let mut diagnostics = request.diagnostics.clone();
        ...
        let selection_vector_guaranteed =
            safe && matches!(
                local_primitive_report.primitive_kind,
                VortexQueryPrimitiveKind::FilterAndProject
            ) && local_primitive_report.rows_selected.is_some();
        Self {
            schema_version: SCHEMA_VERSION,
            report_id: REPORT_ID.to_string(),
            primitive_kind: request.kind,
            predicate_summary: request.predicate.as_ref().map(PredicateExpr::summary),
            projected_columns: local_primitive_report.projected_columns.clone(),
            status,
    ...
    pub fn to_human_text(&self) -> String {
        ...
        let _ = writeln!(&mut out, "Vortex generalized projection execution");
...
/// Executes the currently approved generalized local projection slice.
///
/// This is intentionally limited to local `.vortex` scan-pushdown evidence. It
/// does not add new readers, object-store access, decoded fallback,
...
pub fn execute_vortex_generalized_projection_from_local_scan_pushdown(
    request: &VortexQueryPrimitiveRequest,
    policy: VortexLocalPrimitiveExecutionPolicy,
) -> Result<VortexGeneralizedProjectionExecutionReport> {
    if !matches!(
        request.kind,
...
fn generalized_projection_local_scan_pushdown_safe(
    ...
    projection_safe
        && match report.primitive_kind {
            VortexQueryPrimitiveKind::ProjectColumns => true,
            VortexQueryPrimitiveKind::FilterAndProject => {
                report.rows_selected == report.rows_projected
...
mod tests {
    ...
    fn copied_struct_fixture_path(name: &str) -> std::path::PathBuf {
        ...
        let temp_path = std::env::temp_dir().join(format!(
            "shardloom-generalized-projection-{name}-{}-{nanos}.vortex",
            std::process::id()
        ));
    ...
    #[cfg(feature = "vortex-local-primitives")]
    #[test]
    fn generalized_projection_executes_copied_local_vortex_project_scan_pushdown() {
        let path = copied_struct_fixture_path("project");
        ...
        assert_eq!(report.local_primitive_report.rows_projected, Some(5));
    ...
    #[cfg(feature = "vortex-local-primitives")]
    #[test]
    fn generalized_projection_executes_copied_local_vortex_filter_project_scan_pushdown() {
        let path = copied_struct_fixture_path("filter-project");
        ...
        assert_eq!(report.local_primitive_report.rows_projected, Some(3));
        assert_eq!(report.projected_columns, vec!["metric".to_string()]);
        assert!(report.runtime_execution_allowed);
        assert!(report.encoded_projection_guaranteed);
        assert!(report.selection_vector_guaranteed);
    ...
    #[test]
    fn generalized_projection_rejects_filter_only_without_execution() {
        ...
        assert_eq!(
            report.status,
            VortexGeneralizedProjectionExecutionStatus::BlockedUnsupportedPrimitive
        );
        assert!(!report.runtime_execution_allowed);
        assert!(!report.encoded_projection_guaranteed);
