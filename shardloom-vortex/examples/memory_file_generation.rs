...
mod native {
    ...
    use shardloom_vortex::{
        ...
        memory_file_generation::MemoryFileGenerationBounds,
    ...
    pub fn run() -> Result<(), Error> {
        ...
        for _ in 0..iterations {
            ...
            let started = Instant::now();
            let ordinary = black_box(
                source
                    .prepare_projection(&COLUMNS, filter(), Some(7))?
                    .execute()?,
            );
            ordinary_samples.push(u64::try_from(started.elapsed().as_nanos())?);
            verify(&ordinary, &fixture.expected_query)?;
            drop(ordinary);
            let started = Instant::now();
            let result = black_box(generation.collect(&COLUMNS, filter(), 7, 64 * 1024)?);
            generation_samples.push(u64::try_from(started.elapsed().as_nanos())?);
        ...
        let report = json!({
            ...
            "encoding_policy": "one native Flat segment retaining typed primitive, nullable varbin and boolean encodings; no compression transform",
