...
The Field Guide is generated by `build_static_pages.py` from a small in-repo concept registry.
Each dossier includes a one-sentence answer, why the concept matters, how ShardLoom uses it, what
it proves, what it does not prove, evidence fields, related concepts, source docs, and a claim
boundary. Keep the dossiers educational and claim-safe; they must not imply runtime support beyond
the evidence already present in the repository.
