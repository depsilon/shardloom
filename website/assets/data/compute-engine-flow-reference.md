...
| SQL local-source smoke | `sql-local-source-smoke` admits local CSV direct-transient projection/filter/limit, scalar aggregate, and one-column group-by aggregate shapes. The aggregate slices cover `COUNT(*)`, `COUNT(column)`, `SUM`, `AVG`, `MIN`, and `MAX` after the same simple `WHERE` predicate. The scalar path emits `execution_certificate_ref=sql-local-source.csv.aggregate-filter-limit.execution.v1`; the group-by path emits `execution_certificate_ref=sql-local-source.csv.group-by-aggregate-filter-limit.execution.v1` plus `group_by_runtime_execution=true` evidence. It performs ShardLoom-owned parsing, binding, planning, local CSV read/parse, projection/filter/limit or aggregate evaluation, bounded inline JSONL result rendering or optional local JSONL output, and source/output/execution/materialization/no-fallback evidence. Python `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` wraps the projection path, and `ctx.read_csv(...).filter(...).aggregate(...).limit(1).collect()/write(...)` wraps the scalar aggregate path. | Later slices can add Python `ctx.sql(...)`, Python `group_by().agg(...)`, Parquet/Vortex SQL sources, broader output sinks, pushdown, multi-key/grouped aggregate generality, joins, order/top-N, and broader planner lowering only with source/sink/correctness evidence. | This is a scoped fixture-smoke local CSV SQL/Python query-builder path. It is not broad SQL/DataFrame support, not a production SQL claim, not an object-store/table source claim, and not performance evidence. |
...
projection/scalar runtime. Python grouped aggregation, adjacent formats, multi-key grouped
aggregate generality, joins, windows, broader result sinks, broad SQL/DataFrame access, and broader
transient runtime behavior still return deterministic unsupported diagnostics.
...
local CSV projection/filter/limit and scalar aggregate shapes; SQL group-by aggregate is CLI-only
until a later Python `group_by().agg(...)` wrapper slice.
