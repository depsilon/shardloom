...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=115`
and `executable_fixture_count=93`, including scoped UTF-8 regex predicates, composed string functions, temporal arithmetic/
...
IN/EXISTS/quantified subquery predicates, scoped correlated `outer.<column>` scalar/row-value IN/NOT IN, EXISTS/NOT EXISTS, and quantified filters, scoped source-qualified subquery diagnostics, scoped subquery-backed predicate/CASE projections, COUNT(DISTINCT), hidden HAVING aggregates, mixed
