...
| Output/fanout reports | Multi-output fanout route | One admitted source/result needs multiple local outputs. | Query/reuse timing, `ResultBatchState` creation, sink-driven OutputPlan requirements, output conversion, per-output planning, write, replay, and evidence. | Shared fanout conversion DAG completion, object-store/table commit, broad nested writer fidelity, or production sink support. |
