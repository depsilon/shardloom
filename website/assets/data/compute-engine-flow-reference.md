...
through `GAR-IOREUSE-1K` now cover cold-lane attribution, Vortex-native source/sink/split
preparation, differential preparation, and capillary I/O with PulseWeave control on the existing
`vortex_ingest` route. `GAR-IOREUSE-1L` remains the scout ingress/triage follow-up, and
`GAR-PERF-2J` / `GAR-PERF-2K` cover layout/write advice and cold-lane copy/buffer evidence. The
flow must keep cold costs visible instead of treating them as query compute or hiding them behind
`auto`.
