...
`source_state_ranked_metric_*` fields. All batch source-state paths emit
...
`source_state_group_category_metric_*`, and `source_state_ranked_metric_*` fields to show which
scoped source-state family was reused. Rows that still use explicit single-scenario CLI
...
across sort/top-k, top-N per group, and row-number/window children. These reuse families emit
explicit `source_state_*` evidence and remain residual-native runtime plumbing.
