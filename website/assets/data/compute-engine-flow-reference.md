...
4D/5G/F1 closeout note: current release-gated admitted semantics now include `matrix_row_count=45`
and `executable_fixture_count=30`, including scoped UTF-8 regex predicates, composed string functions, temporal arithmetic/
...
non-admitted quantified shapes while scoped EXISTS, scoped quantified ANY/ALL, scoped UTF-8 regex predicates, and scoped interval literals inside temporal helper functions are admitted. `runtime.5g-f1` now exposes the current physical operator plan, operator
