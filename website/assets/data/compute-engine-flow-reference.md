...
`source_state_ranked_metric_*` fields. For clean/cast/filter/write plus malformed timestamp / dirty
...
`source_state_dirty_input_*` fields to show which scoped source-state family was reused. Rows that
...
across sort/top-k, top-N per group, and row-number/window children, and can reuse one dirty-input
cleanup state across clean/cast/filter/write and malformed timestamp / dirty CSV children. These
reuse families emit explicit `source_state_*` evidence and remain residual-native runtime plumbing.
