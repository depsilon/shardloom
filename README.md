...
| SQL local-source smoke | `sql-local-source-smoke` admits one scoped local CSV shape: `SELECT <columns> FROM '<local.csv>' WHERE <simple predicate> LIMIT <n>`. It parses, binds, plans, reads local CSV rows, applies ShardLoom-owned projection/filter/limit semantics, prints bounded inline JSONL or writes optional local JSONL output, and emits source, output, execution, materialization/decode, no-fallback, and claim-gate evidence. Python `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` wraps this same admitted path. | Broader SQL over Parquet/Vortex, joins, aggregates, functions, subqueries, catalogs, object-store/table sources, and production SQL/DataFrame support remain gated. |
...
- scoped local CSV SQL execution through `sql-local-source-smoke` for one
  `SELECT <columns> FROM '<local.csv>' WHERE <simple predicate> LIMIT <n>` shape, with parser,
  binder, planner, local source-read, ShardLoom-native projection/filter/limit, bounded inline JSONL
  result evidence, optional local JSONL output evidence, materialization/decode fields, and
  no-fallback/no-external-engine evidence; Python query-builder
