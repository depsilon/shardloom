...
- scoped local CSV SQL execution through `sql-local-source-smoke` for projection/filter/limit,
  scalar aggregate, one-column group-by aggregate, and single-key numeric `ORDER BY ... LIMIT ...`
  top-N shapes, with parser, binder, planner, local source-read, ShardLoom-native projection/
  filter/limit or `COUNT`/`SUM`/`AVG`/`MIN`/`MAX` or top-N semantics, bounded inline JSONL result
  evidence, optional local JSONL output evidence, materialization/decode fields, and no-fallback/
  no-external-engine evidence; Python query-builder
  ...
  same scoped projection/scalar/grouped/top-N paths. This is not broad SQL/DataFrame runtime,
  generalized grouped aggregate or ordering support, a SQL compatibility claim, object-store/table
  support, or a production claim
