...
- scoped local CSV/JSONL SQL execution through `sql-local-source-smoke` for projection/filter/limit,
  ...
  source-read, ShardLoom-native projection/filter/limit or `COUNT`/`SUM`/`AVG`/`MIN`/`MAX` or top-N
  or hash-join semantics, bounded inline JSONL result evidence, optional local JSONL output
  ...
  `filter(...).aggregate(...).limit(1).collect()/write(...)`,
  `filter(...).group_by(...).agg(...).limit(n).collect()/write(...)`, and
  `select(...).filter(...).sort(...).limit(n).collect()/write(...)` wrap these same scoped
