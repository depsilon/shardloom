...
- scoped local CSV SQL execution through `sql-local-source-smoke` for projection/filter/limit and
  scalar aggregate shapes, with parser, binder, planner, local source-read, ShardLoom-native
  projection/filter/limit or `COUNT`/`SUM`/`AVG`/`MIN`/`MAX`, bounded inline JSONL result evidence,
  optional local JSONL output evidence, materialization/decode fields, and no-fallback/no-external-engine
  evidence; Python query-builder
  `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` and
  `ctx.read_csv(...).filter(...).aggregate(...).limit(1).collect()/write(...)` wrap these same
  scoped paths; this is not broad SQL/DataFrame runtime, grouped aggregate support, a SQL
