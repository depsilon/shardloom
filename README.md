...
- scoped source-free user-row, literal-table, calendar/date-dimension, range, sequence, SQL
  ...
  generator smoke
- scoped local CSV/JSONL/flat JSON SQL execution through `sql-local-source-smoke` for projection/optional-filter/limit,
  ...
  `.json`/`.jsonl`/`.ndjson` sources, plus CSV or flat JSON/JSONL/NDJSON
