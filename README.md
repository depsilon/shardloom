...
- scoped local CSV SQL execution through `sql-local-source-smoke` for projection/filter/limit and
  ...
  `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` wraps this same scoped
  path; this is not broad SQL/DataFrame runtime, a SQL compatibility claim, object-store/table
