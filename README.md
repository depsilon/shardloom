...
| Source-free generated output | Scoped local JSONL smokes exist for caller-provided rows through `generated-source-user-rows-smoke` / `ctx.from_rows([...]).write(...)`, Python literal/calendar helpers through `ctx.literal_table([...]).write(...)` and `ctx.calendar(...).write(...)`, and one ShardLoom-native range generator through `generated-source-range-smoke` / `ctx.range(...).write(...)`, with generated-source and output evidence and no source Native I/O certificate. | Sequence/SQL `VALUES`/literal execution, broad DataFrame runtime, object-store writes, and Foundry generated-output runtime remain gated. |
...
- scoped source-free user-row, literal-table, calendar/date-dimension, and range generated output to
