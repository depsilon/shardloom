...
- scoped prepared/native Vortex query paths for `selective filter`, `wide projection`,
  ...
  `value:vortex.sequence` chunks into admitted kernel inputs, and keep encoded-native claims blocked
  until selected metric aggregation consumes the admitted selection vector with certificates and
