...
| SQL local-source smoke | `sql-local-source-smoke` admits scoped local CSV projection/filter/limit, scalar aggregate, and one-column group-by aggregate shapes. The aggregate slices cover `COUNT(*)`, `COUNT(column)`, `SUM`, `AVG`, `MIN`, and `MAX` after a simple `WHERE` predicate; group-by currently requires the selected group column to exactly match the `GROUP BY` column. It parses, binds, plans, reads local CSV rows, applies ShardLoom-owned projection/filter/limit or aggregate semantics, prints bounded inline JSONL or writes optional local JSONL output, and emits source, output, execution, materialization/decode, no-fallback, and claim-gate evidence. Python `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` wraps the projection path, and `ctx.read_csv(...).filter(...).aggregate(...).limit(1).collect()/write(...)` wraps the scoped scalar aggregate path. | Broader SQL over Parquet/Vortex, joins, multi-key/grouped aggregate generality, functions, subqueries, catalogs, object-store/table sources, Python `group_by().agg(...)`, and production SQL/DataFrame support remain gated. |
...
- scoped local CSV SQL execution through `sql-local-source-smoke` for projection/filter/limit and
  ...
  `ctx.read_csv(...).filter(...).aggregate(...).limit(1).collect()/write(...)` wrap these same
  scoped projection/scalar paths; Python grouped aggregation remains blocked. This is not broad
  SQL/DataFrame runtime, generalized grouped aggregate support, a SQL compatibility claim,
