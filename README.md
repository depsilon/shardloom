...
- scoped prepared/native Vortex query paths for `selective filter`, `wide projection`,
  ...
  `encoded_predicate_provider_*` fields that block encoded predicate claims until reader-backed
  encoded value batches, selection-vector evidence, certificates, and no-fallback evidence are
  attached
