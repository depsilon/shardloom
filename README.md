...
| SQL local-source smoke | `sql-local-source-smoke` admits scoped local CSV projection/filter/limit, scalar aggregate, and one-column group-by aggregate shapes. The aggregate slices cover `COUNT(*)`, `COUNT(column)`, `SUM`, `AVG`, `MIN`, and `MAX` after a simple `WHERE` predicate; group-by currently requires the selected group column to exactly match the `GROUP BY` column. It parses, binds, plans, reads local CSV rows, applies ShardLoom-owned projection/filter/limit or aggregate semantics, prints bounded inline JSONL or writes optional local JSONL output, and emits source, output, execution, materialization/decode, no-fallback, and claim-gate evidence. Python `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)`, `ctx.read_csv(...).filter(...).aggregate(...).limit(1).collect()/write(...)`, and `ctx.read_csv(...).filter(...).group_by(...).agg(...).limit(n).collect()/write(...)` wrap the scoped projection, scalar aggregate, and one-column group-by paths. | Broader SQL over Parquet/Vortex, joins, multi-key/grouped aggregate generality, named grouped aggregate aliases, functions, subqueries, catalogs, object-store/table sources, and production SQL/DataFrame support remain gated. |
| Source-free generated output | Scoped local JSONL smokes exist for caller-provided rows through `generated-source-user-rows-smoke` / `ctx.from_rows([...]).write(...)`, Python literal/calendar helpers through `ctx.literal_table([...]).write(...)` and `ctx.calendar(...).write(...)`, one ShardLoom-native range generator through `generated-source-range-smoke` / `ctx.range(...).write(...)`, and source-free SQL `VALUES` / literal `SELECT` through `generated-source-sql-smoke` / `ctx.sql_values(...).write(...)` / `ctx.sql_literal_select(...).write(...)`, with generated-source and output evidence and no source Native I/O certificate. | Sequence/generate_series, broad SQL/DataFrame runtime, object-store writes, and Foundry generated-output runtime remain gated. |
...
- scoped source-free user-row, literal-table, calendar/date-dimension, range, SQL `VALUES`, and SQL literal `SELECT` generated output to
  local JSONL with generated-source certificate status, output Native I/O certificate status, output
  digest, and no-fallback/no-external-engine evidence
- scoped local CSV SQL execution through `sql-local-source-smoke` for projection/filter/limit and
  scalar aggregate plus one-column group-by aggregate shapes, with parser, binder, planner, local
  source-read, ShardLoom-native projection/filter/limit or `COUNT`/`SUM`/`AVG`/`MIN`/`MAX`, bounded
  inline JSONL result evidence, optional local JSONL output evidence, materialization/decode fields,
  and no-fallback/no-external-engine evidence; Python query-builder
  `ctx.read_csv(...).select(...).filter(...).limit().collect()/write(...)` and
  `ctx.read_csv(...).filter(...).aggregate(...).limit(1).collect()/write(...)` and
  `ctx.read_csv(...).filter(...).group_by(...).agg(...).limit(n).collect()/write(...)` wrap these
  same scoped projection/scalar/grouped paths. This is not broad SQL/DataFrame runtime,
  generalized grouped aggregate support, a SQL compatibility claim, object-store/table support, or
  a production claim
