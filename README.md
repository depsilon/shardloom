...
| SQL local-source smoke | `sql-local-source-smoke` admits scoped local CSV/JSONL/flat JSON plus feature-gated flat scalar Parquet projection/optional-filter/limit, explicit literal projections shaped as `<literal> AS <column>`, scalar aggregate, one-column group-by aggregate, single-key numeric `ORDER BY ... LIMIT ...` top-N, scoped `CAST(column AS dtype)` predicates for `int64`, `float64`, `utf8`, `boolean`, and `date32`, scoped Date32 `DATE_YEAR(column)` / `DATE_MONTH(column)` / `DATE_DAY(column)` extract predicates, scoped Date32 `DATE_ADD_DAYS(column, days)` / `DATE_SUB_DAYS(column, days)` predicates, and bounded `column IN (<literal>,...)` predicates over local rows, plus one explicit local CSV inner equi-join shape with aliases. The aggregate slices cover `COUNT(*)`, `COUNT(column)`, `SUM`, `AVG`, `MIN`, and `MAX` across all rows or after a scoped `WHERE` predicate; group-by currently requires the selected group column to exactly match the `GROUP BY` column. The top-N slice admits one numeric non-null sort key with `ASC` or `DESC`; null ordering, expressions, collation, and multi-key sorts are deterministic blockers. Parquet local-source admission and Parquet local-output writes require `shardloom-cli --features universal-format-io`; default builds return deterministic Parquet adapter/sink blockers. The date extract slice emits `date_extract_runtime_execution`, `date_extract_operator`, and `date_extract_source_column` evidence; the date arithmetic slice emits `date_arithmetic_runtime_execution`, `date_arithmetic_operator`, `date_arithmetic_days`, and `date_arithmetic_source_column` evidence; unsupported extract literals, day counts, and non-Date32 shapes block before fallback. The `IN` predicate slice admits up to 32 non-null literals of one scalar family, including `DATE 'YYYY-MM-DD'` lists, and emits `in_predicate_runtime_execution` plus `in_list_value_count`; empty, NULL, mixed DATE/non-DATE, and oversized lists are deterministic blockers. The join slice admits `FROM '<left.csv>' AS f [INNER] JOIN '<right.csv>' AS d ON f.key = d.key [WHERE <qualified predicate>] LIMIT <n>` and emits left/right source, key, row-count, matched-row, output-row, memory-estimate, execution-certificate, no-fallback, and claim-gate evidence. It parses, binds, plans, reads local rows, applies ShardLoom-owned projection/literal-projection/optional-filter/limit, cast/date-extract/date-arithmetic/IN predicate, aggregate, top-N, or join semantics, prints bounded inline JSONL or writes optional local JSONL/CSV plus feature-gated flat scalar Parquet output with format-specific sink certificate fields, and emits source, adapter, route, output, execution, materialization/decode, no-fallback, and claim-gate evidence. Python wraps the scoped CSV, flat `.json`/`.jsonl`/`.ndjson`, and local `.parquet` projection, literal `with_column(...)`, scalar aggregate, one-column group-by, preview/select-star, single-key numeric top-N, and local CSV inner equi-join paths, including local JSONL writes, `write_csv(...)`, feature-gated `write_parquet(...)`, and `sl.col(...)` predicate helpers for scoped comparison/null/string/IN/cast/date/date-extract/date-arithmetic/logical filters. | Broader SQL over Vortex, nested JSON/JSONPath, broader Parquet type/nesting coverage, broader Python/DataFrame joins, Arrow/Avro/ORC/Vortex output sinks, broader Parquet output feature parity, multi-output fanout, multi-key/expression/outer/semi/anti/cross joins, non-literal `with_column`, multi-key/grouped aggregate generality, named grouped aggregate aliases, functions beyond admitted scalar helpers, subqueries, catalogs, object-store/table sources, timestamp/timezone completeness, collation/null-ordering parity, window ranking, and production SQL/DataFrame support remain gated. |
| Source-free generated output | Scoped local JSONL/CSV smokes exist for caller-provided rows through `generated-source-user-rows-smoke` / `ctx.from_rows([...]).write(...)`, Python literal/calendar helpers through `ctx.literal_table([...]).write(...)` and `ctx.calendar(...).write(...)`, ShardLoom-native range/sequence generators through `generated-source-range-smoke` / `ctx.range(...).limit(...).write(...)` and `generated-source-sequence-smoke` / `ctx.sequence(...).take(...).write(...)`, and source-free SQL `VALUES`, literal `SELECT`, and `SELECT * FROM generate_series/range(...)` through `generated-source-sql-smoke` / `ctx.sql_values(...).write(...)` / `ctx.sql_literal_select(...).write(...)` / `ctx.sql(...).write(...)`, with generated-source and output evidence and no source Native I/O certificate. | Broad SQL/DataFrame runtime, object-store writes, and Foundry generated-output runtime remain gated. |
...
- scoped local CSV/JSONL/flat JSON plus feature-gated flat scalar Parquet SQL execution through `sql-local-source-smoke` for projection/optional-filter/limit,
  ...
  `date32`, scoped Date32 `DATE_YEAR`/`DATE_MONTH`/`DATE_DAY` extract predicates with
  `date_extract_*` evidence, scoped Date32 `DATE_ADD_DAYS(column, days)` /
  `DATE_SUB_DAYS(column, days)` predicates with date-arithmetic evidence fields, and one explicit
  ...
  or hash-join semantics, bounded inline JSONL result evidence, optional local JSONL/CSV or
  feature-gated flat scalar Parquet output evidence with format-specific sink certificate refs,
  materialization/decode fields, and no-fallback/no-external-engine evidence; Python
  query-builder
  ...
  `.json`/`.jsonl`/`.ndjson` sources, feature-gated
  `ctx.read_parquet(...).select(...).limit().collect()/write(...)` over local flat scalar Parquet,
  `write_parquet(...)` for feature-gated flat scalar Parquet local output,
  plus CSV, flat JSON/JSONL/NDJSON, or admitted Parquet
  ...
  inclusive `between(...)`, null, string `LIKE`, bounded `IN`, cast/date, Date32 extract/day arithmetic,
  and logical predicates into the same ShardLoom-owned runtime;
  ...
  join support, non-literal `with_column`, nested JSON runtime, broad Parquet output parity,
  generalized join/grouped aggregate/ordering support,
  timestamp/timezone completeness, a SQL compatibility claim, object-store/table support, or a production claim
