...
- scoped local CSV SQL execution through `sql-local-source-smoke` for one
  ...
  result evidence, materialization/decode fields, and no-fallback/no-external-engine evidence; this
  is not broad SQL/DataFrame runtime, a SQL compatibility claim, object-store/table support, or a
  production claim
