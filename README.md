...
- scoped local CSV/JSONL SQL execution through `sql-local-source-smoke` for projection/optional-filter/limit,
  ...
  `date32`, scoped Date32 `DATE_ADD_DAYS(column, days)` / `DATE_SUB_DAYS(column, days)` predicates
  with date-arithmetic evidence fields, and one explicit
  ...
  inclusive `between(...)`, null, string `LIKE`, bounded `IN`, cast/date, Date32 day arithmetic,
