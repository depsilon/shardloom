...
- scoped prepared/native Vortex query paths for `selective filter`, `wide projection`,
  ...
  selection-vector bridge contract, and block encoded predicate claims until the observed
  `flag:fastlanes.bitpacked` / `value:vortex.sequence` chunks can be lowered into admitted kernel
  inputs with certificates and no-fallback evidence
