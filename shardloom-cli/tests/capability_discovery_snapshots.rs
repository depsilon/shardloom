...
fn cross_cg_capability_parity_surfaces_shared_blocker_contracts() {
    ...
    assert!(workflow.contains(&string_field_pair("workflow_operation_count", "35")));
    assert!(workflow.contains(&string_field_pair(
        "workflow_operation_names",
    ...
    assert!(cross_cg.contains(&string_field_pair(
        "required_evidence",
        "execution_certificate,native_io_certificate,operator_capability_matrix,semantic_conformance_suite,sql_parser,binder,write_intent,rest_api_contract,decoded_columnar_boundary,python_object_boundary,schema_metadata_report,data_quality_report,notebook_display_boundary,workload_correctness_evidence,benchmark_evidence,broad_source_sink_certification,durable_checkpoint_store,object_store_commit_protocol,openapi_contract,asyncapi_contract,execution_certificate,native_io_certificate,security_governance_policy,data_plane_fidelity_report"
    )));
    assert!(cross_cg.contains(&string_field_pair(
        "suggested_next_action",
