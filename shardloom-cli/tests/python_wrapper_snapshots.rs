...
fn python_wrapper_plan_json_exposes_cli_json_foundation() {
    ...
    assert!(output.contains(&field(
        ...
        "status,capabilities,api-compat-plan,python-wrapper-plan,vortex-run,traditional-analytics-run,traditional-analytics-vortex-run,traditional-analytics-vortex-batch-run,dynamic-work-shaping-plan,sizing-feedback-plan,benchmark-plan,benchmark-claim-evidence-plan,benchmark-constitution"
    )));
}
