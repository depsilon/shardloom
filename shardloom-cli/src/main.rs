...
fn run(args: Vec<String>) -> ExitCode {
    ...
    match args.next().as_deref() {
        ...
        Some("workload-certification-dossier") => {
            workload_certification::handle_workload_certification_dossier(args, format)
        }
        Some("claim-gate-closeout") => {
            workload_certification::handle_claim_gate_closeout(args, format)
        }
        Some("global-architecture-gate") => {
            status_capabilities::handle_global_architecture_gate(args, format)
        }
        Some("compute-capability-matrix") => {
            status_capabilities::handle_compute_capability_matrix(args, format)
        }
        Some("semantic-conformance-suite") => {
            semantic_conformance::handle_semantic_conformance_suite(args, format)
        }
...
mod tests {
    ...
    #[test]
    fn object_store_read_smoke_local_emulator_returns_success() {
        let fixture = std::env::temp_dir().join(format!(
            "shardloom-cli-object-store-read-smoke-{}.bin",
            std::process::id()
        ));
    ...
    fn usage_includes_cg22_engine_fabric_plans() {
        ...
        assert!(cli_usage_line().contains("compute-capability-matrix"));
        assert!(cli_usage_line().contains("semantic-conformance-suite"));
        assert!(cli_usage_line().contains("capabilities [sql|functions"));
        assert!(
            cli_usage_line().contains(
                "security-governance|engines|workflow|remote-api|cross-cg|compatibility]"
            )
        );
    }
    #[test]
    ...
    fn usage_includes_local_delete_tombstone_read_smoke() {
        assert!(cli_usage_line().contains("local-delete-tombstone-read-smoke"));
    }
    #[test]
    ...
    fn capabilities_usage_lists_certification_scopes() {
        ...
        assert!(usage.contains("workflow|remote-api|cross-cg|compatibility]"));
    }
